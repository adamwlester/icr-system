...
float runSpeed;
bool haltRun = true;
int16_t t_lastRunUpdate = millis();
...
int vtTS[2];
float vtPosCM[2];
const float vtTarg = 0.78 * ((140 * PI) / (2 * PI)); // (cm)
// Pixy
...
void UpdateSpeed()
{
	...
	float runScale = abs(velDif)*scale;
	
	// Update run speed value
	// match rat speed
	runSpeed = estRobVel + velDif;
	runSpeed = runSpeed  + runScale;

	t_lastRunUpdate = millis();
	
	// Keep speed in range [0, maxSpeed]
	if (runSpeed > maxSpeed) runSpeed = maxSpeed;
	else if (runSpeed < 0.0f) runSpeed = 0.0f;
