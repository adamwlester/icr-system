...
float estRatVel;
float estRobVel;
float posArr[M] = { 0, 0, 0, 0, 0, 0 }; // ekf measurements
long posTS[2][3]; // ts history
float posRad[2][3]; // pos history
...
void loop() {
	...
	if (Serial1.available() > 0)
	{
		...
		if (pass)
		{
			...
			// pos
			posRad[0][vtEnt] = posRad[1][vtEnt];
			posRad[1][vtEnt] = vtRad[vtEnt];
		}
	}
	...
	if (posUpdate[0] && posUpdate[1] && posUpdate[2])
	{
		...
		// Save pixy rat pos
		posRad[0][2] = posRad[1][2];
		posRad[1][2] = PixyCM_ToRad(pixyPosCM, vtRad[1]);

		double z[M] = {
			posRad[1][0],
			posRad[1][2],
			posRad[1][1],
			CompRadVel(posRad[0][0], posRad[1][0], posTS[0][0], posTS[1][0]),
			CompRadVel(posRad[0][2], posRad[1][2], posTS[0][2], posTS[1][2]),
			CompRadVel(posRad[0][1], posRad[1][1], posTS[0][1], posTS[1][1]),
		};
		if (firstPass)
		{
			z[3] = 0;
