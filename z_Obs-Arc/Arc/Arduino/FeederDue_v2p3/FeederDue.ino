...
volatile bool doCheckDoneRcvd = false;
...
void loop() {
	...
	else
	{
		...
		// Check for (Y) done recieved confirmation
		else if (msg_id == c2r_id[8] && msg_pass)
		{
		...
		if (isMoving && !isDoneMoving)
		{
			...
			if (isDoneMoving)
			{
				...
				Send2_CS(r2c_id[6], c2r_packLast[2]);
		...
		{
			Send2_CS(r2c_id[6], c2r_packLast[7]);
			doStreamCheck = false;
		...
		if (doCheckDoneRcvd && millis() > t_doneResend)
		{
			...
			Send2_CS(r2c_id[6], r2c_packLast[6]);
		...
		if (doTrackRun && !isMoving && millis() > t_holdTill)
		{
			//board.run(FWD, 100);
			//doTrackRun = false;
		}
