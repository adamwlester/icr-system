    ...
    while true
        ...
        if ...
            ...
            if data(2) ~= 0
                ...
                % Wall cue image
                img_fi = imgRunFiles{img_ind};
            else
                
                % Dark image
                img_fi = imgBackFiles{2};
            end
            
            % Show image trigger
            frame_java = fullscreenAWL(fullfile(imgDir,subDir{2},img_fi));
            
            % Pause to allow trigger to register
            pause(trgDel);
            
            % Show main image
            frame_java = fullscreenAWL(fullfile(imgDir,subDir{1},img_fi));
            
            % Print status message
            fprintf('\r\tchange image to \"%s\": %s\r', ...
                img_fi, datestr(now, 'HH:MM:SS AM'))
