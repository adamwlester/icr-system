...
% on the ICR. Data is saved to "SS_IO_1.mat", "SS_IO_1.mat" and
...
PAR.dobList = cellstr(datestr(PAR.dobList, 'yyyy/mm/dd'));
...
T.Include_Analysis = true(length(PAR.ratList),1);

T.Implanted = false(length(PAR.ratList),1);

T.Yoke_Mate = categorical(PAR.yokeList, ...
    [{'None'}; PAR.ratList]);

T.Feeder_Version = categorical(repmat({'Mobile_Feeder'},length(PAR.ratList),1), ...
    feeder_version_cats);

T.Age_Group = categorical(PAR.ageList, age_group_cats);

T.DOB = PAR.dobList;

T.Tail_Mark = PAR.tailMrkList;

T.Weight_Baseline = PAR.weightList;

T.Human = categorical(repmat({'AWL'},length(PAR.ratList),1), ...
    human_cats);

T.Session_Type = categorical(repmat({'ICR_Session'},length(PAR.ratList),1), ...
    session_type_cats);

T.Session_Condition = categorical(repmat({'Manual_Training'},length(PAR.ratList),1), ...
    session_condition_cats);

T.Session_Task = categorical(repmat({'Track'},length(PAR.ratList),1), ...
    session_task_cats);

T.Session_Manual_Training = repmat([0, 0],length(PAR.ratList),1);

T.Session_Behavior_Training = repmat([0, 0],length(PAR.ratList),1);
...
T.Finished_Behavior_Training = false(length(PAR.ratList),1);

T.Finished_Study = false(length(PAR.ratList),1);

T.Feeder_Condition = categorical(repmat({'<undefined>'},length(PAR.ratList),1), ...
    feeder_condition_cats);

T.Reward_Delay = categorical(repmat({'0.0'},length(PAR.ratList),1), ...
    reward_delay_cats, 'Ordinal', true);

T.Cue_Condition = categorical(repmat({'None'},length(PAR.ratList),1), ...
    cue_condition_cats, 'Ordinal', true);

T.Sound_Conditions = repmat([false, false], length(PAR.ratList) ,1);

T.Start_Quadrant = repmat({categorical(repmat({'<undefined>'},200,1), ...
    start_quadrant_cats)}, length(PAR.ratList),1);

T.Rotation_Direction = repmat({categorical(repmat({'<undefined>'},200,1), ...
    rotation_direction_cats)}, length(PAR.ratList),1);

T.Rotation_Positions = repmat({categorical(repmat({'<undefined>'},200,9), ...
    rotation_positions_cats)}, length(PAR.ratList),1);

T.Rotations_Per_Session = repmat({categorical(repmat({'<undefined>'},200,1), ...
    rotations_per_session_cats)}, length(PAR.ratList),1);

T.Laps_Per_Rotation = repmat({categorical(repmat({'<undefined>'},200,9), ...
    laps_per_rotation_cats)}, length(PAR.ratList),1);
...
if exist(fullfile(ioDir, 'SS_IO_1.mat'), 'file')
    ...
    exc_rats = PAR.ratList(ismember(PAR.ratList,SS_IO_1.Properties.RowNames));
    if ~isempty(exc_rats)
...
nUnique = sum(ismember(PAR.yokeList, 'None')) + sum(ismember(PAR.ratList, PAR.yokeList))/2;
...
[~,mateInd] = ismember(PAR.ratList, PAR.yokeList);
yokePrs = [(1:length(PAR.ratList))',mateInd];
...
for i = 1:length(PAR.ratList)
    ...
    SS_IO_1{char(PAR.ratList(i)), {'Feeder_Condition'}} = ...
        fdcnd(yokeInd(i));
    
    SS_IO_1{char(PAR.ratList(i)), {'Start_Quadrant'}}{:} = ...
        categorical(strqd(:,yokeInd(i)), start_quadrant_cats);
    
    SS_IO_1{char(PAR.ratList(i)), {'Rotation_Direction'}}{:} = ...
        categorical(rtdir(:,yokeInd(i)), rotation_direction_cats);
    
    SS_IO_1{char(PAR.ratList(i)), {'Rotation_Positions'}}{:} = ...
        categorical(rotps(:,:,yokeInd(i)), rotation_positions_cats);
    
    SS_IO_1{char(PAR.ratList(i)), {'Rotations_Per_Session'}}{:} = ...
        categorical(nrot(:,yokeInd(i)), rotations_per_session_cats);
    
    SS_IO_1{char(PAR.ratList(i)), {'Laps_Per_Rotation'}}{:} = ...
        categorical(nlap(:,:,yokeInd(i)), laps_per_rotation_cats);
    
    SS_IO_1{char(PAR.ratList(i)), {'Days_Till_Rotation'}}{:} = ...
