...
while D.B.poll_nlx
    ...
    if D.B.ratIn
        ...
        % Run ozone trigger check code
        D = SF_Ozone_Trigger_Check(D);
...
D.I.lap_hunt_ind = 1;
% current lap quadrant for rew reset track
D.I.rew_reset_hunt_ind = 1;
% hd sample index
D.I.rv_smp_ind = 0;
...
% feeder solonoids
D.NLX.port_solnd_bit_Ind = ...
    [48 - D.NLX.solnd_bit(1), ...
    48 - D.NLX.solnd_bit(2)];
% sound/air
...
% feeders
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.solnd_bit(1)), ' TTL_Feeder_1_Open']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.solnd_bit(2)), ' TTL_Feeder_2_Open']);
% audio channels
...
% SETUP DONE
    function [] = BtnStupDn(~, ~, ~)
        ...
        % Calculate crossing points for 90 180 and 270 deg from rew feeders
        ozcuBndLocs = [...
            (fdLocs(D.UI.rewFeed(1))-60:-10:fdLocs(D.UI.rewFeed(1))-300)', ...
            (fdLocs(D.UI.rewFeed(2))-60:-10:fdLocs(D.UI.rewFeed(2))-300)'];
        
        % Calculate 10 deg wide bounds for each reward feeder
        D.UI.ozcuBnds = arrayfun(@(x,y) cat(3, [x, x-10], [y, y-10]), ...
            ozcuBndLocs(:,1), ozcuBndLocs(:,2), 'Uni', false);
        D.UI.ozcuBnds = cell2mat(D.UI.ozcuBnds);
        % set range to [0, 360]
        D.UI.ozcuBnds = wrapTo360(D.UI.ozcuBnds);
        % remove zero crossing
        D.UI.ozcuBnds(any(diff(D.UI.ozcuBnds,1,2)>0,3),:,:) = [];
        % convert to radians
        D.UI.ozcuBnds = deg2rad(D.UI.ozcuBnds);
        
        % Set initial ozone and cue bounds to start quad
        D.UI.ozBnd = D.UI.strQuadBnds;
...
% FEEDER FILL
    function [] = BtnFillFd(~, ~, ~)
        ...
        ud_othr = ... % other button user data
            get(D.UI.btnFillFd([1, 2] ~=  ud_sltd), 'UserData');
        ...
        if strcmpi(get(D.UI.timFdFillDur, 'Running'), 'Off')
            ...
            % change command
            D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(ud_sltd)) = '1';
            % post command
            NlxSendCommand(D.NLX.port_out_update_cmd);
...
% FEEDER CLEAN
    function [] = BtnCleanFd(~, ~, ~)
        ...
        ud_othr = ...
            get(D.UI.btnCleanFd([1, 2] ~=  ud_sltd(1)), 'UserData');
        % update user data for selected button
        ud_sltd = [ud_sltd(1), find([1, 2] ~=  ud_sltd(2))];
        
        % Turn on/off feeder by changing/posting NLX ttl comand
        if ud_sltd(2) == 1
            % set feeder bit high
            D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(ud_sltd(1))) = '1';
        else
            % set feeder bit low
            D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(ud_sltd(1))) = '0';
        end
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
% FEEDER FILL TIMER STOP
    function [] = StopFill(~, ~, ~)
        ...
        % change command
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(ud_sltd)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
% FEEDER REWARD TIMER STOP
    function [] = StopFeed(~, ~, ~)
        ...
        % Turn off feeder by changing/posting NLX ttl comand
        % feed solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(D.I.feed_ind(D.I.rot))) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
if ...
    ...
    if ...
        ...
        % Turn on feeder by changing/posting NLX ttl comand
        % feed solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(D.I.feed_ind(D.I.rot))) = '1';
        % white noise bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = '0';
        % reward tone bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(2)) = ...
        ...
        if ...
            ...
            % Get new bounds
            D.UI.cueBnd = ...
                D.UI.ozcuBnds(ceil(rand(1,1)*size(D.UI.ozcuBnds,1)), :, D.I.rot);
            ...
            % Plot new patch
            [xbnd, ybnd] =  Get_Trk_Bnds(D,D.UI.cueBnd);
            D.UI.ptchCuTrg = ...
                patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                [ybnd(1,:),fliplr(ybnd(2,:))], ...
                D.UI.cuTrgCol, ...
        ...
        if D.UI.sndair(5)
            ...
            % Plot new patch
            [xbnd, ybnd] =  Get_Trk_Bnds(D,D.UI.ozBnd);
            D.UI.ptchOzTrg = ...
                patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                [ybnd(1,:),fliplr(ybnd(2,:))], ...
                D.UI.ozTrgCol, ...
...
% If all bounds not triggered yet
if ~all(D.B.track_rew_reset)
    
    % Check if rat in reward reset quad bound
    track_quad = ...
        D.P.rad >= min(D.UI.rewRstBnds(D.I.rew_reset_hunt_ind, :, D.I.rot)) & ...
        D.P.rad <= max(D.UI.rewRstBnds(D.I.rew_reset_hunt_ind, :, D.I.rot));
    
    if any(track_quad);
        
        % Set reward reset to true
        D.B.track_rew_reset(D.I.rew_reset_hunt_ind) = true;
        
    end
    
    % Set later quads to false to prevent backwards runs
    D.B.track_rew_reset(1:3 > D.I.rew_reset_hunt_ind) = false;
    ...
    % Update current quadrant
    if any(D.B.track_rew_reset)
        D.I.rew_reset_hunt_ind = find(~D.B.track_rew_reset, 1, 'first');
    else
        D.I.rew_reset_hunt_ind = 1;
    end
...
function [D] = SF_Ozone_Trigger_Check(D)
...
if isfield(D, 'NLX')
    if ~isempty(D.NLX)
        ...
        % feed 1 solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(1)) = '0';
        % feed 2 solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(2)) = '0';
        % white noise bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = '0';
        % reward tone bit
...
D.SS_Out_ICR.(D.PAR.ratLab).Sound_Conditions(end,:) = D.UI.sndair(1:3);
D.SS_Out_ICR.(D.PAR.ratLab).Air_Conditions(end,:) = D.UI.sndair(4:5);
D.SS_Out_ICR.(D.PAR.ratLab).Start_Quadrant(end) = D.PAR.ratStrQuad;
D.SS_Out_ICR.(D.PAR.ratLab).Reversals(end) = D.C.rev_cnt;
D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Standard{end} = sum(D.C.lap_cnt{3});
D.SS_Out_ICR.(D.PAR.ratLab).Laps_Standard{end} = sum(D.C.rew_cnt{3});
...
if D.B.rot_ses
    ...
    D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Per_Rotation{end} = ...
        [D.C.rew_cnt{3}, reshape([[D.C.rew_cnt{1}]; [D.C.rew_cnt{2}]], 1, [])];
    D.SS_Out_ICR.(D.PAR.ratLab).Days_Till_Rotation(end) = D.PAR.daysTilRot;
    D.SS_Out_ICR.(D.PAR.ratLab).Rewards_40_Deg{end} = D.C.rew_cnt{2};
    D.SS_Out_ICR.(D.PAR.ratLab).Laps_40_Deg{end} = D.C.lap_cnt{2};
    D.SS_Out_ICR.(D.PAR.ratLab).Rewards_0_Deg{end} = D.C.rew_cnt{1};
    D.SS_Out_ICR.(D.PAR.ratLab).Laps_0_Deg{end} = D.C.lap_cnt{1};
else
    ...
    D.SS_Out_ICR.(D.PAR.ratLab).Days_Till_Rotation = NaN;
    D.SS_Out_ICR.(D.PAR.ratLab).Rewards_40_Deg{end} = 0;
    D.SS_Out_ICR.(D.PAR.ratLab).Rewards_0_Deg{end} = 0;
    D.SS_Out_ICR.(D.PAR.ratLab).Laps_40_Deg{end} = 0;
    D.SS_Out_ICR.(D.PAR.ratLab).Laps_0_Deg{end} = 0;
end

% Update SS_In_All
D.SS_In_All.ICRb_Track_Session(D.PAR.ratInd) = D.PAR.sesNum;
if D.B.rot_ses
    D.SS_In_All.Rotation_Session(D.PAR.ratInd) = D.PAR.sesNumICR;
end
...
D.SS_In_All.Air_Conditions(D.PAR.ratInd,:) = D.UI.sndair(4:5);

% Save out data
SS_Out_ICR = D.SS_Out_ICR;
save(fullfile(D.DIR.ssSD, 'SS_Out_ICR_temp.mat'), 'SS_Out_ICR');
SS_In_All = D.SS_In_All;
save(fullfile(D.DIR.ssSD, 'SS_In_All_temp.mat'), 'SS_In_All');
