...
D.DB.rat = '9857';
% ses file
D.DB.sesFi = '2015-11-10_12-55-06';
% ses file dir
D.DB.sesDir = 'J:\BehaviorPilot';
% sampling interleve
D.DB.subSmp = 1;
...
% Feeder paramiters
D.PAR.fdPlsDur = 300; % pulse duration for small and large rewards (ms)
D.PAR.fdFillDur = 25; % time it should take to backfill feeders (sec)
D.PAR.feedBnds = [10,5]; % boundary before and after rew feeder (deg)

% Sound paramiters
D.PAR.toneDur = 1000; % tone duration (ms)

% Air paramiters
D.PAR.pumpDel = 1000; % duration before turning pump back on after feeder bigins (ms)
D.PAR.ozDur = 250; % duration to leave ozone on (ms)
D.PAR.vacDel = 750; % delay before turning vac on after feeder fills (ms)
D.PAR.vacDur = 500; % duration to leave vac on (ms)
...
%   val 5 = Ozone [true, false]
%   val 6 = Vacuum [true, false]
%...........................D.AC.data......................................
...
% Load data tables
T = load(fullfile(D.DIR.ssSD, 'SS_In_All.mat'));
D.SS_In_All = T.SS_In_All;
T = load(fullfile(D.DIR.ssSD, 'SS_Out_ICR.mat'));
...
load(fullfile(D.DIR.ioTop, D.DIR.oppSD, D.DIR.trkBndsFi))
% track bound radius
D.PAR.R = R;
...
D.B.ratIn = false;
% check for rotation bounds crossing
D.B.rot_check = false;
...
% Run Arenacontroller network setup code
D = AC_Setup(D);
...
try D = guidata(D.UI.figH); catch; return; end
...
if D.B.poll_nlx
    ...
    % Right chan state (white noise)
    % Note: because left chan cannot be turned off without right chan, all
    % sound shoul be turned off if white noise = false
    if D.UI.sndair(1) == 0;
        % turn off all sound if white noise is set ot off
        D.AC.data(5) = 0;
    else
        % set white noise on
        D.AC.data(5) = 2;
    ...
    % Post to AC computer
    %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double');
...
while D.B.poll_nlx
    ...
    % Get UI data and current condition
    try D = guidata(D.UI.figH); catch; break; end
    ...
    if ~D.DB.run
        ...
        % Get NXL vt data
        [~, ~, vtPos, vtHD, D.vtNRecs, ~] = NlxGetNewVTData(D.NLX.vt_acq_ent);
    ...
    smp_str_ind = D.I.rv_smp_ind + 1;
    smp_end_ind = smp_str_ind + D.vtNRecs-1;
    ...
    elseif smp_end_ind > D.A.rv_nsmp
        
        % Get shift range
        shft = smp_end_ind - D.A.rv_nsmp;
        ...
        % Change indeces
        % start ind
        smp_str_ind = smp_str_ind - shft;
        % end ind
        smp_end_ind = smp_end_ind - shft;
    ...
    if D.vtNRecs <= D.A.rv_nsmp
        ...
        % Get radian values
        % track
        D.A.rv_samples(smp_str_ind:smp_end_ind,1) = D.P.rad;
        % hd
        D.A.rv_samples(smp_str_ind:smp_end_ind,2) = D.P.hd_rad;
    ...
    D.B.track_strqd = D.P.rad >= D.UI.lapBnds(1,1) & D.P.rad <= D.UI.lapBnds(1,2);
    if ...
            any(D.B.track_strqd) && ...
            ~D.B.ratIn &&  ...
            D.B.rec;
        ...
        % Set cue to on after first display
        if D.UI.cueFeed ~= 'None'
            ...
            % Update bools
            % set cue state on to true
            D.B.cue_on = true;
            % set cue next to false
            D.B.cue_next = false;
            
            % Set cue marker face color
            set(D.UI.cueH(D.I.rew), 'MarkerFaceColor',  D.UI.cuTrgCol);
            
        end
        
        % Reset D.B.rew_reset if rew accidentally triggered early
        D.B.rew_reset = true;
        
        % Remove reward reset patch
        if isfield(D.UI, 'ptchFdRstH')
            delete(D.UI.ptchFdRstH);
            D.UI = rmfield(D.UI, 'ptchFdRstH');
        end
        
        % Post NLX event: rat in
        NlxSendCommand(D.NLX.ratin_cmd{1});
    ...
    if D.B.ratIn
        ...
        if D.B.rot_check
            ...
            D.B.track_rot = ...
                D.P.rad >= D.UI.rotbnd(1) & ...
                D.P.rad <= D.UI.rotbnd(2);
            ...
            if any(D.B.track_rot)
                ...
                % Get new image
                D.I.img = find(D.I.img_ind ~=  D.I.img);
                
                % Update reward feeder
                D.I.rew = D.I.rew_ind(D.I.img);
                
                % Post NLX event: rotaion *deg
                NlxSendCommand(D.NLX.rot_cmd{D.I.img});
                ...
                % active feeder
                set(D.UI.fdH(D.I.rew), ...
                    'MarkerSize', 15, ...
                    'LineWidth', 2);
                % inactive feeder
                set(D.UI.fdH(D.I.img_ind ~=  D.I.rew), ...
                    'MarkerSize', 10, ...
                ...
                [x,y] = TrkBnds(D,[r(1),r(1)]);
                plot(x,y, ...
                    'Color', D.UI.rotCol(D.I.img,:), ...
                    'LineWidth', 3, ...
                    'Parent',D.UI.axH);
                
                % Change reward bounds patch
                % active feeder
                set(D.UI.ptchFdH(D.I.img), ...
                    'FaceAlpha', 0.5, ...
                    'EdgeAlpha', 1)
                % inactive feeder
                set(D.UI.ptchFdH(D.I.img_ind ~=  D.I.img), ...
                    'FaceAlpha', 0, ...
                    'EdgeAlpha',0)
                
                % Change button features
                set(D.UI.btnICR,'string', D.UI.btnICRstr{imgLast}, ...
                    'BackgroundColor', D.UI.rotCol(imgLast,:));
                
                % Change session info font weight
                % active feeder
                set(D.UI.txtPerfInf(D.I.img), 'FontWeight', 'Bold')
                % inactive feeder
                set(D.UI.txtPerfInf(D.I.img_ind ~=  D.I.img), 'FontWeight', 'Light')
                ...
                % Add new lap counter
                D.C.lap_cnt{D.I.img} = [D.C.lap_cnt{D.I.img}, 0];
                D.C.rew_cnt{D.I.img} = [D.C.rew_cnt{D.I.img}, 0];
        ...
        D.B.track_fd = ...
            D.P.rad >= D.UI.rewBnds(D.I.rew,1) & ...
            D.P.rad <= D.UI.rewBnds(D.I.rew,2);
        ...
        if ...
            ...
            if ...
                ...
                % Activate timers
                % start feeder reward timer
                start(D.UI.timFdRewDur)
                % start reward tone timer
                start(D.UI.timToneRewDur)
                % start pump timer
                start(D.UI.timPumpDel)
                % start vac timer
                start(D.UI.timVacDel)
                
                % Turn on feeder by changing/posting NLX ttl comand
                % feed solonoid bit
                D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(D.I.rew)) = '1';
                % white noise bit
                D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = '0';
                % reward tone bit
                D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(2)) = ...
                    num2str(D.UI.sndair(2));
                % pump bit
                D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = '0';
                % post command
                NlxSendCommand(D.NLX.port_out_update_cmd);
                ...
                % Post Feeder cue off to NLX and AC
                if ...
                        D.UI.cueFeed ~= 'None' && ...
                        D.B.cue_on
                ...
                % Get and plot cue trigger patch
                if D.UI.cueFeed ~= 'None'
                    
                    % Set cue marker face color to none for both feeders just
                    % in case one is left on after rotation
                    set(D.UI.cueH(1), 'MarkerFaceColor', 'none')
                    set(D.UI.cueH(2), 'MarkerFaceColor', 'none')
                    
                    % Check if this lap should be cued
                    if ...
                            D.UI.cueFeed == 'All' || ...
                        ...
                        % Get new bounds
                        D.UI.cueBnd = ...
                            D.UI.ozcuBnds(ceil(rand(1,1)*size(D.UI.ozcuBnds,1)), :, D.I.rew);
                        
                        % Delete old patch
                        if isfield(D.UI, 'ptchCuTrg')
                            delete(D.UI.ptchCuTrg)
                        end
                        
                        % Plot new patch
                        [xbnd, ybnd] =  TrkBnds(D,D.UI.cueBnd);
                        D.UI.ptchCuTrg = ...
                ...
                % Change ozone bounds patch
                if D.UI.sndair(5)
                    
                    % Change bool to allow ozone for next lap
                    D.B.was_ozed = false;
                    ...
                    % Delete old bounds patch
                    if isfield(D.UI, 'ptchOzTrg')
                        delete(D.UI.ptchOzTrg)
                    end
                    
                    % Plot new patch
                    [xbnd, ybnd] =  TrkBnds(D,D.UI.ozBnd);
                    D.UI.ptchOzTrg = ...
                        patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                        [ybnd(1,:),fliplr(ybnd(2,:))], ...
                        D.UI.ozTrgCol, ...
                ...
                % Plot reset patch
                [xbnd, ybnd] =  TrkBnds(D,D.UI.resetBnds(D.I.rew,:));
                D.UI.ptchFdRstH = ...
                    patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                    [ybnd(1,:),fliplr(ybnd(2,:))], ...
                    [0.5,0.5,0.5], ...
                ...
                if D.B.rotated
                    D.C.rew_cnt{D.I.img}(end) = D.C.rew_cnt{D.I.img}(end) + 1;
                else
        ...
        if ...
            ...
            if sum(~isnan(D.A.rv_samples(:,1))) >= D.A.rv_nsmp*0.75
                ...
                hed_ang_mu(hed_ang_mu<0) = ...
                    360 + hed_ang_mu(hed_ang_mu<0);
                ...
                if D.B.rat_rev
                    ...
                    [~, ttl_val] = NlxSendCommand( ...
                        ['-GetDigitalIOPortString ', D.NLX.DevName, ' ', D.NLX.port_out]);
                    % keep bit value
                    ttl_val = str2double(ttl_val{:}(8 - D.NLX.snd_lft_bit));
                    
                    % Check if sound should be turned back on
                    if ...
                            D.UI.sndair(3) && ...
                            D.B.rat_rev && ...
                            ttl_val ~= 1
                        
                        % Change AC laft channel data and post
                        D.AC.data(6) = 2;
                        %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double')
                        ...
                        % Turn on aversive sound by changing/posting NLX ttl comand
                        % change aversive tone bit
                        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(3)) = ...
                            num2str(D.UI.sndair(3));
                        % post command
                        NlxSendCommand(D.NLX.port_out_update_cmd);
                ...
                elseif ...
                    ...
                    % Turn off aversive sound by changing/posting NLX ttl comand
                    % change aversive tone bit
                    D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(3)) = '0';
                    % post command
                    NlxSendCommand(D.NLX.port_out_update_cmd);
        ...
        % Loop through each lab bound index
        for i_qd = 1:4
            
            % Check if rat in lap bound
            D.B.track_lap = ...
                D.P.rad >= D.UI.lapBnds(i_qd,1) & ...
                D.P.rad <= D.UI.lapBnds(i_qd,2);
            
            % If in quad bounds
            if sum(D.B.track_lap) > 0
                
                % Set specific bound bool to true
                D.B.track_lap(i_qd) = true;
                
                % For all but first quad
                if i_qd ~= 1
                    
                    % Set start quadrant back to false
                    D.B.track_lap(1) = false;
                    
                    % Set later quads to false to prevent backwards runs
                    D.B.track_lap(1:4 > i_qd) = false;
        ...
        % Update lap count
        % exclude start quadrant
        if all(D.B.track_lap);
            
            % Add lap to counter
            if D.B.rotated
                D.C.lap_cnt{D.I.img}(end) = D.C.lap_cnt{D.I.img}(end) + 1;
            else
        ...
        %% -------------CHECK REWARD RESET BOUND---------------------------
        
        % Check if reward reset bounds are crossed
        D.B.track_rset = ...
            D.P.rad >= D.UI.resetBnds(D.I.rew,1) & ...
            D.P.rad <= D.UI.resetBnds(D.I.rew,2);
        
        if any(D.B.track_rset);
            
            % Set reward reset to true
            D.B.rew_reset = true;
        ...
        %% --------------CHECK CUE TRIGGER BOUND---------------------------
        
        % Check if this is a cued session and cue is not already on
        if ...
                D.UI.cueFeed ~= 'None' && ...
                ~D.B.cue_on && ...
                ~D.B.was_cued
            
            % Check if this lap should be cued or has already been cued
            if ...
                    D.UI.cueFeed == 'All' || ...
                    (D.UI.cueFeed == 'Half' && D.B.cue_next)
                ...
                if any(track_cue);
                    ...
                    % Specify cue has occured
                    D.B.was_cued = true;
                    
                    % Check if this is not a cued lap
                elseif ...
                        D.UI.cueFeed == 'Half' && ...
                        ~D.B.cue_next
                    
                    % Set bool so wont be triggered again this lap
                    D.B.cue_next = true;
        ...
        %% -------------CHECK OZONE TRIGGER BOUND--------------------------
        
        % Check if ozone should be used and has not been
        if ...
                D.UI.sndair(5) && ...
                ~D.B.was_ozed
            
            % Check if ozone trigger bounds are crossed
            track_oz = ...
                D.P.rad >= min(D.UI.ozBnd) & ...
                D.P.rad <= max(D.UI.ozBnd);
            
            if any(track_oz);
                
                % Turn on ozone by changing/posting NLX ttl comand
                % ozone bit
                D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(5)) = '1';
                % post command
                NlxSendCommand(D.NLX.port_out_update_cmd);
                
                % Post NLX event: ozone on
                NlxSendCommand(D.NLX.oz_cmd{1});
    ...
    if ...
        ...
        if ~D.B.rat_rev;
            ...
            if any(D.B.track_fd)
                D.UI.posCol = D.UI.rotCol(D.I.img,:);
    ...
    infstr = sprintf([...
        ...
        'Rewards_Total:%s%d\n', ...
        'Reverse_Total:%s%d\n'], ...
        repmat('_',1,4), D.C.rot_cnt, ...
        repmat('_',1,4), sum([D.C.lap_cnt{:}]), ...
        repmat('_',1,4), sum([D.C.rew_cnt{:}]), ...
        repmat('_',1,4), D.C.rev_cnt);
    ...
    infstr = sprintf([...
        'Laps____Stand:%s%d\n', ...
        'Rewards_Stand:%s%d'], ...
        repmat('_',1,4), D.C.lap_cnt{3}(end), ...
        repmat('_',1,4), D.C.rew_cnt{3}(end));
    ...
    infstr = sprintf([...
        'Laps____40%c:%s%d|%d\n', ...
        'Rewards_40%c:%s%d|%d\n'], ...
        char(176), repmat('_',1,6), D.C.lap_cnt{2}(end), sum(D.C.lap_cnt{2}), ...
        char(176), repmat('_',1,6), D.C.rew_cnt{2}(end), sum(D.C.rew_cnt{2}));
    set(D.UI.txtPerfInf(2), 'String', infstr)
    % 0 deg
    infstr = sprintf([...
        'Laps____0%c:%s%d|%d\n', ...
        'Rewards_0%c:%s%d|%d\n'], ...
        char(176), repmat('_',1,7), D.C.lap_cnt{1}(end), sum(D.C.lap_cnt{1}), ...
        char(176), repmat('_',1,7), D.C.rew_cnt{1}(end), sum(D.C.rew_cnt{1}));
    ...
    % acquistion
    if D.B.acq
        nowTim(1) = etime(clock, D.T.acq_tim_reset) + D.T.acq_tim;
    else nowTim(1) = D.T.acq_tim; % keep showing save time
    end
    % recording
    if  D.B.rec
    ...
    % Get session time
    D.PAR.sesTim =  etime(clock,D.T.ses_tim);
    ...
    infstr = sprintf([ ...
        ...
        '(REC):%s%s\n'...
        '(LAP):%s%s\n'], ...
        repmat('_',1,6), datestr(D.PAR.sesTim/(24*60*60), 'HH:MM:SS'), ...
...
D.NLX.port_in = '2'; % input port for photo transducers

%% Define Event Bit Values

% Use unique bit string to identify each feeder and sound event
% Note:
%   Feeder solonoids are normally close
%   Ensure air solinoids are normally open
%   Reward/aversive sound channel (left) is open by bit 5
%   White noise channel (right) is open by bit 6
...
D.NLX.air_ozone_bit = 5;
D.NLX.air_vac_bit = 6;
% reset
D.NLX.port_out_update_bit = 7;

% Get binary strings
str = '00000000';
% get string bytes for port out
D.NLX.port_out_update_bin = str;
D.NLX.port_out_update_bin(8-D.NLX.port_out_update_bit) = '1';
% get binary command string for port out
D.NLX.port_out_update_cmd = ...
    ['-SetDigitalIOportString ', D.NLX.DevName, ' ', D.NLX.port_out, ' "', D.NLX.port_out_update_bin, '"'];

% Index cammand bype string
% feeders
D.NLX.port_feed_bit_Ind = ...
    [48 - D.NLX.feed_bit(1), ...
    48 - D.NLX.feed_bit(2)];
% sound/air
D.NLX.port_snd_air_bit_Ind = ...
    [48 - D.NLX.snd_rt_bit, ...
    48 - D.NLX.snd_lft_bit, ...
    NaN, ...
    48 - D.NLX.air_pump_bit, ...
    48 - D.NLX.air_ozone_bit, ...
    48 - D.NLX.air_vac_bit];
...
D.NLX.ratin_cmd{1} = '-PostEvent Post_Rat_In 201 0';
D.NLX.ratin_cmd{2} = '-PostEvent Post_Rat_Out 202 0';
...
% Reversal event command strings
D.NLX.oz_cmd{1} = '-PostEvent Post_Ozone_On 209 0';
D.NLX.oz_cmd{2} = '-PostEvent Post_Ozone_Off 210 0';

% Session end command string
D.NLX.sesend_cmd = '-PostEvent Post_Session_End 211 0';
...
if ~D.DB.run
    ...
    if notrun == 1
        ...
        if exist('C:\Program Files\Neuralynx', 'dir')
            system(fullfile('Cheetah.exe C:\Program Files\Neuralynx\Cheetah5\Configuration\', [top_cfg_fi,'&']));
        else
            system(fullfile('Cheetah.exe C:\Program Files (x86)\Neuralynx\Cheetah5\Configuration\', [top_cfg_fi,'&']));
        end
    ...
    NlxSetApplicationName('ICR_BEHAVIOR in MATLAB');
    
    %Get a list of all objects in Cheetah, along with their types.  Then, check to see
    %if the acquisition entities we want actually exist in Cheetah.
    [~, cheetahObjects, ~] = NlxGetCheetahObjectsAndTypes();
    if isempty(strmatch(D.NLX.vt_acq_ent, cheetahObjects))
        fprintf('FAILED the acquisition entity %s does not exist.\r', D.NLX.vt_acq_ent);
    ...
    NlxOpenStream(D.NLX.vt_acq_ent);
...
NlxSendCommand(['-SetDigitalIOportDirection ', D.NLX.DevName, ' ', D.NLX.port_in, ' Input']);

% Enable digital io events
NlxSendCommand(['-SetDigitalIOEventsEnabled ', D.NLX.DevName, ' ', D.NLX.port_in, ' True']);
...
% feeders
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.feed_bit(1)), ' TTL_Feeder_1_Open']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.feed_bit(2)), ' TTL_Feeder_2_Open']);
% audio channels
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.snd_lft_bit), ' TTL_Sound_Left_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.snd_rt_bit), ' TTL_Sound_Right_On']);
% air pump, ozone and vacuum
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.air_pump_bit), ' TTL_Air_Pump_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.air_ozone_bit), ' TTL_Air_Ozone_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.air_vac_bit), ' TTL_Air_Vacuum_On']);
% port reset
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_out, ' ', num2str(D.NLX.port_out_update_bit), ' TTL_Port_Out_Update']);
% photo transdicers
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_in, ' ', D.NLX.north_bit, ' TTL_North_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_in, ' ', D.NLX.east_bit, ' TTL_East_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_in, ' ', D.NLX.south_bit, ' TTL_South_On']);
NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_in, ' ', D.NLX.west_bit, ' TTL_West_On']);
...
% Set backround axis pos which will be slightly larger
ax_bk_pos = [ ...
    ax_pos(1) - 0.05, ...
    ax_pos(2) - 0.05, ...
    ax_pos(3) + 0.1, ...
    ax_pos(4) + 0.1];
set(D.UI.axBkH, 'Position', ax_pos);
...
% make rat list
D.UI.ratList = ...
    [D.SS_In_All.Properties.RowNames(D.SS_In_All.Include), ...
    cellstr(char(D.SS_In_All.Yoke_Mate(D.SS_In_All.Include)))];
...
botm(3) = panBtm + pos_ht_dflt*0.25 + obj_gap_ht;
lft(1:3) = pos_lft_dflt + offset;
lft(4:5) = pos_lft_dflt + pos_wd_dflt/1.75;
% panel
pos = [pos_lft_dflt, ...
    panBtm, ...
    pos_wd_dflt, ...
...
% SAVE SESSION DATA
pos = [0, ax_bk_pos(2), ax_bk_pos(1)/2, 0.05];
...
% QUIT ALL
pos = [ax_bk_pos(1)/2, ax_bk_pos(2), ax_bk_pos(1)/2, 0.05];
...
% CLEAR VT BUTTON
pos = [ax_bk_pos(3)+ax_bk_pos(1)-0.16, 0.01, 0.14, 0.03];
...
% Position settings
obj_gap_ht = 0.025;
pos_lft_dflt = (ax_bk_pos(1)+ax_bk_pos(3)) + 0.005;
pos_wd_dflt = 1 - (ax_bk_pos(1)+ax_bk_pos(3)) - 2*0.005;
pan_lft = (ax_bk_pos(1)+ax_bk_pos(3));
pan_wd = 1 - (ax_bk_pos(1)+ax_bk_pos(3));
...
% Fill stop timer
D.UI.timFdFillDur = timer('TimerFcn',@StopFill, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.fdFillDur);

% Feeder solonoid stop timer
D.UI.timFdRewDur = timer('TimerFcn',@StopFeed, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.fdPlsDur/1000);

% Reward tone stop timer
D.UI.timToneRewDur = timer('TimerFcn',@StopTone, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.toneDur/1000);

% Pump start timer
D.UI.timPumpDel = timer('TimerFcn',@StartPump, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.pumpDel/1000);

% Ozone stop timer
D.UI.timOzDur = timer('TimerFcn',@StopOz, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.ozDur/1000);

% Vac start timer
D.UI.timVacDel = timer('TimerFcn',@StartVac, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.vacDel/1000);

% Vac stop timer
D.UI.timVacDur = timer('TimerFcn',@StopVac, ...
    'UserData', 0, ...
    'StartDelay', D.PAR.vacDur/1000);
...
% RAT SELECTION
    function [] = PopRat(~, ~, ~)
        ...
        if get(D.UI.popRat,'Value') ~= 1
            ...
            % session number
            if size(D.SS_Out_ICR.(D.PAR.ratLab),1) == 1
                D.PAR.sesNum = 1; % first session entry
            else
                D.PAR.sesNum = ...
                    D.SS_In_All.ICRb_Track_Session(D.PAR.ratInd) + 1;
            end
            % icr session
            D.PAR.sesNumICR = ...  % [1:200]
                D.SS_In_All.Rotation_Session(D.PAR.ratInd) + 1;
            % start quadrant
            ...
            if D.PAR.sesNum > 1
                ...
                % Set reward delay
                D.PAR.rewDel = ...
                    D.SS_Out_ICR.(D.PAR.ratLab).Reward_Delay(D.PAR.sesNum-1);
                ...
                % Set cue condition
                D.UI.cueFeed = ...
                    D.SS_Out_ICR.(D.PAR.ratLab).Cue_Condition(D.PAR.sesNum-1);
                set(D.UI.radCue( ...
                    ismember(D.PAR.catCueCond, D.UI.cueFeed)), 'Value', 1);
                
                % Set sound/air condition
                D.UI.sndair(1:5) = logical(...
                    [D.SS_Out_ICR.(D.PAR.ratLab).Sound_Conditions(D.PAR.sesNum-1,:), ...
                    D.SS_Out_ICR.(D.PAR.ratLab).Air_Conditions(D.PAR.sesNum-1,:)]);
                set(D.UI.radSndAir(D.UI.sndair(1:5)), 'Value', 1);
...
% SOUND/AIR CONDITION
    function [] = RadSndAir(~, ~, ~)
        ...
        % Turn off all air if pump is off
        if D.UI.sndair(4) == false
            D.UI.sndair(5:6) = false;
            set(D.UI.radSndAir(5), 'Value', 0);
        end
        
        % Change NLX command strings based on sound settings
        if changeNLX
            
            % Change port out update command
            % set white noise bit
            D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = ...
                num2str(D.UI.sndair(1));
            % set pump bit
            D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = ...
                num2str(D.UI.sndair(4));
            
            % Post output port update command
            NlxSendCommand(D.NLX.port_out_update_cmd);
        end
        if changeAC
...
% SETUP DONE
    function [] = BtnStupDn(~, ~, ~)
        ...
        infstr = sprintf( ...
            'Start:%s%s\n', ...
            repmat('_',1,6), datestr(D.T.ses_tim, 'HH:MM:SS'));
        ...
        D.UI.rewFeed = rewFeeds(D.PAR.ratFeedCnd_Num,:);
        D.UI.oppFeed = rewFeeds(D.I.img_ind ~=  D.PAR.ratFeedCnd_Num,:);
        
        % Reverse start feeder if rotation is CW
        if D.PAR.ratRotDrc == 'CW';
            D.I.rew_ind = fliplr(D.I.rew_ind);
        end
        % update reward
        D.I.rew = D.I.rew_ind(1);
        
        % Plot reward feeder locations
        % All feed locs
        allfdlocs = circshift((0:10:350)+5,[0,0]);
        [fd_X,fd_Y] = pol2cart(deg2rad(allfdlocs), ones(1,length(allfdlocs)) * D.UI.arnRad);
        D.UI.xfd = fd_X*D.UI.xCMcnv + D.UI.lowLeft(1) + D.UI.arnRad*D.UI.xCMcnv;
        D.UI.yfd = fd_Y*D.UI.yCMcnv + D.UI.lowLeft(2) + D.UI.arnRad*D.UI.yCMcnv;
        
        % Rewarded feeder bounds
        % calculate rewarded feeder inclusion positions in radians (based on D.PAR.feedBnds)
        rewfdlocs = [circshift((0:10:350)+5,[0,0]), circshift((0:10:350)+5,[0,0])];
        D.UI.rewBnds = [deg2rad(rewfdlocs(D.UI.rewFeed(1)+36)) - deg2rad(D.PAR.feedBnds(2)), ...
            deg2rad(rewfdlocs(D.UI.rewFeed(1)+36)) + deg2rad(D.PAR.feedBnds(1)); ...
            deg2rad(rewfdlocs(D.UI.rewFeed(2)+36)) - deg2rad(D.PAR.feedBnds(2)), ...
            deg2rad(rewfdlocs(D.UI.rewFeed(2)+36)) + deg2rad(D.PAR.feedBnds(1))];
        % set negative radian values to posetive
        D.UI.rewBnds(abs(D.UI.rewBnds) ~= D.UI.rewBnds) = ...
            D.UI.rewBnds(abs(D.UI.rewBnds) ~= D.UI.rewBnds) + 2*pi;
        
        % 180 deg bounds
        % calculate 180 deg bounds animal must pass to get next reward
        D.UI.resetBnds = D.UI.rewBnds + pi;
        D.UI.resetBnds(D.UI.resetBnds > 2*pi) = D.UI.resetBnds(D.UI.resetBnds > 2*pi) - 2*pi;
        
        % Lap crossing bounds
        % calculate lap count crossing points for every 45 deg
        lapBndLocs = 360-45:-90:45;
        % shift so first ind in NE
        lapBndLocs = circshift(lapBndLocs,[0, 1]);
        % shift so that start quadrant is first entry
        strQuadInd = find(D.PAR.catStrQuad == D.PAR.ratStrQuad);
        lapBndLocs = circshift(lapBndLocs,[0,-1*(strQuadInd-1)]);
        D.UI.lapBnds = NaN(4,2);
        for z_qd = 1:4;
            D.UI.lapBnds(z_qd,1) = deg2rad(lapBndLocs(z_qd)) - deg2rad(20);
            D.UI.lapBnds(z_qd,2) = deg2rad(lapBndLocs(z_qd)) + deg2rad(20);
        end
        % set negative radian values to posetive
        D.UI.lapBnds(abs(D.UI.lapBnds) ~= D.UI.lapBnds) = ...
            D.UI.lapBnds(abs(D.UI.lapBnds) ~= D.UI.lapBnds) + 2*pi;
        
        % Cue and ozone bounds
        % 20 deg bounds begin/end 60 deg from rewarded feeder
        ozcuBndLocs = (360-60:-20:60)';
        ozcuBndLocs = [ozcuBndLocs(1:end-1), ozcuBndLocs(2:end)];
        D.UI.ozcuBnds = NaN(size(ozcuBndLocs,1),2,2);
        D.UI.ozcuBnds(:,:,1) = ozcuBndLocs + D.UI.rewFeed(1)*10;
        D.UI.ozcuBnds(:,:,2) = ozcuBndLocs + D.UI.rewFeed(2)*10;
        D.UI.ozcuBnds(D.UI.ozcuBnds>360) = D.UI.ozcuBnds(D.UI.ozcuBnds>360) - 360;
        D.UI.ozcuBnds = deg2rad(D.UI.ozcuBnds);
        % initial ozone and cue bounds
        D.UI.ozBnd = D.UI.ozcuBnds(ceil(rand(1,1)*size(D.UI.ozcuBnds,1)),:,1);
        D.UI.cueBnd = D.UI.ozcuBnds(ceil(rand(1,1)*size(D.UI.ozcuBnds,1)),:,1);
        
        % PLOT feeder data
        
        % Plot start quadrant
        [xbnd, ybnd] =  TrkBnds(D,D.UI.lapBnds(1,:)); % reward feeder bounds
        D.UI.ptchStQ = ...
            patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
            [ybnd(1,:),fliplr(ybnd(2,:))], ...
            [0.5,0.5,0.5], ...
            ...
            'Parent',D.UI.axH);
        D.UI.txtStQ = ...
            text(mean(mean(xbnd)), mean(mean(ybnd)), ...
            'Start', ...
            'FontSize', 12, ...
            'FontWeight', 'Bold', ...
            'Color', [0.3,0.3,0.3], ...
        ...
        [xbnd, ybnd] =  TrkBnds(D,D.UI.rewBnds(D.I.rew_ind(1),:)); % reward feeder bounds
        D.UI.ptchFdH(1) = ...
            patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
            [ybnd(1,:),fliplr(ybnd(2,:))], ...
            D.UI.rotCol(1,:), ...
        ...
        % Plot 40-deg feeder bounds and set alpha to 0 (transparent)
        [xbnd, ybnd] =  TrkBnds(D,D.UI.rewBnds(D.I.rew_ind(2),:));
        D.UI.ptchFdH(2) = ...
            patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
            [ybnd(1,:),fliplr(ybnd(2,:))], ...
            D.UI.rotCol(2,:), ...
        ...
        % Plot all feeders
        plot(D.UI.xfd, D.UI.yfd, 'o', ...
            'MarkerFaceColor', [0.5 0.5 0.5], ...
            'MarkerEdgeColor', 'k', ...
            'MarkerSize', 10, ...
            'Parent', D.UI.axH);
        
        % cue marker (indicates when cue on)
        % 0-deg
        D.UI.cueH(D.I.rew_ind(1)) = ...
            plot(D.UI.xfd(D.UI.rewFeed(D.I.rew_ind(1))), ...
            D.UI.yfd(D.UI.rewFeed(D.I.rew_ind(1))), 's', ...
            'MarkerFaceColor', 'none', ...
            'MarkerEdgeColor', 'none', ...
            'MarkerSize', 25, ...
            'Parent',D.UI.axH);
        % 40-deg
        D.UI.cueH(D.I.rew_ind(2)) = ...
            plot(D.UI.xfd(D.UI.rewFeed(D.I.rew_ind(2))), ...
            D.UI.yfd(D.UI.rewFeed(D.I.rew_ind(2))), 's', ...
            'MarkerFaceColor', 'none', ...
            'MarkerEdgeColor', 'none', ...
            'MarkerSize', 25, ...
            'Parent',D.UI.axH);
        ...
        % feeder 0-Deg marker
        D.UI.fdH(D.I.rew_ind(1)) = ...
            plot(D.UI.xfd(D.UI.rewFeed(D.I.rew_ind(1))), ...
            D.UI.yfd(D.UI.rewFeed(D.I.rew_ind(1))), 'o', ...
        ...
        D.UI.fdH(D.I.rew_ind(2)) = ...
            plot(D.UI.xfd(D.UI.rewFeed(D.I.rew_ind(2))), ...
            D.UI.yfd(D.UI.rewFeed(D.I.rew_ind(2))), 'o', ...
        ...
        % opposite unrewarded feeders plotted with thicker line
        plot(D.UI.xfd(D.UI.oppFeed), D.UI.yfd(D.UI.oppFeed), 'o', ...
            'MarkerFaceColor', [0.5 0.5 0.5], ...
            'MarkerEdgeColor', 'k', ...
            'LineWidth', 2, ...
            'MarkerSize', 10, ...
        ...
        if D.PAR.ratFeedCnd == 'C1'
            x1 = D.UI.xfd(D.UI.rewFeed(1)) - 18;
            x2 = D.UI.xfd(D.UI.rewFeed(2)) - 18;
        else
            x1 = D.UI.xfd(D.UI.rewFeed(1)) + 18;
            x2 = D.UI.xfd(D.UI.rewFeed(2)) + 18;
        end
        text(x1, D.UI.yfd(D.UI.rewFeed(1)), ...
            ...
            'HorizontalAlignment', 'Center', ...
            'Parent',D.UI.axH);
        text(x2, D.UI.yfd(D.UI.rewFeed(2)), ...
        ...
        % Calculate rotation range positions if ICR Training
        if D.B.rot_ses
            % NOTE: once rotation is triggered, a boundary -90, -180 or
            % -270 deg from post rotation rewarded feeder will be selected
            D.UI.rotBnds = repmat(D.UI.rewBnds, [1, 1, 3]) - ...
                deg2rad(permute(repmat(-90 - (0:90:180), [2, 1, 2]), [1, 3, 2]));
            % rescale values > 2*pi
            D.UI.rotBnds(D.UI.rotBnds > 2*pi) = D.UI.rotBnds(D.UI.rotBnds > 2*pi) - 2* pi;
        end
...
% ROTATION BUTTON
    function [] = BtnICR(~, ~, ~)
        ...
        imgNext = D.I.img_ind ~=  D.I.img;
        
        % Determine rotation trigger bounds for this event
        D.UI.rotbnd = ...
            D.UI.rotBnds(D.I.rew_ind(imgNext), :, D.PAR.rotPos(D.C.rot_cnt+1));
        
        % Plot selected rotation trigger bounds
        [xbnd, ybnd] =  TrkBnds(D,D.UI.rotbnd);
        D.UI.ptchRtTrgH = ...
            patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
            [ybnd(1,:),fliplr(ybnd(2,:))], ...
            D.UI.rotCol(imgNext,:), ...
...
% RECORDING DONE
    function [] = BtnRecDn(~, ~, ~)
        ...
        % Reset feeder bit string in case feeder was mid run
        NlxSendCommand(D.NLX.port_out_update_cmd);
        ...
        switch choice
            case 'Yes'
                ...
                NlxSendCommand(D.NLX.sesend_cmd);
        ...
        switch choice
            case 'Yes'
                ...
                NlxSendCommand(D.NLX.ratin_cmd{2});
        ...
        % Print end time
        tim = ...
            datestr(datevec(datenum(D.T.ses_tim)+(D.PAR.sesTim/60/60/24)), 'HH:MM:SS');
        infstr = sprintf( ...
            'Stop_:%s%s\n', ...
            repmat('_',1,6), tim);
        set(D.UI.txtTimEndInf, 'String', infstr)
...
% SAVE SESSION DATA
    function [] = BtnSav(~, ~, ~)
        ...
        % Note: this will move data from temp directory to rat directory
        % get current folder name
        dirs = dir(D.DIR.cheetTempTop);
        D.DIR.recFi = dirs([dirs.datenum] == max([dirs.datenum])).name;
        % make directory
        % datestr(clock, 'yyyy-mm-dd_HH-MM-SS', 'local');
        D.DIR.rec = fullfile(D.DIR.cheetSaveTop, num2str(D.PAR.ratNum));
        ...
        % Save sesion data to XLS spreadsheet
        fipath = fullfile(D.DIR.ioTop, D.DIR.ssSD, [num2str(D.PAR.ratNum), '.csv']);
        SaveSsDat(D)
        D.UI.saved = true;
        
        % Save Cheetah file to rat directory
        copyfile(fullfile(D.DIR.cheetTempTop, D.DIR.recFi),fullfile(D.DIR.rec, D.DIR.recFi))
        
        % Disable buttons
        % save button
        set(gcbo, 'Enable', 'Off', 'BackgroundColor', D.UI.disabledCol)
        % clean buttons
        set(D.UI.btnCleanFd, 'Enable', 'Off', 'BackgroundColor', D.UI.disabledCol)
...
% FEEDER FILL
    function [] = BtnFillFd(~, ~, ~)
        ...
        ud_sltd = get(gcbo, 'UserData'); % get selected button user data
        ud_othr = ... % other button user data
            get(D.UI.btnFillFd(D.I.img_ind ~=  ud_sltd), 'UserData');
        
        % Set timer UserData to specify running feeder
        set(D.UI.timFdFillDur, 'UserData', ud_sltd)
        
        % Check that timer is not running
        if strcmp(get(D.UI.timFdFillDur, 'Running'), 'Off')
            % Turn off sound on AC computer
            if D.UI.sndair(1)
                D.AC.data(5) = 0;
                %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double');
            ...
            % Turn on feeder by changing/posting NLX ttl comand
            % change command
            D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(ud_sltd)) = '1';
            % post command
            NlxSendCommand(D.NLX.port_out_update_cmd);
            
            % Set selected button properties on
            set(D.UI.btnFillFd(ud_sltd), 'String', D.UI.btnFlstr{1}, ...
...
% FEEDER CLEAN
    function [] = BtnCleanFd(~, ~, ~)
        ...
        ud_othr = ...
            get(D.UI.btnCleanFd(D.I.img_ind ~=  ud_sltd(1)), 'UserData');
        % update user data for selected button
        ud_sltd = [ud_sltd(1), find(D.I.img_ind ~=  ud_sltd(2))];
        
        % Turn on/off feeder by changing/posting NLX ttl comand
        if ud_sltd(2) == 1
            % set feeder bit high
            D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(ud_sltd(1))) = '1';
        else
            % set feeder bit low
            D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(ud_sltd(1))) = '0';
        end
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
        ...
        set(D.UI.btnCleanFd(ud_sltd(1)), ...
            'String', D.UI.btnFdstr{ud_sltd(2)}, ...
            'BackgroundColor', D.UI.btnFdcol(ud_sltd(2),:), ...
            'UserData', ud_sltd);
        
        % Set other button paramiters to off if running
        if ud_othr(2) == 1;
            ud_othr = [ud_othr(1), find(D.I.img_ind ~=  ud_othr(2))];
...
% FEEDER FILL TIMER STOP
    function [] = StopFill(~, ~, ~)
        ...
        % get selected button UserData
        ud_sltd = get(D.UI.timFdFillDur, 'UserData');
        % other button user data
        ud_othr = ...
            get(D.UI.btnFillFd(D.I.img_ind ~=  ud_sltd), 'UserData');
        % set button paramiters
        set(D.UI.btnFillFd(ud_sltd), ...
            'String', D.UI.btnFlstr{2}, ...
            'BackgroundColor', D.UI.btnFlcol(2,:));
        
        % Turn off feeder by changing/posting NLX ttl comand
        % change command
        D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(ud_sltd)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
        
        % Turn on sound on AC computer
        if D.UI.sndair(1)
            D.AC.data(5) = D.UI.sndair(1)+1;
            %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double');
        end
...
% FEEDER REWARD TIMER STOP
    function [] = StopFeed(~, ~, ~)
        ...
        % Turn off feeder by changing/posting NLX ttl comand
        % feed solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(D.I.rew)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
        
        % Return feeder marker color to normal
        set(D.UI.fdH(D.I.rew), ...
            'MarkerFaceColor', D.UI.rotCol(D.I.img,:), ...
            'MarkerSize', 15);
...
% REWARD TONE TIMER STOP
    function [] = StopTone(~, ~, ~)
        % Get handle data
        D = guidata(D.UI.figH);
        
        % Turn off reward tone by changing/posting NLX ttl comand
        % reward tone bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(2)) = ...
            num2str(D.UI.sndair(2));
        % post command
...
% PUMP TIMER START
    function [] = StartPump(~, ~, ~)
        ...
        % Turn on pump by changing/posting NLX ttl comand
        % pump bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = ...
            num2str(D.UI.sndair(4));
        % post command
...
% OZONE TIMER STOP
    function [] = StopOz(~, ~, ~)
        ...
        % Turn off ozone by changing/posting NLX ttl comand
        % ozone bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(5)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
% VAC TIMER START
    function [] = StartVac(~, ~, ~)
        ...
        % Turn on vaccum by changing/posting NLX ttl comand
        % vaccum bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(6)) = ...
            num2str(D.UI.sndair(6));
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
% VAC TIMER STOP
    function [] = StopVac(~, ~, ~)
        ...
        % Turn off vaccum by changing/posting NLX ttl comand
        % vaccum bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(6)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
...
%!!!!!!!!!!!!!!!!!!!!!!!!!!! FUNCTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
function [xbnd, ybnd] = TrkBnds(D,polbnds)
...
if isfield(D, 'AC')
    if ~isempty(D.AC)
        ...
        D.AC.data(1) = 1; % keep connection
        D.AC.data(2) = 0; % close image
        %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double');
        pause(1); % pause to allow image to close
        D.AC.data = zeros(1, length(D.AC.data)); % close all
        %!!DB!!fwrite(D.AC.tcpIP,D.AC.data,'double');
        fclose(D.AC.tcpIP);
...
if isfield(D, 'NLX')
    if ~isempty(D.NLX)
        % Close VT stream
        if isfield(D.NLX, 'vt_acq_ent')
            NlxCloseStream(D.NLX.vt_acq_ent);
        end
        
        % Make sure all TTL bits but update bit are off
        % feed 1 solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(1)) = '0';
        % feed 2 solonoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_feed_bit_Ind(2)) = '0';
        % white noise bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = '0';
        % reward tone bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(2)) = '0';
        % pump bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = '0';
        % ozone bit
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = '0';
...
%!!!!!!!!!!!!!!!!!!!!!!!!!!! FUNCTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
function [] =  SaveSsDat(D)

% Add new row at end
D.SS_Out_ICR.(D.PAR.ratLab) = [D.SS_Out_ICR.(D.PAR.ratLab); D.SS_Out_ICR.(D.PAR.ratLab)(end,:)];

% Add entries
D.SS_Out_ICR.(D.PAR.ratLab).Include(end) = true;
D.SS_Out_ICR.(D.PAR.ratLab).Date{end} = D.DIR.recFi;
D.SS_Out_ICR.(D.PAR.ratLab).Recording_Directory{end} = D.DIR.rec;
D.SS_Out_ICR.(D.PAR.ratLab).Start_Time{end} = datestr(D.T.ses_tim, 'HH:MM:SS');
D.SS_Out_ICR.(D.PAR.ratLab).Total_Time(end) = D.PAR.sesTim/60;
D.SS_Out_ICR.(D.PAR.ratLab).ICRb_Track(end) = true;
D.SS_Out_ICR.(D.PAR.ratLab).ICRb_Forage(end) = false;
D.SS_Out_ICR.(D.PAR.ratLab).ICRi_Track(end) = false;
D.SS_Out_ICR.(D.PAR.ratLab).ICRi_Forage(end) = false;
D.SS_Out_ICR.(D.PAR.ratLab).ICR_Rotation(end) = D.B.rot_ses;
D.SS_Out_ICR.(D.PAR.ratLab).ICRb_Track_Session(end) = D.PAR.sesNum;
D.SS_Out_ICR.(D.PAR.ratLab).ICRb_Forage_Session(end) = 0;
D.SS_Out_ICR.(D.PAR.ratLab).ICRi_Track_Session(end) = 0;
D.SS_Out_ICR.(D.PAR.ratLab).ICRi_Forage_Session(end) = 0;
D.SS_Out_ICR.(D.PAR.ratLab).Feeder_Condition(end) = D.PAR.ratFeedCnd;
D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Direction(end) = D.PAR.ratRotDrc;
D.SS_Out_ICR.(D.PAR.ratLab).Reward_Delay(end) = sprintf('%1.1f', D.PAR.rewDel);
D.SS_Out_ICR.(D.PAR.ratLab).Cue_Condition(end) = D.UI.cueFeed;
D.SS_Out_ICR.(D.PAR.ratLab).Pulse_Duration(end) = D.PAR.fdPlsDur;
% D.SS_Out_ICR.(D.PAR.ratLab).Sound_Conditions(end)
% D.SS_Out_ICR.(D.PAR.ratLab).Air_Conditions(end)
D.SS_Out_ICR.(D.PAR.ratLab).Start_Quadrant(end) = D.PAR.ratStrQuad;
D.SS_Out_ICR.(D.PAR.ratLab).Reversals(end) = D.C.rev_cnt;
% D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Standard(end)
% D.SS_Out_ICR.(D.PAR.ratLab).Laps_Standard(end)

% rotation session specifir vars
if D.B.rot_ses
    D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Session(end) = D.PAR.sesNumICR;
    D.SS_Out_ICR.(D.PAR.ratLab).Rotations_Per_Session(end) = D.C.rot_cnt;
    D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Positions{end}(1:D.C.rot_cnt) = ...
        D.PAR.rotPos(1:D.C.rot_cnt);
    %D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Per_Rotation
    %D.SS_Out_ICR.(D.PAR.ratLab).Days_Till_Rotation
    % D.SS_Out_ICR.(D.PAR.ratLab).Rewards_40_Deg(end)
    % D.SS_Out_ICR.(D.PAR.ratLab).Laps_40_Deg(end)
    % D.SS_Out_ICR.(D.PAR.ratLab).Rewards_0_Deg(end)
    % D.SS_Out_ICR.(D.PAR.ratLab).Laps_0_Deg(end)
else
    D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Session(end) = 0;
    D.SS_Out_ICR.(D.PAR.ratLab).Rotations_Per_Session(end) = NaN;
    D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Positions{end} = ...
...
save(fullfile(D.DIR.ssSD, 'SS_Out_ICR.mat'), 'SS_Out_ICR');
end
...
if D.DB.run
    ...
    if arg == 3
        ...
        % rotation session
        D.SS_In_All.Rotation_Session(D.PAR.ratInd) = ...
            D.SS_Out_ICR.(['r',D.DB.rat]).Rotation_Session(oldSesInd);
        % start quadrant
        D.SS_In_All.Start_Quadrant{D.PAR.ratInd}(D.PAR.sesNum) = ...
