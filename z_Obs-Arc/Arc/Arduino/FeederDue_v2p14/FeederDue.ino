...
int printQueueInd = printQueue_lng - 1;
bool doPrint = false;
// debug tracking
...
bool fc_doStartTracking = false;
bool fc_isTrackingReady = false;
...
const float defualtSetPoint = 35; // (cm)
...
// Reward
const uint32_t solDir = 1500; // (ms) sol open duration
...
public:
	String mode = "Manual"; // ["Manual" "Automatic"]
	...
	void UpdatePID()
	{
		...
		if (ekfReady)
		{
			...
			if (mode == "Automatic")
			{
				...
				if (ekf_ratVel < 1 && errorDefault < -5 && !isHolding4cross)
				{
					// halt running
					HardStop("[UpdatePID]");
				}
	...
	void SetDamp()
	{
		if (doDamp)
		{
			...
			if (error > 0)
			{
				// Change setpoint to rat pos
				deltaSetpoint = error;
				ChangeSetpoint(deltaSetpoint);

				// Initialize time to dampen till
				t_dampDur = 0;

				// Must cross behind setpoint to reset
				isDampened = true;
			}
			else
			{
	...
	void CheckDamp()
	{
		if (isDampened)
		{
			...
			// Local vars
			static int cnt = 0;
			static float damp_for = 0;
			static uint32_t t_start;
			float delta_set;

			// Compute time to dampen for
			if (damp_for == 0)
			{
				// 100 ms for every cm
				damp_for = deltaSetpoint * 100;
				cnt = 0;

				// Dampening start time
				t_start = millis();
			}

			// Update ellapsed time and setpoint
			t_dampDur = (float)(millis() - t_start);
			delta_set = deltaSetpoint - deltaSetpoint*(t_dampDur / damp_for);

			// Check that max time has not ellapsed
			if (t_dampDur <= damp_for &&
				delta_set > 0 &&
				errorDefault > 0)
			{
				cnt++;
				ChangeSetpoint(delta_set);
			}
			else
			{
				ChangeSetpoint(0);
...
public:
	String state = "Inactive"; // ["Active" "Inactive"]
	String mode = "Off"; // ["off", "On", "Hold"]
	...
	void RunBulldoze(String called_from)
	{
		// Take control
		SetMotorControl("Bull");
	...
	void StopBulldoze(String called_from)
	{
		// Stop movement
		RunMotor('f', 0, "Bull");
	...
	void Hold(String called_from)
	{
		...
		if (state == "Active")
		{
			...
			// Set state back to active for later
			state = "Active";
		}
...
#pragma region //--- (M) DO MOVE ---
	...
	if (fc_doMove)
	{
		...
		if (fc_isMoveTargSet)
		{
			bool is_targ_reached = DoDecelerate("MoveTo");
			if (is_targ_reached)
			{
				fc_doMove = false;
...
#pragma region //--- (R) DO REWARD ---
	if (msg_id == 'R' && msg_pass)
	{
		...
		{
			fc_doRew = true;
			DebugState("NOW REWARD");
		}
		else
		{
			fc_doCue = true;
			DebugState("CUED REWARD");
		}
	}
	...
	if (fc_doCue)
	{
		...
		if (!fc_isCueTargSet)
		{
			...
			if (is_targ_set)
			{
				...
				// Move setpoint back
				pid.ChangeSetpoint(0);
			}
		}
		// Check if robot is ready to be stopped
		if (fc_isCueTargSet)
		{
			bool is_targ_reached = DoDecelerate("Cue");
			if (is_targ_reached)
			{
				// Trigger reward
				StartRew();
	...
	if (fc_isRewarding)
	{
		...
		if (!fc_isRewarding)
		{
			// Tell CS reward is done
			//Store4_CS('D', c2r_packLast[CharInd('R', "c2r")]);
...
#pragma region //--- (B) BULLDOZE RAT STATUS ---
	if (msg_id == 'B' && msg_pass)
	{
		...
		bull.Reinitialize(bullDel, bullSpeed, "[Msg B]");
...
#pragma region //--- (I) START/STOP PID ---
	if (msg_id == 'I' && msg_pass)
	{
		...
		else
		{
			...
			fc_isTrackingReady = false;

			DebugState("STOP TRACKING");
		}
	}
#pragma endregion
...
bool ParseSerial()
{
	...
	{
		// Get delay in sec
		bullDel = WaitBuffRead(0);
		// Get speed
		bullSpeed = WaitBuffRead(0);
	}

	// Get start/end rat tracking data
	if (msg_id == 'I')
	{
		// Get session comand
		byte track_rat = WaitBuffRead(0);
		fc_doStartTracking = track_rat == 1 ? true : false;
	}
...
{
	// Local vars
	static byte msg[r2_lngC - 1];
	char rcv_id;
	bool do_send = false;
	int buff_tx;

	// Move next in queue to temp msg array
	for (int j = 0; j < r2_lngC - 1; j++)
	{
		msg[j] = r2_queue[r2_lngR - 1][j];
	}

	// save reviever id
	rcv_id = r2_queue[r2_lngR - 1][5];
	...
	if (do_send && Serial.availableForWrite() >= 10)
	{
			...
			(doDebugConsole || doDebugLCD))
		{
			...
			// Store
			sprintf(str, "sent: i:%c p:%d", id, pack);
			StoreDebugStr(str);
		}
...
{
	if (!(bull.state == "Active" && bull.mode == "On") &&
		!fc_isManualSes)
	{
...
// RUN AUTODRIVER
bool RunMotor(char dir, float speed, String called_from)
{
	// Local vars
	...
	if (called_from == fc_motorControl)
	{
		if (dir == 'f')
		{
		...
		// Update stored pid speed
		if (pid.runSpeed != speed)
		{
			pid.runSpeed == speed;
		}
		return true;
...
bool SetMotorControl(String set_to)
{
	...
	if (!fc_isHalted)
	{
		...
		{
			// Can set to "MoveTo" anytime
			if (set_to == "MoveTo")
			{
				fc_motorControl = set_to;
			}
			// Can set to open if tracking is setup
			else if (set_to == "Open" && fc_isTrackingReady)
			{
				fc_motorControl = set_to;
			}
		}
...
void CheckBlockTimElapsed()
{
	...
	// Check for rat movment
	if (fc_isTrackingReady)
		err = errorFeeder;
	else err = 0;

	if (fc_isBlockingTill && (millis() > t_blockTill ||
		err > 0)) // rat moved 10 cm past setpoint
	{
		// Run
...
bool CompTarg(float move_targ, String move_type)
{
	...
	// Handle weird error (16/09/30)
	if (move_type == "Cue" && !fc_isTrackingReady)
	{
		fc_doCue = false;
		fc_isCueTargSet = false;
		DebugState("CANCEL CUE");
		return pass = false;
	}
	...
	else
	{
		...
		feed_pos = move_targ - feedDist;
		if (feed_pos < 0)
		{
			feed_pos = feed_pos + (140 * PI);
		}
		...
		if (move_type == "MoveTo")
		{
			...
			// Start movement
			pass = RunMotor(moveDir, moveSpeed, "MoveTo");
		}
	}
	return pass;
}
...
// DECELERATE TO POS
bool DoDecelerate(String move_type)
{

	static float base_speed = 0;
	static uint32_t t_updateNext = millis();
	static uint32_t t_updateDT = 10;
	static float min_speed = 10;
	static float dec_pos;
	float new_speed;

	// Pos to begin decelerating
	dec_pos = move_type == "MoveTo" ? 40 : 30;
	...
	if (distLeft <= dec_pos)
	{
		...
		if (base_speed == 0)
		{
			...
			// increase so speed up before slow down
			//base_speed += 10;
			...
			//else if ((distLast - distLeft) >= 5)
		{
			...
			if (move_type == "Cue")
			{
				RunMotor('f', new_speed, "MoveTo");
			}
			else
			{
				RunMotor(moveDir, new_speed, "MoveTo");
			}
	...
	if ((distLeft < 2.5 && distLeft > -2.5) ||
		(move_type == "Cue" && distLeft < 0))
	{
		// Hard stop
		HardStop("[DoDecelerate]");
...
void UpdatePixyPos() {
	...
	if (pos_robVT.newData)
	{
		...
		pxAbs = pxAbs + defualtSetPoint;
		pxAbs = pos_robVT.posAbs + pxAbs;
		if (pxAbs > (140 * PI))
		{
			pxAbs = pxAbs - (140 * PI);
		}
...
void UpdateEKF()
{
	...
	if ((pos_ratVT.newData && pos_ratPixy.newData && pos_robVT.newData) ||
		(pos_robVT.newData && !fc_doStartTracking))
	{
...
void StoreDebugStr(String str)
{
	...
	sprintf(t_str, "%0.2f sec", t);

	// Shift queue and put at front of queue
	...
	{
		// Set first entry to " "
		printQueue[printQueue_lng - 1] = str;
...
void PrintDebug2LCD()
{
	...
	if (printQueueInd == printQueue_lng - 1)
	{
		doPrint = false;
	}
...
void PrintDebug2Comp()
{
	// Local vars
	String str;

	// Get current string
	str = printQueue[printQueue_lng - 1];
	...
	// Remove sent msg from front of queue
	for (int i = printQueue_lng - 1; i >= 1; i--)
	{
		printQueue[i] = printQueue[i - 1];
	}
	// Set first entry to " "
...
void DebugRsvd(char id, uint16_t pack)
{
	...
	if (doPrint_rcvd && (doDebugConsole || doDebugLCD))
	{
		...
		if (id == 'S')
		{
			sprintf(str, "rcvd: [i:%c d1:%d d2:%d p:%d", id, setupCmd[0], setupCmd[1], pack);
		}
		else if (id == 'M')
		{
			sprintf(str, "rcvd: [i:%c d1:%0.2f p:%d", id, movePos, pack);
		}
		else if (id == 'R')
		{
			sprintf(str, "rcvd: [i:%c d1:%0.2f p:%d", id, rewPos, pack);
		}
		else if (id == 'H')
		{
		...
		{
			sprintf(str, "rcvd: [i:%c d1:%d p:%d", id, bullDel, pack);
		}
		else if (id == 'I')
		{
			sprintf(str, "rcvd: [i:%c d1:%d p:%d", id, fc_doStartTracking, pack);
		}
...
void Interupt_Btn1() {
	...
	t_debounce[1] = millis() + max(ledDir, solDir) + 100;
