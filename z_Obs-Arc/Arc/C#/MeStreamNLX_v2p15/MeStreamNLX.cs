...
namespace MeStreamNLX
{
    ...
    class MeStreamNLX
    {
        ...
        private static bool isGUIclosed = false;
        ...
        private static char[] m2c_id = new char[10] { // prefix giving masage id
            'S', // start session
            'Q', // quit session
            'M', // move to position
            'R', // dispense reward
        ...
        static void Main()
        {
            ...
            PrintAction("[Main] RUNNING: ICR_GUI Load...");
            while (matIn_id != 'L' && !doAbort) ;
            if (!doAbort) PrintAction("[Main] FINISHED: ICR_GUI Load");
            ...
            if (!(com_netComClient.AreWeConnected()))
            {
                if (com_netComClient.ConnectToServer(NETCOM_IP))
                {
                    ...
                    if (!doAbort) PrintAction("[Main] RUNNING: MoveTo Start...");
                    ...
                    // Main holding loop
                    if (!doAbort) PrintAction("[Main] RUNNING: Main Loop...");
                    ...
                    PrintAction("[Main] FINISHED: Wait for Last Pack...");
                ...
                if (isMovedToStart && com_netComClient.AreWeConnected())
                {
                    ...
                    matIn_dat1 = CalcMove(4.7124 - feedDist);
                    // Send move command and wait for done reply
                    RepeatSend('M', 0, true);
                    PrintAction("[Main] FINISHED: MoveTo South");
                }

                // Wait 1 second to see move finish
                Thread.Sleep(1000);
                ...
                RepeatSend('Q');
                PrintAction("[Main] FINISHED: Sent Quit");
        ...
        private static bool RepeatSend(char id, ushort pack = 0, bool check_done = false)
        {
        ...
        public static ushort SendData(char id, ushort pack = 0)
        {
            ...
            lock (lock_sendData)
            {
                // Local vars
                long c2r_del = 15;
                double dat_1 = matIn_dat1;
                double dat_2 = matIn_dat2;
        ...
        private static void DoWork_RunGUI(object sender, DoWorkEventArgs e)
        {
            ...
            if (doDebugMat)
            {
                ...
                //com_Matlab.Execute(@"dbstop in ICR_GUI at 3002");
            }
        ...
        private static void RunWorkerCompleted_RunGUI(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (!doQuit)
            {
                ...
                doQuit = true;
                doAbort = true;
                PrintAction("[RunWorkerCompleted_RunGUI] !!FORCED EXIT!!");
            }
        }
        ...
        private static void ProgressChanged_MatCOM(object sender, ProgressChangedEventArgs e)
        {
            ...
            // Relay all matlab data to arduino except quit and save
            if (matIn_id == 'Q' || matIn_id == 'X') // quit
            {
                if (matIn_id == 'Q')
                {
                    // end polling
