...
if size(who('global'),1) > 0
    if ischar(c2m_O)
        fi_path = fullfile(c2m_O, 'ICR_GUILogFile.csv');
...
%% ==========================MAIN FUNCTION==================================
    ...
    function[] = RunScript()
            ...
            while c2m_E == 0
                ...
                elseif D.B.poll_nlx
                    ...
                    if (D.T.loop >= D.PAR.polRate)
                        ...
                        if Elapsed_Time(now, D.C.moveDel) > 2.5 &&  ~D.C.doneMove
        ...
        function[] = SF_NLX_Setup()
            ...
            % Print GUI ready
            Update_Console('FINISHED: GUI Loading');
        end
            ...
            if all(~isnan(D.P.Rat.rad))
                ...
                if any(check_inbound)
                    ...
                    if inbndTim >= D.PAR.strQdDel
                        ...
                        Update_Console(sprintf('Rat In: OCC=%0.2fsec', ...
                            inbndTim));
        ...
        function[] = SF_Run_Test_Setup()
            if isTestRun
                ...
                Update_Console('FINISHED: Test Run Setup Complete');
            end
        end
                ...
                if doHaultErrorTest
                    ...
                    if D.DB.isHalted && ...
                        ...
                        if D.DB.haltCnt == D.DB.stepSamp
                            ...
                            Update_Console(sprintf('New Vel: %d cm/sec', ...
                                D.DB.nowVel));
                        end
...
% ---------------------------PRINT TO CONSOLE------------------------------
    function [] = Update_Console(str)
        ...
        l_msg = sprintf('[%d],%d,%s\r', logCount, t_m, str);
        
        % Print new info to console
        if exist('consoleStr','var')
            consoleStr = [p_msg, consoleStr];
            logStr = [logStr,l_msg];
            disp(p_msg);
            if exist('FigH','var')
                if isfield(D,'UI')
...
% -----------------------------Disconnect ALL-------------------------------
    function [] = Disconnect_All()
        ...
        if exist('D','var')
            ...
            Update_Console('RUNNING: Disconnect all...');
