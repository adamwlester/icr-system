    ...
    function[] = Run()
        ...
        function[] = Forage_Task_Setup()
            ...
            if exist(D.DIR.ioRFPath, 'file')
                ...
                %ind = (floor(D.PAR.pathWdth/2)-3)*-1 : (floor(D.PAR.pathWdth/2)+3);
                ind = floor(D.PAR.pathWdth/2)*-1 : floor(D.PAR.pathWdth/2);
                ...
                ih = imagesc(sum(sum(D.P.pathMat(:,:,:,:),3),4), 'Parent', D.UI.axH(3));
                pause(1);
                delete(ih);
        ...
        function [] = VT_Proc(fld)
            ...
            if D.P.(fld).vtNRecs > 0
                ...
                if D.PAR.sesTask == 'Forage' && strcmp(fld, 'Rat')
                    ...
                    scale = prctile(D.P.occMatRaw(:),99.99);
                    if scale == 0
                        scale = 1;
                    end
        ...
        function [] = Forage_Reward_Zone_Check()
            ...
            path_rad = deg2rad(path_arr(path_ind)*2);
            ...
            if D.DB.doSimRatTest
                ...
                if D.F.rec
                    ...
                    if Elapsed_Seconds(now) - D.T.rec_tim > 5
                        ...
                        if D.F.rat_in && ...
                            ...
                            if D.PAR.sesTask == 'Track'
                                ...
                                if D.DB.simTargAng ~= D.PAR.pathTargArr(D.I.targInd)
                                    ...
                                    ang_str = D.DB.simTargAng;
                                    ang_targ = D.PAR.pathTargArr(D.I.targInd);
                                    [x1,y1] = pol2cart(deg2rad(ang_str), 1);
                                    [x2,y2] = pol2cart(deg2rad(ang_targ), 1);
                                    x_diff = x2-x1;
                                    y_diff = y2-y1;
                                ...
                                % TEMP
                                fprintf('roh=%0.2f bnd=%0.2f\r\n', roh, D.P.rfRohBnd(2)-(5/D.UI.arnRad))
                        ...
                        % TEMP
                        %fprintf('roh_diff=%0.2f roh_now=%0.2f rad_now=%0.2f rad_diff=%0.2f x=%0.2f y=%0.2f\r\n', roh_diff, roh_now, rad_now, rad_diff, x, y)
