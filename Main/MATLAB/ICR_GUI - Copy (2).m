function[status] = ICR_GUI(sysTest, doDebug, isCheetahOpen, isMatSolo)
% INPUT:
%	sysTest = [0,1,2,3]
%    	0: No test
%     	1: PID calibration
    ...
    function[] = Run()
        ...
        function [] = Finish_Setup()
            ...
            if D.PAR.isImplanted
                ...
                %NlxSendCommand('-ProcessConfigurationFile AWL-ICR_Ephys.cfg'); TEMP
                ...
                % TEMP Propt to load last configuration file
                if ~isempty(nlx_cfg_file_last) && false
                    load_last = dlgAWL(...
        ...
        function[] = Forage_Task_Setup()
            ...
            set(D.UI.ptchRewTargBnds(D.I.targInd), ...
        ...
        function [] = Track_Reward_Zone_Check()
            ...
            if  D.F.check_rew_confirm && ...
                ...
                D.I.zoneHist(sum([D.C.rew_cnt{:}])) = -1*D.PAR.zoneLocs(D.I.zone_now);
        ...
        function [] = Forage_Reward_Zone_Check()
            ...
            % Bail if waiting for targ select
            if get(D.UI.toggPickRewPos, 'UserData')
                return
            end
            ...
            if (get(D.UI.toggPickRewPos, 'Value') == 1)
                ...
                sub_targ = wrapTo360(D.PAR.rfTargDegArr(D.I.targInd) + 180 + D.PAR.rfPathDegArr(D.PAR.rfPathSelectInd)*2);
                ...
                % Set flag
                set(D.UI.toggPickRewPos, 'UserData', true);
            ...
            % Get new targ
            rad_last_targ = deg2rad(D.PAR.rfTargDegArr(D.I.targInd));
            ...
            Console_Write(sprintf('[Forage_Reward_Zone_Check] Rewarded: Targ_Last=%ddeg Targ_New=%ddeg', ...
                D.PAR.rfTargDegArr(targ_ind_last), D.PAR.rfTargDegArr(D.I.targInd)));
                    ...
                    if Elapsed_Seconds(now) - D.T.rec_tim > 5
                        ...
                        if D.F.initVals
                            ...
                            if D.PAR.sesTask == 'Forage'
                                ...
                                D.DB.simTargAng = D.PAR.rfTargDegArr(D.I.targInd);
                        ...
                        if D.F.rat_in && ...
                            ...
                            if D.PAR.sesTask == 'Track'
                                ...
                                if D.DB.simTargAng ~= D.PAR.rfTargDegArr(D.I.targInd)
                                    
                                    % Get current rad pos
                                    [rad_start, ~] = VT_2_Pol(D.DB.simXY);
                                    
                                    % Add some noise
                                    ang_end = D.PAR.rfTargDegArr(D.I.targInd);
                                    noise = (rand(1) - 0.5) * ...
        ...
        function [] = MouseTrack(~, ~, ~)
            ...
            if D.PAR.sesTask == 'Track' && ...
                    ...
                    get(D.UI.toggPickRewPos, 'UserData')
