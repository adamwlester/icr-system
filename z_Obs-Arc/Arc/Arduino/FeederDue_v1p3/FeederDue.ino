...
float estRatVel;
float estRobVel;
float posArr[M] = { -100, 0, 0, 0, 0, 0 }; // ekf measurements
...
void loop() {
	...
	if (posUpdate[0] && posUpdate[1] && posUpdate[2])
	{
		...
		double z[M] = {
			...
			pixyPosRad,
			vtRad[1],
			CompRadVel(ekf.getX(0), vtRad[0], posTS[0][0], posTS[1][0]),
			CompRadVel(ekf.getX(0), pixyPosRad, posTS[0][2], posTS[1][2]),
			CompRadVel(ekf.getX(1), vtRad[1], posTS[0][1], posTS[1][1]),
		};
		if (firstPass)
		{
			z[3] = 0;
