...
const bool doPrint_motorControl = true;
...
const float defualtSetPoint = 48; // (cm)
...
#pragma region //--- (Q) DO QUIT ---
	...
	// Wait for queue buffer to empty, move to finish and quit delay to pass 
	if (
		fc_doQuit &&
		millis() > t_quitCmd && !doSend)
	{
		// Retell ard to quit
		Store4_Ard('q');
		DebugState("QUITING...");
...
#pragma region //--- (B) BULLDOZE RAT STATUS ---
	if (msg_id == 'B' && msg_pass)
	{
		...
		bull.Reinitialize(bullDel, bullSpeed, "MsgB");
...
void CheckBlockTimElapsed()
{
	if (fc_isBlockingTill)
	{
		...
		bool passed_feeder =
			...
			pos_ratVT.GetPos() - (ekf_robPos + feedDist) > 0 &&
			pos_ratPixy.GetPos() - (ekf_robPos + feedDist) > 0;
...
bool CompTarg(float move_targ, String move_type)
{
	...
	if (move_type == "Cue" && !fc_isTrackingEnabled)
	{
		fc_doCue = false;
		fc_isCueTargSet = false;
	...
	else
	{
		...
		{
			//  Take motor control
			pass = SetMotorControl("MoveTo", "CompTarg");
...
bool DoDecelerate(String move_type)
{
	...
	if (distLeft <= dec_pos)
	{
		...
		if (base_speed == 0)
		{
			...
			{
				//  Take motor control
				SetMotorControl("MoveTo", "DoDecelerate");
			}
		}
...
void StartRew()
{
	...
	if (vol_doRew)
	{
		vol_isButtonReward = true;
		PrintLCD("REWARDING...");
	}
	else
	{
		DebugState("REWARDING...");
	}
...
void EndRew()
{
	...
	if (millis() > t_rewSolClose)
	{
		...
		// Clear LCD
		if (vol_isButtonReward)
		{
			ClearLCD();
			vol_isButtonReward = false;
		}
		else
		{
