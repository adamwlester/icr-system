...
try
    ...
    Update_Console(sprintf(['\r!!!!!!!!!!!!!ERROR!!!!!!!!!!!!!\r\r'...
        'ID: %s\r\r'...
        'Msg: %s\r\r'...
        'Line: %d\r\r'...
        'Time: %s\r\r' ...
...
% ------------------------------VAR SETUP----------------------------------
    ...
    function [] = SF_Var_Setup()
        ...
        D.P.Rat.vel_avg = NaN(3,2);
        D.P.Rob.vel_avg = NaN(3,2);
        % position cutoff
...
% -------------------------------UI SETUP-----------------------------------
    ...
    function[] = SF_UI_Setup()
        ...
        n_rings = ceil(D.P.velRohMax/10)+1;
...
% ------------------------------PROCESS VT---------------------------------
    ...
    function [] = SF_VT_Proc(fld)
        ...
        if isempty(vel) || isempty(D.P.(fld).velRad)
            ...
            D.P.(fld).vel_avg = circshift(D.P.(fld).vel_avg, -1, 1);
            ...
            % convert to car
            [D.P.(fld).vel_avg(end,1), D.P.(fld).vel_avg(end,2)] = ...
                pol2cart(D.P.(fld).velRad, velRoh);
            D.P.(fld).vel_avg(end,1) =  D.P.(fld).vel_avg(end,1).*D.PAR.R + D.PAR.XC;
            D.P.(fld).vel_avg(end,2) =  D.P.(fld).vel_avg(end,2).*D.PAR.R + D.PAR.YC;
...
% ----------------------------LAP CHECK------------------------------------
    ...
    function [] = SF_Lap_Check()
        ...
        if all(D.B.track_lap);
            ...
            set(D.UI.ratPltHvel(~isnan(D.UI.ratPltHvel)), 'MarkerEdgeColor', D.UI.ratHistCol);
            set(D.UI.robPltHvel(~isnan(D.UI.robPltHvel)), 'MarkerEdgeColor', D.UI.robHistCol);
    ...
    function [] = SF_Pos_Plot()
        ...
        if D.B.Rat.plot_vel
            D.UI.ratPltHvel(Hind) = ...
                plot(D.P.Rat.vel_avg(:,1), D.P.Rat.vel_avg(:,2), '-', ...
        ...
        if D.B.Rob.plot_vel
            D.UI.robPltHvel(Hind) = ...
                plot(D.P.Rob.vel_avg(:,1), D.P.Rob.vel_avg(:,2), '-', ...
                'Color', D.UI.robPosCol, ...
                'LineWidth', 1, ...
                'Parent', D.UI.axH(1));
        end
