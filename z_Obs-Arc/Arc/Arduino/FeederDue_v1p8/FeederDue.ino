...
public:
	...
	float CompVel(float posNew, int tsNew)
	{
		// shift and add data
		...
		if (iteration >= nSamp)
		{
			...
			acc = (velLast - vel / (dtSum / (float)1000));
...
void loop() {
	...
	if (posUpdate[0] && posUpdate[1] && posUpdate[2])
	{
		...
		double z[M] = {
			...
			pixyPosCM,
			vtPosCM[1],
			vtRatHist.CompVel(vtPosCM[0], vtTS[0]),
			pixyRatHist.CompVel(pixyPosCM, pixyPosTS),
			vtRobHist.CompVel(vtPosCM[1], vtTS[0]),
		};
