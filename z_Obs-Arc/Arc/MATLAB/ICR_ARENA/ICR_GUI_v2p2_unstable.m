...
global m2c_idGlob; % message out to CS
global c2m_idGlob; % message in from CS
global m2c_dat1Glob; % data out to CS
global m2c_dat2Glob; % data out to CS
global m2c_flagGlob; % new data flag out to CS
...
% ----------------------------RAT IN CHECK---------------------------------
    ...
    function [D] = SF_Rat_In_Check(D)
        ...
        if ...
            ...
            D.CS.setup(1) = double(~D.B.manual_ses);
            D.CS.setup(2) = double(D.UI.snd(2));
            CS_Send(1, ...
                D.CS.setup(1), ... % session cond
                D.CS.setup(2)); % reward tone
...
% -------------------------REWARD FEEDER CHECK-----------------------------
    ...
    function [D] = SF_Reward_Feeder_Check(D)
        ...
        if ...
            ...
            D.C.rec_cnt = D.C.rec_cnt + D.vtNRecs;
            ...
            if ...
                ...
                if ...
                    ...
                    % Tell CS next reward pos to slow to
                    CS_Send(4, 0);
...
% SOUND CONDITION
    function [] = RadSnd(~, ~, ~)
        ...
        % Change robot command strings based on sound settings
        if updateRob
            
            % Turn on tone
            if (D.CS.setup(2) ~= double(D.UI.snd(2)))
                D.CS.setup(2) = double(D.UI.snd(2));
                CS_Send(1, ...
                    D.CS.setup(1), ... % session cond
                    D.CS.setup(2)); % reward tone
