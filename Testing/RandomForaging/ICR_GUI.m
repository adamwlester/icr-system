...
% ------------------------------MAIN RUN-----------------------------------
    function[] = Run()
        ...
        function [] = Var_Setup()
            ...
            % pos occ bins
            D.UI.rfBins = D.UI.arnRad*2 + 1;
            % width of path
            D.PAR.pathWdth = 5;
            % distance between paths in degrees
            D.PAR.pathDegDist = 5;
            % target width
            D.PAR.pathTargWdt = 10;
            % target array
            ...
            D.P.trackRohMax = 1 + (5/D.UI.arnRad);
            D.P.trackRohMin = (1 - (D.UI.trkWdt+5)/D.UI.arnRad);
            % velocity cutoff
            D.P.velRohMax = D.P.trackRohMin;
            D.P.velRohMin = D.P.trackRohMin - 0.25;
            D.P.velMin = 0;
            D.P.velMax = 100;
            % random forage
            D.P.occMat = zeros(D.UI.rfBins,D.UI.rfBins);
            D.P.pathMat = zeros(D.UI.rfBins,D.UI.rfBins,D.PAR.nPaths,length(D.PAR.pathTargArr));
        ...
        function[] = UI_Setup()
            ...
            %                 'XLim', [0,D.UI.vtRes],...
            %                 'YLim', [0,D.UI.vtRes]);
            %             imshow(ones(D.UI.vtRes,D.UI.vtRes)*0.5, 'Parent',D.UI.axH(2));
            %             set(FigH,'visible','on');
            ...
            % Store path mask
            D.PAR.pathMask = imresize(~D.PAR.pathMask, [D.UI.rfBins,D.UI.rfBins]);
            ...
            % Wall image axis
            set(D.UI.axH(4), 'Position', img_ax_pos);
            
            % Read in image
            [img{1}, ~, alph] = imread(D.DIR.ioWallImage);
            set(D.UI.axH(4), 'XLim', [0,size(img{1},2)], 'YLim', [0,size(img{1},1)]);
            img{1} = flip(img{1}, 1);
            ...
            % Store for later
            D.UI.wallImgH(1) = image(img{1}, 'Parent', D.UI.axH(4), 'Visible', 'on');
            D.UI.wallImgH(2) = image(img{2}, 'Parent', D.UI.axH(4), 'Visible', 'off');
            D.UI.wallImgH(3) = image(img{3}, 'Parent', D.UI.axH(4), 'Visible', 'off');
            % set alpha
        ...
        function [] = Finish_Setup()
            ...
            for z_targ = 1:length(D.PAR.pathTargArr)
                D.PAR.rewTargBnds(z_targ,:) = [...
                    deg2rad(D.PAR.pathTargArr(z_targ) - D.PAR.pathTargWdt/2), ...
                    deg2rad(D.PAR.pathTargArr(z_targ) + D.PAR.pathTargWdt/2)];
            end
            % [zone,min_max,rot_cond]
        ...
        function[] = Forage_Task_Setup()
            ...
            % Compute occ bins
            %D.PAR.rfBinEdgeX = linspace(0,pixels,nBins+1);
            ...
            if (do_plot_avg)
                % Plot accross paths
                ih = imagesc(sum(D.P.pathMat(:,:,:,1),3), 'Parent', D.UI.axH(1));
                pause(1);
                delete(ih);
                % Plot accross start pos and paths
                ih = imagesc(sum(sum(D.P.pathMat(:,:,:,:),3),4), 'Parent', D.UI.axH(1));
                pause(1);
                delete(ih);
            end
            ...
            if (do_plot_all)
                for j = 1:n_targs
                    ih = imagesc(sum(D.P.pathMat(:,:,:,j),3), 'Parent', D.UI.axH(1));
                    pause(0.5);
                end
            end
        ...
        function [] = Rat_In_Check()
            ...
            check_inbound = Check_Rad_Bnds(D.P.Rat.rad, D.PAR.strQuadBnds);
        ...
        function [] = VT_Proc(fld)
            ...
            if D.P.(fld).vtNRecs > 0
                ...
                if ~(D.PAR.sesTask == 'Forage' && strcmp(fld, 'Rat'))
                    exc_1 = roh > D.P.trackRohMax | roh < D.P.trackRohMin;
                else
                    exc_1 = false(length(roh),1);
                end
        ...
        function [] = Lap_Check()
            ...
            track_quad = Check_Rad_Bnds(D.P.Rat.rad, D.PAR.lapBnds(D.I.lap_hunt_ind, :));
