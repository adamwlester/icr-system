function[status] = ICR_GUI(sysTest, doDebug, isCheetahOpen, isMatSolo, isTTtrackSolo, doAutoLoad)
% INPUT:
%	sysTest = [0,1,2,3]
%    	0: No test
%     	1: PID calibration
...
% -----------------------------MAIN SETUP----------------------------------
    function[] = Setup()
        ...
        timer_graphics.TimerFcn = {@TimerCallbackGraphics, 'Other'};
        
        % Setup save timer
        timer_save = timer;
        timer_save.ExecutionMode = 'fixedRate';
        timer_save.Period = 0.5;
        timer_save.TimerFcn = {@TimerCallbackGraphics, 'Save'};
        timer_save.StopFcn = {@StopBtnStatus, 'Save'};
        
        % Setup quit timer
        timer_quit = timer;
        timer_quit.ExecutionMode = 'fixedRate';
        timer_quit.Period = 0.5;
        timer_quit.TimerFcn = {@TimerCallbackGraphics, 'Quit'};
        timer_quit.StopFcn = {@StopBtnStatus, 'Quit'};
...
% ------------------------------MAIN RUN-----------------------------------
    function[] = Run()
        ...
        while ~doExit
            ...
            % Handle flow
            if ~D.F.rat_loaded && ...
                    (~isTTtrackSolo || D.F.tt_track_setup_done)
            ...
            switch D.F.main_what
                ...
                case 'RUN TASK'
                    ...
                    elseif (D.F.rec_done || D.F.quit) && ~D.F.rat_out
                        D.F.poll_what = 'SEND RECORDING DONE CONFIRMATION';
                        
                    elseif D.F.rec_done && D.F.rat_out
                        D.F.poll_what = 'WAIT FOR SAVE';
                    ...
                    switch D.F.poll_what
                        ...
                        case 'WAIT FOR SAVE'
                            ...
                            if c2m.('Y').dat1 == 1
                                ...
                                % Enable save button
                                Set_Button_State(D.UI.btnSaveSes, 'Enable', D.UI.activeCol);
...
% ------------------------------VAR SETUP--------------------------
    ...
    function Var_Setup()
        ...
        % Figure position
        fig_wh = [1240 1025];
        fig_lft = D.UI.monPos(D.UI.figMon,1) + (D.UI.monPos(D.UI.figMon,3)-fig_wh(1))/2;
        fig_btm = D.UI.monPos(D.UI.figMon,2) + (D.UI.monPos(D.UI.figMon,4)-fig_wh(2))/2;
        ...
        D.UI.pxl2norm_x = 1/D.UI.figGrpPos(3);
        D.UI.pxl2norm_y = 1/D.UI.figGrpPos(4);
...
% -------------------------------UI SETUP--------------------------
    ...
    function[] = UI_Setup()
        ...
        % Set axis pos
        ax_btm = 1 - D.UI.fig_rng(2)/D.UI.figGrpPos(4);
        D.UI.axPos = [ ...
        ...
        % Dialogue box
        D.UI.dlgPos = [D.UI.monPos(D.UI.figMon,1) + D.UI.monPos(D.UI.figMon,3)/2, D.UI.monPos(D.UI.figMon,4)/2];
        ...
        % Panel console
        pan_ht = 1 - D.UI.fig_rng(2)/D.UI.figGrpPos(4);
        D.UI.cnsl_pan_pos = ...
        ...
        lft = pos_lft_dflt*2;
        pos = [lft, btm, wd, ht];
        D.UI.toggCellsCut = uicontrol('Style','togglebutton', ...
            'Parent',D.UI.tabICR, ...
            'String','Cells Cut', ...
        ...
        % RECORDING DONE
        ht = 0.04;
        wd = D.UI.rec_pan_pos(3)/2;
        btm = D.UI.rec_pan_pos(2) + 0.01;
        pos = [wd/2, btm, wd, ht];
...
% ------------------------------NLX SETUP--------------------------
    ...
    function[] = NLX_Setup()
        ...
        % Set port directions
        SendM2NLX(['-SetDigitalIOportDirection ', D.NLX.DevName, ' ', D.NLX.port_0, ' Input']);
        SendM2NLX(['-SetDigitalIOportDirection ', D.NLX.DevName, ' ', D.NLX.port_1, ' Input']);
        
        % Enable digital io events
        SendM2NLX(['-SetDigitalIOEventsEnabled ', D.NLX.DevName, ' ', D.NLX.port_0, ' True']);
        SendM2NLX(['-SetDigitalIOEventsEnabled ', D.NLX.DevName, ' ', D.NLX.port_1, ' True']);
        
        % CONFIGURE TTL EVENTS
        
        % Audio channels
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.snd_rt_wn_bit{1}, ' ', D.NLX.snd_rt_wn_bit{2}, ' ', D.NLX.snd_rt_wn_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.snd_lft_rt_bit{1}, ' ', D.NLX.snd_lft_rt_bit{2}, ' ', D.NLX.snd_lft_rt_str]);
        
        % Reward
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.rew_on_bit{1}, ' ', D.NLX.rew_on_bit{2}, ' ', D.NLX.rew_on_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.rew_off_bit{1}, ' ', D.NLX.rew_off_bit{2}, ' ', D.NLX.rew_off_str]);
        
        % Pid state
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.pid_run_bit{1}, ' ', D.NLX.pid_run_bit{2}, ' ', D.NLX.pid_run_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.pid_stop_bit{1}, ' ', D.NLX.pid_stop_bit{2}, ' ', D.NLX.pid_stop_str]);
        
        % Bulldozer state
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.bull_run_bit{1}, ' ', D.NLX.bull_run_bit{2}, ' ', D.NLX.bull_run_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.bull_stop_bit{1}, ' ', D.NLX.bull_stop_bit{2}, ' ', D.NLX.bull_stop_str]);
        
        % Photo transdicers
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.north_bit{1}, ' ', D.NLX.north_bit{2}, ' ', D.NLX.north_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.west_bit{1}, ' ', D.NLX.west_bit{2}, ' ', D.NLX.west_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.south_bit{1}, ' ', D.NLX.south_bit{2}, ' ', D.NLX.south_str]);
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.east_bit{1}, ' ', D.NLX.east_bit{2}, ' ', D.NLX.east_str]);
        
        % IR time sync LED
        SendM2NLX(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.ir_ts_bit{1}, ' ', D.NLX.ir_ts_bit{2}, ' ', D.NLX.ir_ts_str]);
    ...
    function Finish_NLX_Setup()
        ...
        if ~D.F.implant_session && ~isTTtrackSolo
            ...
            if ~any(ismember(das_types, 'TTScAcqEnt'))
                SendM2NLX('-ProcessConfigurationFile AWL-ICR_Ephys.cfg');
            end
            
            % Turn on Cube LEDs
            %SendM2NLX('-SendLynxSXCommand AcqSystem1 -WHSSetTrackingLED 1 1');
            % Turn off Cube LEDs
            %SendM2NLX('-SendLynxSXCommand AcqSystem1 -WHSSetTrackingLED 1 0');
            ...
            if strcmp(load_last, 'Yes')
                ...
                SendM2NLX(sprintf('-ProcessConfigurationFile "%s"', nlc_cfg_path));
            end
    ...
    function Finish_Ephys_Setup()
        ...
        if D.F.implant_session
            ...
            if ~isSS3DOpen
                ...
                Console_Write('[NLX_Setup] SpikeSort3D.exe Already Running');
            end
            
            % Wait for SpikeSort3D to open
            tic
            while D.F.implant_session && ...
                    ~isSS3DOpen && ...
                    ~doExit
        ...
        if D.F.implant_session
            ...
            for z_c = 1:10
                ...
                % Actual plot axis
                D.UI.axClstH(z_c,1) = ...
                    copyobj(D.UI.axH(5), D.UI.tabICR);
                hold on;
...
% ----------------------ROTATION TRIGGER CHECK---------------------
    ...
    function Rotation_Trig_Check()
        ...
        SendM2NLX(D.NLX.rot_evt{D.I.rot});
    ...
    function Inf_Print()
        ...
        if D.F.implant_session
            ...
            if Elapsed_Seconds(now) >= D.T.cube_vcc_update
                ...
                % Send NLX command
                [pass, nlx_str] = SendM2NLX('-SendLynxSXCommand AcqSystem1 -WHSGetStateOfCharge 1', false);
            ...
            if D.F.rec
                ...
                if Elapsed_Seconds(now) - D.T.rec_tim > 5
                    ...
                    if isCheetahOpen
                        ...
                        SendM2NLX('-PostEvent Get_Sim_TS 0 0', false);
...
% -------------------------SAVE SESSION DATA---------------------------
    function  Save_Ses_Data()
        ...
        Set_Button_State(D.UI.btnQuitSes, 'Enable', D.UI.activeCol);
        ...
        if isfield(D, 'NLX')
            if ~isempty(D.NLX)
                if NlxAreWeConnected() == 1
                    ...
                    SendM2NLX('-StopRecording');
                    SendM2NLX('-StopAcquisition');
                    pause(0.1);
...
% ------------------------------QUIT ALL-----------------------------------
    function BtnQuitSes(~, ~, ~)
        ...
        % Only run once
        if get(D.UI.btnQuitSes(1), 'UserData') > 0
            return
        end
        
        % Check for save unless quit before setup done
        if ~D.UI.save_done && D.F.rat_loaded
        ...
        % Disable save button so only pressed once
        set(D.UI.btnQuitSes, 'UserData', 1);
        ...
        % Log/print
        Console_Write(sprintf('[%s] Set to \"%d\"', 'BtnQuitSes', get(D.UI.btnQuitSes(1),'Value')));
        Update_UI(10);
...
% ---------------------------PLAY SOUND FO TT------------------------------
    function ToggHearTT(hObject, ~, ~)
        ...
        if do_dev_reset
            ...
            SendM2NLX('-SetAudioSource "AcqSystem1_Audio1" Right None');
            
            % Disable speaker
            SendM2NLX('-SetAudioSource "Primary Sound Driver" Left None');
            SendM2NLX('-SetAudioSource "Primary Sound Driver" Right None');
        ...
        % Set chanels that are on on this tt
        keep_on = chan_on_ind(b_on_ind==b_ind & tt_on_ind==tt_ind);
        if ~isempty(keep_on)
        ...
        if strcmp(hObject.String, 'L') || strcmp(hObject.String, 'R')
            ...
            if val == 0
                ...
                % Get list of all chanels on this side that are on
            ...
            if val == 0
                ...
                ent_set = D.TT.ttFlds{b_ind,tt_ind};
        ...
        % Set device
        SendM2NLX(['-SetAudioSource ', msg_str]);
        ...
        if ~strcmp(ent_set, 'None')
            for z_chn = 1:4
                ...
                % Send command
                SendM2NLX(['-SetAudioStreamEnabled ', msg_str]);
...
% ----------------BUTTON PRESS TO LOAD TT DATA INTO PANEL------------------
    function ToggLoadTT(hObject, ~, ~)
        ...
        Set_Button_State(D.UI.btnSaveTT, 'Enable');
...
% ----------------------------ENABLE/DISABLE TT----------------------------
    function ToggIncludeTT(hObject, ~, ~)
        ...
        % Store value
        tt_list_ind = ismember(D.TT.ttList, D.TT.ttFlds{b_ind, tt_ind});
        D.TT.ttInclude(tt_list_ind) = ~val;
        D.TT_IO.TT_Include{D.TT.ratIndTT,:} = D.TT.ttInclude;
        
        % Update buttons
        Set_Button_State(D.UI.toggIncludeTT(b_ind, tt_ind), 'Update');
...
% ----------------------------SELECT TT ACTION-----------------------------
    function ToggActionTT(hObject, ~, ~)
        ...
        if strcmp('Track TTs', action_str)
            if val == 1
                Show_Hide('Track TTs', 'Show');
                Show_Hide('Plot Spikes', 'Hide');
            else
                Show_Hide('Track TTs', 'Hide');
            end
        end
...
% --------------------BUTTON PRESS TO SAVE TT ENTRIES----------------------
    function BtnSaveTT(~, ~, ~)
        ...
        for z_tt = 1:length(D.TT.ttFlds(b_ind,:))
            
            % Save for session
            tt_fld = D.TT.ttFlds{b_ind,z_tt};
        ...
        Set_Button_State(D.UI.btnSaveTT, 'Unenable');
        set(D.UI.toggTT, 'Value', 0);
        
        % Update track plot
        PlotTTPath(b_ind,tt_ind);
...
% ------------------------BUTTON STATUS RUN TIMER--------------------------
    ...
    function TimerCallbackGraphics(~, ~, agent)
        ...
        switch agent
            ...
            case 'Other'
                ...
                if D.PAR.cube_vcc <= D.PAR.cubeVccWarning && D.PAR.cube_vcc > 0
                    set(D.UI.txtPerfInf(8), 'ForegroundColor', D.UI.warningCol);
            ...
            case 'Quit'
                ...
                % Itterate count
                D.UI.btnQuitSes.UserData = D.UI.btnQuitSes.UserData +1;
                if D.UI.btnQuitSes.UserData>4
                    D.UI.btnQuitSes.UserData = 1;
                end
                
                % Change button string
                bnt_str = ...
                    sprintf('%s%s%s', agent, ...
                    repmat('.',1,D.UI.btnQuitSes.UserData), ...
                    blanks(4-D.UI.btnQuitSes.UserData));
                set(D.UI.btnQuitSes, ...
                    'String', bnt_str);
...
% -----------------------BUTTON STATUS STOP TIMER--------------------------
    ...
    function StopBtnStatus(~, ~, btn_str)
        ...
        if (strcmp(btn_str,'Quit'))
            % Disable quit button
            if ~isfield(D.UI, 'btnQuitSes')
                if ishandle(D.UI.btnQuitSes(1))
                    set(D.UI.btnQuitSes, 'String', btn_str);
                    Set_Button_State(D.UI.btnQuitSes, 'Unenable');
                end
            end
        else
            % Disable save button
...
% ---------------------SHOW HIDE HEAR OBJECTS------------------------------
    ...
    function Show_Hide(action_str, setting_str)
        % Note:
...
% ----------------------------- PLOT TT PATHS -----------------------------
    function PlotTTPath(bndl, tt)
        ...
        [D.UI.ttTrkLin(bndl,tt), D.UI.ttTrkMrk(bndl,tt,1:length(px))] = ...
            Get3dTT([px, py, pz], D.PAR.ttDiam*100, 20, state, D.UI.axe3dH);
...
% --------------------------SEND DATA TO NLX-------------------------------
    ...
    function[pass, arg_out] = SendM2NLX(msg, do_print)
