...
% ------------------------------VAR SETUP--------------------------
    function Var_Setup()
        ...
        D.UI.ttClustAxNow = gobjects(D.PAR.maxTT,D.PAR.maxClust);
        ...
        D.UI.toggSubFlagTT = gobjects(D.PAR.maxTT,6);
        % TT hear sub toggles
...
% -------------------------------UI SETUP--------------------------
    function[] = UI_Setup()
        ...
        Safe_Set(D.UI.toggICR, 'Callback', {@ToggICR})
        ...
        Safe_Set(D.UI.toggSleep, 'Callback', {@ToggSleep})
...
% --------------------------TT TRACKING SETUP----------------------
    function[was_ran] = TT_Track_Setup()
        ...
        % Set callback
        Safe_Set(D.UI.btnSetView, 'Callback', {@Set3dView});
        ...
        % Set callback
        Safe_Set(D.UI.sldSwtchImg, 'Callback', {@SldSwtchImg});
        ...
        % Set callback
        Safe_Set(D.UI.toggHideTT, 'Callback', {@ToggHideShowBndl});
        ...
        % Set callback
        Safe_Set(D.UI.toggMainActionTT, 'Callback', {@ToggMainActionTT})
        ...
        % Set callback
        Safe_Set(D.UI.toggFlagTT, 'Callback', {@ToggFlagTT})
        Safe_Set(D.UI.toggHearSourceTT, 'Callback', {@ToggHearSourceTT})
        Safe_Set(D.UI.toggPlotTypeTT, 'Callback', {@ToggPlotTypeTT});
        ...
        % Specify tt colors
        if D.TT.nBndl==2
            D.UI.ttCol = [hsv(sum(D.I.ttBndl==1)); hsv(sum(D.I.ttBndl==2))];
        else
            D.UI.ttCol = hsv(sum(D.I.ttBndl==1));
        end
        ...
        for z_l = 1:3
            ...
            tt_list = D.TT.ttList(D.I.ttBndl==z_l);
            for z_tt = 1:length(tt_list)
                
                % Find position of next tt
                tt_fld = tt_list{z_tt};
                [x,y] = find(ismember(D.TT.ttMap{z_l}, tt_fld) == 1);
                
                % Get actual tt ind
                tt_ind = ismember(D.TT.ttList, tt_fld);
        ...
        % Get TT loc
        tt_lab_shift = circshift(D.TT.ttList, 1);
        ...
        for z_tt = 1:length(tt_lab_shift)
            ...
            tt_ind = find(ismember(D.TT.ttList, tt_lab));
        ...
        wdth_norm = 0.375;
        lft_norm = 0.005;
        ht_norm = 1/(length(D.TT.ttList)/2) * 0.85;
        btm_norm = ...
            repmat(linspace(1-ht_norm*1.25, 0.005, (length(D.TT.ttList)/2)),1,2);
        ...
        for z_tt = 1:length(D.TT.ttList)
            ...
            D.UI.toggIncludeTT(z_tt) = uicontrol('style','togglebutton', ...
                'Parent', D.UI.panSelectTT(bndl_ind), ...
                'Units', 'Normalized', ...
                'String', sprintf('%c', char(216)),...
            ...
            D.UI.toggSubFlagTT(z_tt,1:3) = copyobj(D.UI.toggSubPlotTT(z_tt,1:3), D.UI.panSelectTT(bndl_ind));
            D.UI.toggSubFlagTT(z_tt,4:end) = copyobj(D.UI.toggSubPlotTT(z_tt,7:9), D.UI.panSelectTT(bndl_ind));
            ...
            for z_f = 1:6
                ...
                % Get value
                val = D.TT.ttLogNew.([tt_fld,'_F'])(z_f);
                ...
                % Change pos
                D.UI.toggSubFlagTT(z_tt,z_f).Position(1) = lft_arr(z_f);
                D.UI.toggSubFlagTT(z_tt,z_f).Position(3) = wd;
        ...
        dpth_max = 0;
        for z_tt = 1:length(D.TT.ttList)
            dpth_max = max(dpth_max, D.TT.ttLogNew.([D.TT.ttList{z_tt},'_D']));
        end
...
% --------------------------FINISH NLX SETUP------------------------
    function[was_ran] = Finish_NLX_Setup()
        ...
        if D.F.implant_session || D.PAR.sesType == 'TT_Turn'
            ...
            %       "A": Medium (blue dot)
            D.F.cube_connected = ...
                Send_M2NLX(sprintf('-SendLynxSXCommand AcqSystem1 -SetBatteryType "%s"', D.PAR.cubeBatteryType));
            D.F.cube_connected = D.F.cube_connected==1;
            
            % Print Cube status
            if D.F.cube_connected
                Console_Write('[Finish_NLX_Setup] FINISHED: Setup Cube');
            else
                Console_Write('**WARNING** [Finish_NLX_Setup] FAILED: Setup Cube');
            end
...
% --------------------------FORAGE TASK SETUP----------------------
    function[] = Forage_Task_Setup()
        ...
        Safe_Set(D.UI.imgFrgMaskH, 'Visible', 'on');
        Patch_State(D.UI.ptchStQ, 'Select');
        Safe_Set(D.UI.txtStQ, 'Visible', 'on');
    ...
    function VT_Proc(fld)
        ...
        if D.P.(fld).vtNRecs > 0
            ...
            if strcmp(fld, 'Rat')
                ...
                ind_str = D.P.Rat.posAll.indAll(2) + 1;
                ind_end = ind_str+length(x)-1;
                
                % Store indeces
                D.P.(fld).posAll.indAll(2) = ind_end;
                D.P.(fld).posAll.indLap(2) = ind_end;
                
                % Store cart pos data
                D.P.(fld).posAll.Cart(ind_str:ind_end, 1) = single(x);
                D.P.(fld).posAll.Cart(ind_str:ind_end, 2) = single(y);
                
                % Store rad data
                D.P.(fld).posAll.Pol(ind_str:ind_end, 1) = single(rad);
                D.P.(fld).posAll.Pol(ind_str:ind_end, 2) = single(roh);
                
                % Store ts
                D.P.(fld).posAll.TS(ind_str:ind_end) = ts;
            end
            ...
            vel_ind = D.P.(fld).velAll.indAll(2) + 1;
            D.P.(fld).velAll.indAll(2) = vel_ind;
            D.P.(fld).velAll.indLap(2) = vel_ind;
            
            % Store TS
            D.P.(fld).velAll.TS(vel_ind) = ts(end);
            ...
            if isempty(vel) || isempty(D.P.(fld).velRad)
                ...
                D.P.(fld).velAll.Cart(vel_ind,:) = single(NaN);
                D.P.(fld).velAll.Pol(vel_ind,:) = single(NaN);
            else
                ...
                if ~set_vel_nan
                    ...
                    % Store values
                    D.P.(fld).velAll.Cart(vel_ind, 1) = single(x);
                    D.P.(fld).velAll.Cart(vel_ind, 1) = single(y);
                else
                ...
                % Save pol history
                D.P.(fld).velAll.Pol(vel_ind, 1) = single(D.P.(fld).velRad);
                D.P.(fld).velAll.Pol(vel_ind, 2) = single(velRoh);
...
% ----------------------PLOT CURRENT POSITION----------------------
    function VT_Plot_New()
        ...
        if D.F.Rat.new_pos_data
            ...
            ind_str = D.P.Rat.posAll.indLap(1);
            ind_end = D.P.Rat.posAll.indLap(2);
            x = D.P.Rat.posAll.Cart( ind_str: ind_end,1);
            y = D.P.Rat.posAll.Cart( ind_str: ind_end,2);
        ...
        if D.F.Rob.new_vel_data
            ...
            ind_str = D.P.Rob.velAll.indLap(1);
            ind_end = D.P.Rob.velAll.indLap(2);
            x = D.P.Rob.velAll.Cart(ind_str:ind_end, 1);
            y = D.P.Rob.velAll.Cart(ind_str:ind_end, 2);
            % Plot
        ...
        if D.F.Rat.new_vel_data
            ...
            % Get non-zero vals to plot
            ind_str = D.P.Rat.velAll.indLap(1);
            ind_end = D.P.Rat.velAll.indLap(2);
            x = D.P.Rat.velAll.Cart(ind_str:ind_end, 1);
            y = D.P.Rat.velAll.Cart(ind_str:ind_end, 2);
            % Plot
...
% ----------------------PLOT POSITION HISTORY----------------------
    function VT_Plot_Hist()
        ...
        % Plot rat pos all
        ind_str = D.P.Rat.posAll.indAll(1);
        ind_end = D.P.Rat.posAll.indAll(2);
        x = D.P.Rat.posAll.Cart(ind_str:ind_end,1);
        y = D.P.Rat.posAll.Cart(ind_str:ind_end,2);
        ...
        for i = [2,1]
            ...
            ind_str = D.P.(fld).velAll.indAll(1);
            ind_end = D.P.(fld).velAll.indAll(2);
            vel_rad = D.P.(fld).velAll.Pol(ind_str:ind_end, 1);
            vel_roh = D.P.(fld).velAll.Pol(ind_str:ind_end, 2);
            ...
            % Add to hist data
            ind_str = D.P.(fld).velAll.indHist(1);
            ind_end = D.P.(fld).velAll.indHist(2)+1;
            D.P.(fld).velAll.Hist(ind_end,1:101,1) = rad_bins;
            D.P.(fld).velAll.Hist(ind_end,1:101,2) = roh_interp;
            
            % Store ind
            D.P.(fld).velAll.indHist(2) = ind_end;
            
            % Get rad vel as 1D array
            vel_rad = reshape(D.P.(fld).velAll.Hist(ind_str:ind_end,:,1)',1,[]);
            vel_roh = reshape(D.P.(fld).velAll.Hist(ind_str:ind_end,:,2)',1,[]);
            ind = ~isnan(vel_rad);
        ...
        for i = [2,1]
            ...
            ind_str = D.P.(fld).velAll.indHist(1);
            ind_end = D.P.(fld).velAll.indHist(2);
            vel_rad = D.P.(fld).velAll.Hist(ind_str,:,1);
            roh_avg = nanmean(D.P.(fld).velAll.Hist(ind_str:ind_end,:,2),1);
...
% -----------------------PROCESS NLX CELL DATA---------------------
    function TT_Proc()
        ...
        for z_tt = 1:length(tt_active_ind)
            ...
            % Get associated pos values
            cart_mat = D.P.Rat.posAll.Cart(1:D.P.Rat.posAll.indAll(2),:);
            pol_mat = D.P.Rat.posAll.Pol(1:D.P.Rat.posAll.indAll(2),:);
            vt_ts = D.P.Rat.posAll.TS(1:D.P.Rat.posAll.indAll(2));
            
            % Only search vt values in range
            exc_ind = vt_ts < min(ts_clust) | vt_ts > max(ts_clust);
            cart_mat(exc_ind, :) = [];
            pol_mat(exc_ind, :) = [];
            vt_ts(exc_ind) = [];
            ...
            kn_ind = knnsearch(vt_ts, ts_clust);
            clust_cart_mat = cart_mat(kn_ind,:);
            clust_pol_mat = pol_mat(kn_ind,:);
            ...
            for z_c = 1:length(clusts)
                ...
                % Get store start ind
                ind_str = D.TT.posClust{tt_ind, clust_ind}.indAll(2) + 1;
                ...
                if isempty(ind_str)
                    ...
                    ind_str = shft;
                    D.TT.posClust{tt_ind, clust_ind}.Cart(ind_str:end,:) = NaN;
                end
                ...
                % Update store inds
                D.TT.posClust{tt_ind, clust_ind}.indAll(2) = ind_end;
                D.TT.posClust{tt_ind, clust_ind}.indLap(2) = ind_end;
                
                % Update pos values
                D.TT.posClust{tt_ind, clust_ind}.Cart(ind_str:ind_end, 1) = clust_cart_mat(inc_ind,1);
                D.TT.posClust{tt_ind, clust_ind}.Cart(ind_str:ind_end, 2) = clust_cart_mat(inc_ind,2);
                D.TT.posClust{tt_ind, clust_ind}.Pol(ind_str:ind_end, 1) = clust_pol_mat(inc_ind,1);
                D.TT.posClust{tt_ind, clust_ind}.Pol(ind_str:ind_end, 2) = clust_pol_mat(inc_ind,1);
                D.TT.posClust{tt_ind, clust_ind}.TS(ind_str:ind_end) = ts_clust(inc_ind);
    ...
    function TT_Plot()
        ...
        for z_tt = 1:length(tt_plot)
            ...
            for z_c = 1:length(clust_plot)
                ...
                % Get data to plot
                ind_str = D.TT.posClust{tt_ind, clust_ind}.indLap(1);
                ind_end = D.TT.posClust{tt_ind, clust_ind}.indLap(2);
                ...
                % Pull out values
                x = D.TT.posClust{tt_ind, clust_ind}.Cart(ind_str:ind_end,1);
                y = D.TT.posClust{tt_ind, clust_ind}.Cart(ind_str:ind_end,2);
                rad = D.TT.posClust{tt_ind, clust_ind}.Pol(ind_str:ind_end,1);
                ...
                if get(D.UI.toggPlotTypeTT(2),'Value') == 1
                    ...
                    if D.PAR.sesTask == 'Track'
                        ...
                        rat_rad = ...
                            D.P.Rat.posAll.Pol(D.P.Rat.posAll.indAll(1):D.P.Rat.posAll.indAll(2), 1);
                    ...
                    rate_now(isnan(rate_now) | isinf(rate_now)) = ...
                        tt_hist(isnan(rate_now) | isinf(rate_now));
                    ...
                    if D.PAR.sesTask == 'Track'
                        ...
                        if ~isgraphics(D.UI.imgClustH(tt_ind, clust_ind))
                            D.UI.imgClustH(tt_ind, clust_ind) = ...
                                imagesc(cdat, ...
                                'AlphaData', alph, ...
                                'Parent', D.UI.ttClustAxNow(tt_ind, clust_ind));
                        else
                            ...
                            if D.UI.imgClustH(tt_ind, clust_ind).Parent ~= ...
                                    D.UI.ttClustAxNow(tt_ind, clust_ind)
                                Safe_Set(D.UI.imgClustH(tt_ind, clust_ind), ...
                                    'Parent', D.UI.ttClustAxNow(tt_ind, clust_ind))
                            end
                        end
...
% ------------------------------RAT SELECTION------------------------------
    function PopRat(~, ~, ~)
        ...
        if D.PAR.sesType == 'ICR_Session'
            ...
            Safe_Set(D.UI.popCond,'String',cond_list);
...
% ----------------------------CLEAR VT BUTTON------------------------------
    function BtnClrVT(~, ~, ~)
        ...
        D.P.Rat.posAll.indAll(1) = D.P.Rat.posAll.indAll(2);
        D.P.Rob.posAll.indAll(1) = D.P.Rob.posAll.indAll(2);
        D.P.Rat.posAll.indLap = D.P.Rat.posAll.indAll;
        D.P.Rob.posAll.indLap = D.P.Rob.posAll.indAll;
        
        % Reset vel data indeces
        D.P.Rat.velAll.indAll(1) = D.P.Rat.velAll.indAll(2);
        D.P.Rob.velAll.indAll(1) = D.P.Rob.velAll.indAll(2);
        D.P.Rat.velAll.indLap = D.P.Rat.velAll.indAll;
        D.P.Rob.velAll.indLap = D.P.Rob.velAll.indAll;
        
        % Reset vel hitogram data
        if D.P.Rat.velAll.indHist(2) > 0
            D.P.Rat.velAll.indHist(1) = D.P.Rat.velAll.indHist(2);
        end
        if D.P.Rob.velAll.indHist(2) > 0
            D.P.Rob.velAll.indHist(1) = D.P.Rob.velAll.indHist(2);
        end
...
% ----------------------------CLEAR VT BUTTON------------------------------
    function BtnClrTT(~, ~, ~)
        ...
        % Reset all plots
        Safe_Set(D.UI.mrkClustH, ...
            'XData', NaN, 'YData', NaN);
        Safe_Set(D.UI.imgClustH, ...
...
% -----------------------SELECT TT SOUND SOURCE--------------------------
    function ToggHearSourceTT(hObject, ~, ~)
        ...
        % Log/print
        Update_Log(sprintf('[%s] Set \"%s\" to %d', 'ToggHearSourceTT', btn_str, val));
...
% -----------------------SELECT TT CHANNEL TO HEAR--------------------------
    function ToggSubHearTT(hObject, ~, ~)
        ...
        ent_set = ' ';
        chan_set_arr = false(1,4); %#ok<PREALL>
        side_str = {'Left', 'Right'};
        ...
        % Keep chanels that are on for this tt
        chan_set_arr = is_on_mat(tt_ind,:);
        ...
        % Log/print
        Update_Log(sprintf('[%s] Set \"%s\" to %d', 'ToggSubHearTT', btn_str, val));
...
% ------------------------TOGGLE TYPE OF TT PLOT---------------------------
    function ToggPlotTypeTT(hObject, ~, ~)
        ...
        if D.UI.toggMainActionTT(2).Value == 0 || ...
            ...
            % Hide 1D patches
            Safe_Set(D.UI.ptchClustH, 'FaceAlpha', 0);
        end
        ...
        % Log/print
        Update_Log(sprintf('[%s] Set \"%s\" to %d', 'ToggPlotTypeTT', btn_str, val));
...
% -----------------------------TOGGLE PLOT TT------------------------------
    function ToggSubPlotTT(hObject, ~, ~)
        ...
        if val == 1
            ...
            % Take over new axes
            D.UI.ttClustAxNow(tt_ind, clust_ind) = D.UI.axClstH(ax_ind);
            ...
            set(D.UI.txtColBarH(ax_ind), ...
                ...
                'String', sprintf('%s-%d', D.TT.ttList{tt_ind}, clust_ind-1));
        ...
        if val == 0
            ...
            % Get index of axis
            ax_ind = find(D.UI.axClstH == D.UI.ttClustAxNow(tt_ind, clust_ind));
            ...
            if ~isempty(ax_ind)
                ...
                % Reset patches
                Safe_Set(D.UI.ptchClustH(ax_ind,:), 'FaceAlpha', 0);
                
                % Hide text
                set(D.UI.txtColBarH(ax_ind), 'Visible', 'off')
        ...
        % Log/print
        Update_Log(sprintf('[%s] Set \"%s\" to %d', 'ToggSubPlotTT', btn_str, val));
...
% ----------------------------ENABLE/DISABLE TT----------------------------
    function ToggIncludeTT(hObject, ~, ~)
        ...
        if val == 1
            ...
            Safe_Set(D.UI.toggSubPlotTT(tt_ind,:), ...
                'Value', 0, ...
                'Enable', 'off')
            Safe_Set(D.UI.toggSubFlagTT(tt_ind,:), ...
                'Value', 0, ...
                'Enable', 'off')
            Safe_Set(D.UI.toggSubHearSdTT(tt_ind,:), ...
                'Value', 0, ...
                'Enable', 'off')
            Safe_Set(D.UI.toggSubHearChTT(tt_ind,:,:), ...
                'Value', 0, ...
                'Enable', 'off')
        else
            ...
            Safe_Set(D.UI.toggSubFlagTT(tt_ind,:), 'Enable', 'on')
            Safe_Set(D.UI.toggSubHearSdTT(tt_ind,:), 'Enable', 'on')
        ...
        % Store value
        D.TT.ttInclude(tt_ind) = ~val;
        D.TT_IO.TT_Include{D.PAR.ratIndTT,:} = D.TT.ttInclude;
        
        % Update buttons
        Button_State(D.UI.toggIncludeTT(tt_ind), 'Update');
    ...
    function ToggSelectTT(hObject, ~, ~)
        ...
        if D.UI.toggMainActionTT(2).Value == 1
            ...
            for z_c = 1:length(stream_clusts)
                ...
                ToggSubPlotTT(D.UI.toggSubPlotTT(tt_ind,z_c))
            end
        ...
        if val == 1
            ...
            if isempty(strfind(D.TT.ttFldNow, 'R')) && ...
                    any([D.UI.toggHearSourceTT.Value] == 1)
                
                % Enable left audio for all but refs
                lft_ind = ismember(Safe_Get(D.UI.toggSubHearSdTT(1,:), 'String'), 'L');
                Safe_Set(D.UI.toggSubHearSdTT(tt_ind,lft_ind), 'Value', 1)
                
                % Run callback
                ToggSubHearTT(D.UI.toggSubHearSdTT(tt_ind,lft_ind));
        ...
        if val == 0
            ...
            Safe_Set(D.UI.toggSelectTT(~([D.UI.toggIncludeTT.Value])), 'Enable', 'on');
            ...
            lft_ind = ismember(Safe_Get(D.UI.toggSubHearSdTT(1,:), 'String'), 'L');
            if Safe_Get(D.UI.toggSubHearSdTT(tt_ind,lft_ind), 'Value') == 1
                
                % Enable left audio
                Safe_Set(D.UI.toggSubHearSdTT(tt_ind,lft_ind), 'Value', 0)
        ...
        % Disable all buttons
        Safe_Set(D.UI.toggSelectTT(~([D.UI.toggIncludeTT.Value])), 'Enable', 'off');
        ...
        % Set appearance based on update status
        if ~D.F.tt_updated(tt_ind)
...
% --------------------BUTTON PRESS TO SAVE TT ENTRIES----------------------
    function ToggUpdateLogTT(~, ~, ~)
        ...
        if val == 0
            ...
            Safe_Set(D.UI.toggSelectTT(tt_ind), 'BackgroundColor', D.UI.enabledCol)
            Safe_Set(D.UI.ttBndlLegMrk(tt_ind), 'MarkerFaceColor', D.UI.ttCol(tt_ind,:));
            Safe_Set(D.UI.ttDriveLegMrk(tt_ind), 'MarkerFaceColor', D.UI.ttCol(tt_ind,:));
        else
            ...
            Safe_Set(D.UI.txtPanTT(6:7), 'ForegroundColor', D.UI.enabledCol)
        ...
        tt_list = D.TT.ttList(D.I.ttBndl==D.I.ttBndl(tt_ind));
        for z_tt = 1:length(tt_list)
        ...
        Safe_Set(D.UI.txtPanTT(6), 'String', orientations_new);
        txt = sprintf('%0.0f %sm', depth_new, char(181));
        Safe_Set(D.UI.txtPanTT(7), 'String', txt);
        ...
        switch set_case
            ...
            case 'Sleep1_Objects'
                ...
                if strcmp(setting_str, 'Disable')
                    ...
                    Safe_Set(D.UI.editSleep(1), 'Enable', 'on')
            ...
            case 'TT_Turn_Panel_Objects'
                ...
                if strcmp(setting_str, 'Enable')
                    ...
                    Safe_Set(D.UI.popOr, 'String', D.PAR.dirVec);
    ...
    function Change_TT_Action(action_str, setting_str)
        ...
        if strcmp(action_str, 'Track TTs')
            ...
            if strcmp(setting_str, 'Disable')
                ...
                ToggSelectTT(D.UI.toggSelectTT(1));
                ...
                ToggHearSourceTT(D.UI.toggHearSourceTT(1));
        ...
        if strcmp(action_str, 'Plot Spikes')
            ...
            if strcmp(setting_str, 'Enable')
                ...
                if D.PAR.sesType ~= 'TT_Turn'
                    Safe_Set(D.UI.ttMask, 'Visible', 'on')
                end
...
% ---------------------------PRINT TO CONSOLE------------------------------
    function Console_Write(str, t_now, is_err)
        ...
        if ~D.F.poll_nlx || D.F.task_done_confirmed
            Update_UI(0);
        else
            Update_UI(100);
        end
