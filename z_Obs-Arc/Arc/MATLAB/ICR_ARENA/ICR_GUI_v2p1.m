...
global cmdMatOut; % message out to C# EXE
global cmdMatIn; % message in from C# EXE
global datMatOut; % data out to C# EXE
%% =========================== MAIN SCRIPT ================================
...
% ---------------------------ARDUINO SETUP---------------------------------
    ...
    function[D] = SF_Arduino_Setup(D)
        ...
        %         % Create serial port object using Arduino Due (Programing Port)
        %         D.ARD.serObj = serial('COM14','BaudRate',9600);
        %
...
% -------------------------------UI SETUP-----------------------------------
    ...
    function[D] = SF_UI_Setup(D)
        ...
        text1_font_sz = ...
            [10, 13/(D.UI.fg_pos(4)*D.UI.rec_pan_pos(4))*1.15] ;
        ...
        %set(D.UI.figH,'WindowStyle','docked');
        %set(D.UI.figH,'Position',D.UI.fg_pos);
...
% ------------------------------NLX SETUP----------------------------------
    ...
    function[D] = SF_NLX_Setup(D)
        ...
        % Tell C# were connected
        cmdMatOut = 'matlabConnected';
...
% -------------------------------AC SETUP----------------------------------
    ...
    function [D] = SF_AC_Setup(D)
        ...
        % Create tcpip object
        tcpIP = tcpip(D.AC.IP,30000,'NetworkRole','Server');
        ...
        % Post to AC computer
        fwrite(D.AC.tcpIP,D.AC.data,'int8');
...
% -----------------------------DISCONECT ALL-------------------------------
    function [shouldExit] = Disconect_All(D)
