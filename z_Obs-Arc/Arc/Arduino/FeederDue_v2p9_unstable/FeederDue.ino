...
public:
	...
	void UpdatePID()
	{
		// Check if dampening in progress
		CheckDamp();
	...
	void CheckDamp()
	{
		...
		if (ekfReady)
		{
			...
			else if (isDampened)
			{
				...
				delta_set = deltaSetpoint - deltaSetpoint*(t_dampDur / 3000);

				// Check that max time has not ellapsed
				if (t_dampDur <= t_dampTill &&
					delta_set > 0)
				{
					if (millis() > next_update)
					{
						ChangeSetpoint(delta_set);
						next_update = millis() + 40;
					}
				}
				else
				{
