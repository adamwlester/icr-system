...
%% ==========================MAIN FUNCTION==================================
    ...
    function[] = RunScript()
        ...
        function [] = SF_Var_Setup()
            ...
            % REWARD VARS
            D.I.targ = 5; % default max
            D.PAR.rewDurList = [250, 500, 1000, 1500,  2000];
            D.PAR.targRewDur = [250, 500, 1000, 1500,  2000, 500, 250];
            D.PAR.rewDur = D.PAR.targRewDur(D.I.targ);
            D.PAR.targLocs = 20:-5:-10;
            D.I.targArr = NaN(1,100);
            D.C.targ = zeros(1,length(D.PAR.targLocs));
        ...
        function[] = SF_UI_Setup()
            ...
            % CREATE AXES FOR TARG DIST PLOT
            wdth = 0.1;
            ht = 0.1;
            lft = (plot_bnds(1)+plot_bnds(3)) - 0.01 - wdth;
            botm = 0.045;
            targ_ax_pos = [...
                lft, ...
                botm, ...
            ...
            botm = botm - text_font_sz(2)*2;
            pos = [pos_lft_dflt, botm, ...
                pos_wd_dflt, text_font_sz(2)*1.75];
        ...
        function[] = SF_Targ_Dist_Setup()
            ...
            % Setup axes
            set(D.UI.axTargH(1), ...
                'XLim',[min(x_long)+sub_samp/2,max(x_long)-sub_samp/2]);
            set(D.UI.axTargH(2), ...
            ...
            % Get long dist
            % front (i.e., < 0)
            han_front = sub_samp*9;
            df = hanning(han_front);
            df = df(1:han_front/2);
            df = (df/max(df)) * (max(df)/5)*4 + max(df)/5;
            df = [min(df)*ones((length(x_long)-han_front)/2, 1); df];
            % back (i.e., > 0)
            han_back = sub_samp*6;
            db = hanning(han_back);
            db = db(round(han_back/2):end);
            db = db*0.6;
            db = [db; zeros(length(df)-length(db),1)];
            dist_long = [df;db]';
            % normalize
            ...
            y = dist_long;
            y(ceil(length(dist_long)/2)) = NaN;
            plot(x_long, y,'k', ...
                'LineWidth', 2, ...
                'Parent',D.UI.axTargH(1));
            % plot center line
            plot([length(dist_long)/2, length(dist_long)/2], [0, max(D.PAR.rewDurList)], 'k', ...
                'LineWidth', 2, ...
                'Color', [0, 0, 0], ...
                'Parent',D.UI.axTargH(1));
            ...
            x_short = x_long(short_ind);
            
            % Plot point for prob at each target
            plot(x_short, dist_short, 'or', ...
                'MarkerFaceColor', [0.75, 0, 0], ...
                'MarkerEdgeColor', [0.5, 0.5, 0.5], ...
                'MarkerSize', 5, ...
                'Parent',D.UI.axTargH(1));
            % Plot point for actual reward value
            x = x_short(ismember(targ_short, -1*D.PAR.targLocs));
            y = D.PAR.targRewDur / max(D.PAR.targRewDur) *  max(dist_short);
            plot(x, y, 'or', ...
            ...
            % set axis
            x_tic_labs = arrayfun(@(x,y) ([sprintf('%d', x), sprintf('(%0.0f%%)', y)]), ...
                targ_short, dist_short*100, 'uni', false);
            set(D.UI.axTargH(2), ...
                'YLim' , [0, 1], ...
                'YTick',linspace(0,1,length(D.PAR.rewDurList)+1), ...
                'YTickLabel',[0,D.PAR.rewDurList], ...
                'XTickLabel' , x_tic_labs);
            grid(D.UI.axTargH(2),'on');
...
%             % rerun in less 0 vals then others
%             while ...
%                     any(samp_hist(targ_interp == 0) <= samp_hist(targ_interp ~= 0)) || ...
%                     any(samp_hist(targ_interp == 5) > samp_hist(targ_interp > -15 & targ_interp <= 0))
%                 % get new sample
%                 rand_dist = round(interp1(cum, targ_interp, sort(rand(1,100))));
%                 samp_hist = histc(rand_dist, bins);
%                 samp_hist = samp_hist(1:end-1);
...
%             % make sure first rew is at 0
%             while targ_arr(1) ~= 0
%                 targ_arr = targ_arr(randperm(length(targ_arr)));
%             end
            % remove unused values
            targ_arr(targ_arr < -20 | targ_arr > 10) = 0;
            ...
            set(D.UI.axTargH(1), ...
                ...
                'XAxisLocation','top', ...
                'YTickLabels', [], ...
                'XTick', x_short(ismember(targ_short, -1*D.PAR.targLocs)), ...
                'XTickLabels', D.C.targ);
        ...
        function [] = SF_Finish_Setup()
            ...
            D.PAR.haltError = 5;
            D.PAR.feedSet = [...
                D.PAR.trigDist - (10 - D.PAR.haltError), ...
                D.PAR.trigDist + D.PAR.haltError];
            ...
            set(D.UI.ptchFdH(:,1), 'FaceAlpha', 0.15);
            ...
            set(D.UI.fdH(1,1), 'MarkerSize', 15)
            uistack(D.UI.fdH(1,:), 'bottom');
            ...
            D.UI.cueH = gobjects(length(D.PAR.targLocs),2);
            for z_targ = 1:length(D.PAR.targLocs)
                % Plot cue distance lines
                for z_fd = 1:2
                    % marker
                    D.UI.cueH(z_targ,z_fd) = ...
                        plot(D.UI.fd_x(D.UI.rewFeed(z_fd)), ...
                        D.UI.fd_y(D.UI.rewFeed(z_fd)), 'o', ...
            ...
            % move down
            uistack(reshape(D.UI.cueH,1,[]),'bottom');
        ...
        function [] = SF_VT_Proc(fld)
            ...
            if vtNRecs > 0
                ...
                if D.B.is_rewarding
                    ...
                    % Check that flag has not been set for too long
                    if etime(clock, D.T.rew_sent) > 5
                        D.B.is_rewarding = false;
                    end
                else
                ...
                if etime(clock, D.T.(fld).last_pos_update) > (D.PAR.rewDur+100)/1000 
                ...
                if strcmp(fld, 'Rob') && ~D.B.rat_in
                    ...
                    % plot break in big jumps
                    if abs(D.P.(fld).velRad - D.P.Rat.radLast) > 0.2
            ...
            if evtNRecs > 0
                ...
                if any(ismember(evtStr, D.NLX.rew_off_str))
                    ...
                    if datenum(D.T.rew_start) > 0 && ...
                        ...
                        D.T.rew_duration(1) = etime(D.T.rew_end, D.T.rew_start) * 1000;
                    ...
                    if any(check_inbound)
                        ...
                        if ~D.B.flag_rew_reset
                            ...
                            if D.B.is_cued_rew
                                ...
                                set(D.UI.cueH(D.I.targ,D.I.rot), 'Visible', 'on');
        ...
        function [] = SF_Reward_Feeder_Check()
            ...
            if D.PAR.sesCond ~= 'Manual_Training'
                ...
                if any(~isnan(D.P.Rat.rad))
                    ...
                    D.B.check_inbound_rew = ...
                        Check_Rad_Bnds(D.P.Rat.rad, D.UI.rewBnds(D.I.targ,:,D.I.rot));
                    ...
                    if any(D.B.check_inbound_rew)
                        ...
                        if D.B.flag_rew_reset
                            ...
                            if inbndTim >= D.PAR.rewDel || ...
                                ...
                                D.C.targ(D.I.targ) = D.C.targ(D.I.targ)+1;
                                x = -1*D.PAR.targLocs;
                                y = D.C.targ / sum(D.C.targ);
                                y = y/max(y);
                                ...
                                % Display count
                                set(D.UI.axTargH(1), 'XTickLabels', D.C.targ)
                                
                                % Plot average targ pos
                                delete(D.UI.targAvgH);
                                avg_trig = D.PAR.targLocs*D.C.targ' / sum(D.C.targ);
                                [xbnd, ybnd] =  ...
                                ...
                                D.UI.rewInfoList = [...
                                    ...
                                    D.PAR.targRewDur(D.I.targ)) ...
                                    }];
                                infstr = [...
                                    sprintf('Reward Info (%0.2f)', avg_trig*-1); ...
                                    D.UI.rewInfoList];
        ...
        function [] = BlockCue(~, ~, ~)
            if (get(D.UI.btnBlockCue, 'Value') == 1)
                ...
                set(D.UI.cueH(D.I.targ,D.I.rot), 'Visible', 'off');
