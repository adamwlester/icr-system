...
% Air paramiters
D.PAR.pumpDel = 2000; % duration before turning pump back on after feeder bigins (ms)
...
% Note:
%   Feeder solenoids are normally close
...
% SETUP DONE
    function [] = BtnStupDn(~, ~, ~)
        ...
        D.UI.ozcuBnds(any(diff(D.UI.ozcuBnds,1,2)>0,3),1,:) = ...
            D.UI.ozcuBnds(any(diff(D.UI.ozcuBnds,1,2)>0,3),2,:) + 10;
        % convert to radians
        ...
        % Plot ozone and cue trigger bounds
        % set alpha to 0 (transparent)
        D.UI.ptchOzCuTrg = gobjects(size(D.UI.ozcuBnds,1),2);
        for z_fd = 1:2
            for z_bnd = 1:size(D.UI.ozcuBnds,1)
                [xbnd, ybnd] =  ...
                    Get_Trk_Bnds(D, D.UI.ozcuBnds(z_bnd,:,z_fd));
                D.UI.ptchOzCuTrg(z_bnd,z_fd) = ...
                    patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                    [ybnd(1,:),fliplr(ybnd(2,:))], ...
                    'none', ...
                    'FaceAlpha', 0, ...
...
% RECORDING DONE
    function [] = BtnRecDn(~, ~, ~)
        ...
        % Print end time
        tim = ...
            datestr(datevec(datenum(D.T.ses_str_tim)+(D.T.ses_end_tim/60/60/24)), 'HH:MM:SS');
        infstr = sprintf( ...
            'Stop_:%s%s\n', ...
            repmat('_',1,6), tim);
        set(D.UI.txtTimEndInf, 'String', infstr)
...
% FEEDER PULSE
    function [] = BtnPlsFd(~, ~, ~)
        ...
        % Activate timers
        % start feeder reward duration timer
        start(D.UI.timFdRewDur)
        % start reward tone duration timer
        if D.UI.sndair(2)
            start(D.UI.timToneRewDur)
        end
        if D.UI.sndair(4)
            % start pump delay timer
            start(D.UI.timPumpDel)
            % start vac delay timer
...
% FEEDER REWARD TIMER STOP
    function [] = StopFeed(~, ~, ~)
        ...
        NlxSendCommand(D.NLX.port_out_update_cmd);
        % print solenoid close time
        fprintf('F%d_CLOSE:%s%3.0f_ms\n', ...
            D.I.feed_ind(D.I.rot), ...
            repmat('_',1,8), ...
            etime(clock,D.T.rew_tim)*1000);
...
% REWARD TONE TIMER STOP
    function [] = StopTone(~, ~, ~)
        ...
        % set white noise bit on
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(1)) = ...
            num2str(D.UI.sndair(1));
        % set reward tone bit off
        D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(2)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
        % print reward tone off time
        fprintf('Reward_Tone_OFF:%s%3.0f_ms\n', ...
            repmat('_',1,1), ...
            etime(clock,D.T.rew_tim)*1000);
...
% OZONE TIMER STOP
    function [] = StopOz(~, ~, ~)
        ...
        NlxSendCommand(D.NLX.port_out_update_cmd);
        % print ozone time
        fprintf('Ozone_OFF:%s%3.0f_ms\n', ...
            repmat('_',1,7), ...
            etime(clock,D.T.oz_tim)*1000);
        
        % Post NLX event: ozone off
        NlxSendCommand(D.NLX.oz_cmd{2});
...
% VAC TIMER START
    function [] = StartVac(~, ~, ~)
        ...
        D.T.vac_tim = clock;
        % print vac on time
        fprintf('Vacuum_ON:%s%3.0f_ms\n', ...
            repmat('_',1,7), ...
            etime(clock,D.T.rew_tim)*1000);
        
        % Start vac duration timer
        start(D.UI.timVacDur)
...
% VAC TIMER STOP
    function [] = StopVac(~, ~, ~)
        ...
        NlxSendCommand(D.NLX.port_out_update_cmd);
        % print vac off time
        fprintf('Vacuum_OFF:%s%3.0f(%3.0f)_ms\n', ...
            repmat('_',1,6), ...
            etime(clock,D.T.rew_tim)*1000, ...
            etime(clock,D.T.vac_tim)*1000);
...
if D.B.rot_check
    ...
    if any(D.B.track_rot)
        ...
        % Set all ozone and cue patches to transparent
        set(D.UI.ptchOzCuTrg, ...
            'FaceColor', 'none', ...
            'FaceAlpha', 0, ...
            'EdgeAlpha', 0)
...
if ...
    ...
    if ...
        ...
        if ...
            ...
            % Get new bounds
            D.I.cue_bnd_ind = ceil(rand(1,1)*size(D.UI.ozcuBnds,1));
            D.UI.cueBnd = ...
                D.UI.ozcuBnds(D.I.cue_bnd_ind, :, D.I.rot);
            
            % Set new bound patch to visible
            set(D.UI.ptchOzCuTrg(D.I.cue_bnd_ind, D.I.rot), ...
        ...
        if D.UI.sndair(5)
            ...
            % Get new bounds
            D.I.oz_bnd_ind = ceil(rand(1,1)*size(D.UI.ozcuBnds,1));
            D.UI.ozBnd = ...
                D.UI.ozcuBnds(D.I.oz_bnd_ind, :, D.I.rot);
            
            % Set new bound patch to visible
            set(D.UI.ptchOzCuTrg(D.I.oz_bnd_ind, D.I.rot), ...
                'FaceColor', D.UI.ozTrgCol, ...
                'FaceAlpha', 0.1, ...
                'EdgeAlpha', 1)
...
D.SS_Out_ICR.(D.PAR.ratLab).Start_Time{end} = datestr(D.T.ses_str_tim, 'HH:MM:SS');
D.SS_Out_ICR.(D.PAR.ratLab).Total_Time(end) = etime(D.T.ses_end_tim ,D.T.ses_str_tim) / 60;
D.SS_Out_ICR.(D.PAR.ratLab).ICRb(end) = true;
D.SS_Out_ICR.(D.PAR.ratLab).ICRi(end) = false;
D.SS_Out_ICR.(D.PAR.ratLab).Track(end) = true;
D.SS_Out_ICR.(D.PAR.ratLab).Forage(end) = false;
D.SS_Out_ICR.(D.PAR.ratLab).Rotation(end) = D.B.rot_ses;
D.SS_Out_ICR.(D.PAR.ratLab).Session_ICRb_T(end) = D.PAR.sesNum;
D.SS_Out_ICR.(D.PAR.ratLab).Session_ICRb_F(end) = NaN;
D.SS_Out_ICR.(D.PAR.ratLab).Session_ICRi_T(end) = NaN;
D.SS_Out_ICR.(D.PAR.ratLab).Session_ICRi_F(end) = NaN;
...
if D.B.rot_ses
    D.SS_Out_ICR.(D.PAR.ratLab).Session_Rotation(end) = D.PAR.sesNumICR;
    ...
    D.SS_Out_ICR.(D.PAR.ratLab).Session_Rotation(end) = NaN;
...
save(fullfile(D.DIR.ioTop, D.DIR.ssSD, 'SS_Out_ICR_temp.mat'), 'SS_Out_ICR');
SS_In_All = D.SS_In_All;
save(fullfile(D.DIR.ioTop, D.DIR.ssSD, 'SS_In_All_temp.mat'), 'SS_In_All');
...
if D.DB.run
    ...
    if arg == 3
        ...
        D.SS_In_All.Session_Rotation(D.PAR.ratInd) = ...
            D.SS_Out_ICR.(['r',D.DB.rat]).Session_Rotation(oldSesInd);
        % start quadrant
        D.SS_In_All.Start_Quadrant{D.PAR.ratInd}(D.PAR.sesNum) = ...
