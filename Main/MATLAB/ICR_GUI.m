    ...
    function[] = NLX_Start()
        ...
        if ~D.F.cheetah_running && D.PAR.sesType ~= 'Table_Update'
            ...
            if strcmp(choice, 'Yes') && ~DOEXIT
                ...
                % NLX setup config
                if D.F.implant_session
                    start_cfg_fi = 'ICR_Cheetah_Load_Ephys.cfg';
                else
                    start_cfg_fi = 'ICR_Cheetah_Load_Behavior.cfg';
                end
                ...
                system(fullfile('Cheetah.exe C:\Users\Public\Documents\Cheetah\Configuration\', [start_cfg_fi,'&']));
                
                % Log/print
                Console_Write(sprintf('[NLX_Start] FINISHED: Run Cheetah.exe "%s"', start_cfg_fi));
...
% --------------------------TT TRACKING SETUP----------------------
    function[was_ran] = TT_Track_Setup()
        ...
        % Update list
        D.TT.ttList = [[tt_list{1}], [tt_list{2}]];
        ...
        for z_tt = 1:length(D.TT.ttList)
            D.TT.ttLogNew.([D.TT.ttList{z_tt},'_N']){:} = '';
        end
...
%         end
        ...
        % Get cluster color
        clust_col = [[0.25,0.25,0.25]; hsv(D.PAR.maxClust-1)];
    ...
    function Togg_UpdateLogTT(~, ~, ~)
        ...
        if val == 1
            ...
            if D.TT.ttLogNew.([D.TT.ttFldNow,'_I'])(5) ~= imp_num_new(5) && ...
                ...
                for z_tt = 1:length(D.TT.ttList)
                    D.TT.ttLogNew.([D.TT.ttList{z_tt},'_I'])(5) = imp_num_new(5);
                end
            end
    ...
    function Togg_SubHearTT(hObject, ~, ~)
        ...
        if ~strcmp(ent_set, 'None')
            ...
            if strcmp(source_str, 'CS')
                ent_set =  sprintf('CSC-%s_01', D.TT.ttList{tt_ind}(3:end));
            end
