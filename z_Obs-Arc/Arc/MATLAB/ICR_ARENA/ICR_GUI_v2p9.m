...
try
    ...
    % Should have been run, but run once more
    Disconect_All();
end
...
%% =========================== MAIN SCRIPT ================================
    ...
    function[] = RunScript()
        ...
        Update_Console(sprintf('\rRUNNING ICR_BEHAVIOR.m\rTime: %s\r', ...
            datestr(now, 'HH:MM:SS')));
        ...
        while ~shouldExit
            ...
            if D.B.poll_nlx
                ...
                if etime(clock, D.C.moveDel) > 2.5 &&  ~D.C.doneMove
                    ...
                    if ~D.B.manual_ses
                        CS_Send('M', D.UI.strQuadBnds(1));
                    else
                        CS_Send('M', D.UI.rewBnds(D.I.rot,1));
                    end
                    D.C.doneMove = true;
                end
                ...
                if ~D.B.rat_in && D.B.rec && ~D.B.rec_done
                    SF_Rat_In_Check();
                end
...
% ------------------------------VAR SETUP----------------------------------
    ...
    function [] = SF_Var_Setup()
        ...
        % PID setPoint
        D.PAR.setPoint = 46 * ((2 * pi)/(140 * pi));
        % Robot butt dist
        D.PAR.buttDist = 18 * ((2 * pi)/(140 * pi));
        % Feeder dist from rob tracker
        D.PAR.feedDist = 68 * ((2 * pi)/(140 * pi));
        % pos now
        D.P.Rat.x = NaN;
        D.P.Rat.y = NaN;
        D.P.Rob.x = NaN;
        ...
        D.UI.ratPltHvel = NaN(1,60*120/D.PAR.polRate); % rat
        D.UI.robPltHvel = NaN(1,60*120/D.PAR.polRate); % rob
        ...
        % bulldoze
        D.UI.bullList = {'0 sec'; '10 sec'; '30 sec'; '60 sec'; '120 sec'};
        D.PAR.bullDel = 120;
        D.PAR.bullSpeed = 5;
...
% ------------------------------NLX SETUP----------------------------------
    ...
    function[] = SF_NLX_Setup()
        ...
        if NlxAreWeConnected() 
            ...
            NlxSendCommand(['-SetDigitalIOportString ', D.NLX.DevName, ' ', D.NLX.port_0, ' "00000000"']);
            
            % Set TTL Event strings
            % audio channels
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_1, ' ', num2str(D.NLX.snd_rt_bit), ' ', D.NLX.snd_air_evt_lab{1}]);
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_1, ' ', num2str(D.NLX.snd_lft_bit), ' ', D.NLX.snd_air_evt_lab{2}]);
            % photo transdicers
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_0, ' ', D.NLX.north_bit, ' TTL_North_On']);
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_0, ' ', D.NLX.west_bit, ' TTL_West_On']);
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_0, ' ', D.NLX.south_bit, ' TTL_South_On']);
            NlxSendCommand(['-SetNamedTTLEvent ', D.NLX.DevName, ' ', D.NLX.port_0, ' ', D.NLX.east_bit, ' TTL_East_On']);
...
% ----------------------------RAT IN CHECK---------------------------------
    ...
    function [] = SF_Rat_In_Check()
        ...
        if all(~isnan(D.P.Rat.rad))
            ...
            if any(D.B.track_strqd)
                
                % Get first in-bound ts
                if D.T.strqd_inbnd_t1 == 0
                    D.T.strqd_inbnd_t1 = D.P.Rat.ts(find(D.B.track_strqd, 1, 'first'));
                end
                
                % Get most recent in-bounds ts
                D.T.strqd_inbnd_t2 = D.P.Rat.ts(find(D.B.track_strqd, 1, 'last'));
...
% ------------------------------PROCESS VT---------------------------------
    ...
    function [] = SF_VT_Proc(fld)
        ...
        excInd = roh > D.P.posRohMax | roh < D.P.posRohMin;
        % Exclude valuse based on diff
        rad_diff = [0;diff(rad)];
        excInd = excInd | (rad_diff > (150/33)*((2 * pi)/(140 * pi)));
        ...
        % Store last usable rad value
        if ~isempty(D.P.(fld).rad)
            D.P.(fld).radLast = D.P.(fld).rad(end);
        end
        ...
        if isempty(vel) || isempty(D.P.(fld).velRad)
            ...
            % convert to car
            [D.P.(fld).vel_pol(1), D.P.(fld).vel_pol(2)] = ...
                pol2cart(D.P.(fld).velRad, velRoh);
            D.P.(fld).vel_pol(1) =  D.P.(fld).vel_pol(1).*D.PAR.R + D.PAR.XC;
            D.P.(fld).vel_pol(2) =  D.P.(fld).vel_pol(2).*D.PAR.R + D.PAR.YC;
...
% -------------------------REWARD FEEDER CHECK-----------------------------
    ...
    function [] = SF_Reward_Feeder_Check()
        ...
        if all(D.B.track_fd_reset) && all(~isnan(D.P.Rat.rad))
            ...
            if any(D.B.track_fd) 
                ...
                if D.T.fd_inbnd_t1 == 0
                    D.T.fd_inbnd_t1 = D.P.Rat.ts(find(D.B.track_fd, 1, 'first'));
                end
                
                % Get most recent in-bounds ts
                D.T.fd_inbnd_t2 = D.P.Rat.ts(find(D.B.track_fd, 1, 'last'));
...
% ----------------------------LAP CHECK------------------------------------
    ...
    function [] = SF_Lap_Check()
        ...
        if all(D.B.track_lap);
            ...
            % Set existing pos plot data to lighter color
            set(D.UI.ratPltH(~isnan(D.UI.ratPltH)), 'MarkerEdgeColor', [0.75,0.75,0.75]);
            set(D.UI.ratPltHvel(~isnan(D.UI.ratPltHvel)), 'Color', D.UI.ratHistCol, 'LineWidth', 1);
            set(D.UI.robPltHvel(~isnan(D.UI.robPltHvel)), 'Color', D.UI.robHistCol, 'LineWidth', 1);
...
% ------------------------REWARD RESET CHECK-------------------------------
    ...
    function [] = SF_Reward_Reset_Check()
        ...
        if ...
            ...
            CS_Send('R', D.UI.rewBnds(D.I.rot,1));
            D.B.cue_sent_to_cs = true;
            
            % Set cue maker to robot color indicate cueing
            set(D.UI.fdH(D.I.rot), ...
        ...
        if any(track_quad)
            ...
            D.B.track_fd_reset(cur_quad) = true;
            
            % Set later quads to false to prevent backwards runs
            D.B.track_fd_reset(1:3 > cur_quad) = false;
            
            % Hide patches of passed quadrants
            set(D.UI.ptchFdRstH(D.B.track_fd_reset, D.I.rot), ...
                'FaceAlpha', 0, ...
                'EdgeAlpha', 0);
...
% --------------------------PLOT POSITION----------------------------------
    ...
    function [] = SF_Pos_Plot()
        ...
        % RAT
        Hind = find(isnan(D.UI.ratPltHvel), 1, 'first');
        if D.B.Rat.plot_vel
            D.UI.ratPltHvel(Hind) = ...
                plot(D.P.Rat.vel_pol(:,1), D.P.Rat.vel_pol(:,2), '-', ...
        ...
        % ROB
        Hind = find(isnan(D.UI.robPltHvel), 1, 'first');
        if D.B.Rob.plot_vel
            D.UI.robPltHvel(Hind) = ...
                plot(D.P.Rob.vel_pol(:,1), D.P.Rob.vel_pol(:,2), '-', ...
                'Color', D.UI.robNowCol, ...
                'LineWidth', 2, ...
                'Parent', D.UI.axH(1));
        end
...
% CUE CONDITION
    function [] = ToggCue(~, ~, ~)
        ...
        if get(D.UI.toggCue(cue_cond), 'Value') == 1
            ...
            set(D.UI.toggCue(find([1,2,3] ~= cue_cond)), ...
                'BackgroundColor', D.UI.enabledCol, ...
                'Value',0);
            
            % Save data
            D.UI.cueFeed(:) = cue_str;
        end
    end
...
% BULLDOZE RAT
    function [] = Bulldoze(~, ~, ~)
        if (get(D.UI.btnBulldoze, 'Value') == 1)
            ...
            % Tell CS to bulldoze
            CS_Send('B', D.PAR.bullDel, D.PAR.bullSpeed);
...
% CLEAR VT BUTTON
    function [] = BtnClrVT(~, ~, ~)
        ...
        delete(D.UI.ratPltH(~isnan(D.UI.ratPltH)))
        delete(D.UI.ratPltHvel(~isnan(D.UI.ratPltHvel)))
        delete(D.UI.robPltHvel(~isnan(D.UI.robPltHvel)))
        % Reinitialize handle array back to NAN
        D.UI.ratPltH = NaN(1,60*120/D.PAR.polRate); % VT plot handle array
        D.UI.ratPltHvel = NaN(1,60*120/D.PAR.polRate); % VT plot handles for rat velocity
        D.UI.robPltHvel = NaN(1,60*120/D.PAR.polRate); % VT plot handles for rob velocity
    end
...
% --------------------------SEND CS COMMAND--------------------------------
    ...
    function[] = CS_Send(id, data1, data2)
        ...
        Update_Console(sprintf('\rCS_Send\rm2c_id = %s\rm2c_dat1 = %2.2f\rm2c_dat2 = %2.2f\rTime: %s\r', ...
