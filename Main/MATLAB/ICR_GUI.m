    ...
    function[] = NLX_Start()
        ...
        if ~D.F.cheetah_running && D.PAR.sesType ~= 'Table_Update'
            ...
            if strcmp(choice, 'Yes') && ~DOEXIT
                ...
                % Log/print
                Console_Write(sprintf('[NLX_Start] FINISHED: Run Cheetah.exe "%s"', top_cfg_fi));
    ...
    function Togg_SubHearTT(hObject, ~, ~)
        ...
        if ~strcmp(ent_set, 'None')
            ...
            for z_chn = chan_changed
                ...
                % Send command
                Send_NLX_Cmd(['-SetAudioStreamEnabled ', msg_str]);
...
% ----------------------------ENABLE/DISABLE TT----------------------------
    function Togg_IncludeTT(hObject, ~, ~)
...
% % TEMP
% % --------------------------RUN NLX FUNCTION-------------------------------
%     function[pass, arg_out] = Run_NLX_Fun(fun_str, do_print)
%         
%         % Handle input args
%         if nargin == 1
%             do_print = true;
