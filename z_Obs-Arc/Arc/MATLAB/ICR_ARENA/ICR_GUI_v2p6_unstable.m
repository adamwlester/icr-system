        ...
        while ~shouldExit
            ...
            if D.B.poll_nlx
                ...
                if etime(clock, D.C.moveDel) > 2.5 &&  ~D.C.doneMove
                    ...
                    if ~D.B.manual_ses
                        CS_Send('M', mean(D.UI.strQuadBnds));
                    else
                        CS_Send('M', mean(D.UI.rewBnds(D.I.rot,:)));
                    end
                    D.C.doneMove = true;
                end
...
% ------------------------------VAR SETUP----------------------------------
    ...
    function [] = SF_Var_Setup()
        ...
        clear S;
        % Robot guard dist
        D.PAR.guardDist = 0.0629;
        % PID setPoint
        D.PAR.setPoint = 0.81;
        % Robot butt dist
        D.PAR.buttDist =0.2571;
        ...
        % track reversals
        D.C.rev_cnt = 0;
        % feeder bounds sample counter
        D.C.rew_inbnd_cnt = 0;
        % feeder bounds record counter
        D.C.rew_rec_cnt = 0;
        % start quad bounds sample counter
        D.C.strqd_inbnd_cnt = 0;
        % start quad bounds record counter
        D.C.strqd_rec_cnt = 0;
        ...
        D.P.vel_nsmp = 10;
        % store velocity data
        D.P.Rat.vel_samples = NaN(D.P.vel_nsmp,1);
        D.P.Rob.vel_samples = NaN(D.P.vel_nsmp,1);
        D.P.Rat.vel_smp_ind = 0;
        D.P.Rob.vel_smp_ind = 0;
...
% -------------------------------UI SETUP-----------------------------------
    ...
    function[] = SF_UI_Setup()
        ...
        D.UI.dfltBtnActiveCol = [0.75, 0, 0];
        ...
        D.UI.ratPltH = NaN(1,60*120/D.PAR.polRate);
        % vt plot handles of velocity
        D.UI.ratPltHvel = NaN(1,60*120/D.PAR.polRate);
        D.UI.robPltHvel = NaN(1,60*120/D.PAR.polRate);
        % bool for tracking VT plot handles of reward delivery points
        D.UI.ratPltHrew = false(1,60*120/D.PAR.polRate);
        ...
        % bulldoze
        D.UI.bullList = {'0 sec'; '30 sec'; '60 sec'; '120 sec'};
        D.PAR.bullDel = 120;
    ...
    function [] = SF_Finish_Setup()
        ...
        if ~shouldExit
            ...
            set(D.UI.popBulldoze, ...
                'Value', 4);
            % bulldoze with longest delay by default
            Bulldoze();
            % reward
            set(D.UI.btnReward, ...
                'Enable', 'On', ...
...
% ----------------------------RAT IN CHECK---------------------------------
    ...
    function [] = SF_Rat_In_Check()
        ...
        if ...
                any(D.B.track_strqd) && ...
                D.C.strqd_rec_cnt < 30*(D.PAR.strQdDel*1.25) && ...
                ~D.B.ratIn &&  ...
                D.B.rec;
            
            % Get count of in-bounds samples
            D.C.strqd_inbnd_cnt = D.C.strqd_inbnd_cnt + sum(D.B.track_strqd);
            
            % Add to record counter
            D.C.strqd_rec_cnt = D.C.strqd_rec_cnt + D.P.Rat.recs;
            
            % Check if rat has been in for the min delay period
            if ...
                    D.C.strqd_rec_cnt >= 30*D.PAR.strQdDel && ...
                    D.C.strqd_inbnd_cnt >= 30*D.PAR.strQdDel
...
% ------------------------------PROCESS VT---------------------------------
    ...
    function [] = SF_VT_Proc(fldStr)
        ...
        % Make sure something is stored in vars
        if vtNRecs == 0
            D.P.(fldStr).xy_pos = [NaN, NaN];
            D.P.(fldStr).hd_deg = NaN;
        end
        ...
        if (sum(~isnan(D.P.(fldStr).rad)) > 0)
            % Compute sample velocities
            rad_diff = min(2*pi - abs(diff(D.P.(fldStr).rad(~isnan(D.P.(fldStr).rad)))), ...
                abs(diff(D.P.(fldStr).rad(~isnan(D.P.(fldStr).rad)))));
            dt = double(diff(D.P.(fldStr).ts(~isnan(D.P.(fldStr).ts)))) / 10^6;
            ...
            if nDat == D.P.vel_nsmp
                D.P.(fldStr).vel_samples = vel;
            else
                old_dat = D.P.(fldStr).vel_samples;
                D.P.(fldStr).vel_samples(end-nDat+1:end) = vel;
                D.P.(fldStr).vel_samples(1:end-nDat) = ...
                    old_dat(nDat+1:end);
            end
        end
        
        % Save average
        if (sum(~isnan(D.P.(fldStr).vel_samples)) > 0)
            D.P.(fldStr).vel = nanmean(D.P.(fldStr).vel_samples);
        else
            D.P.(fldStr).vel = NaN;
        end
        
        % Map values to circle
        ampMin = (D.UI.arnRad-D.UI.trkWdt)*0.7;
        ampMax = (D.UI.arnRad-D.UI.trkWdt)*0.9;
        velMin = 0;
        velMax = 100;
        % convert vel to roh range
        velRoh = D.P.(fldStr).vel;
        if velRoh>velMax; velRoh = velMax;
        elseif velRoh<velMin; velRoh = velMin;
        end
        velRoh = velRoh/velMax;
        velRoh = velRoh*(ampMax-ampMin) + ampMin;
        % convert to cart
        lastRad = D.P.Rad.rad(find(~isnan(D.P.Rat.rad), 1, 'last'));
        [D.P.(fldStr).velX, D.P.(fldStr).velY] = pol2cart(lastRad,velRoh);
        D.P.(fldStr).velX =  D.P.(fldStr).velX.*D.PAR.R + D.PAR.XC;
        D.P.(fldStr).velY =  D.P.(fldStr).velY.*D.PAR.R + D.PAR.YC;
        ...
        if strcmp(fldStr, 'Rob')
            ...
            % Get butt pos
            D.P.Rob.buttRad = D.P.Rob.rad(end) + D.PAR.buttDist;
...
% -------------------------REWARD FEEDER CHECK-----------------------------
    ...
    function [] = SF_Reward_Feeder_Check()
        ...
        if ...
                any(D.B.track_fd) && ...
                D.C.rew_rec_cnt < 30*(D.PAR.rewDel+1)
            
            % Get count of in-bounds samples
            D.C.rew_inbnd_cnt = D.C.rew_inbnd_cnt + sum(D.B.track_fd);
            
            
            % Add to record counter
            D.C.rew_rec_cnt = D.C.rew_rec_cnt + D.P.Rat.recs;
            
            if ...
                    D.C.rew_rec_cnt >= 30*D.PAR.rewDel && ...
                    D.C.rew_inbnd_cnt >= 30*D.PAR.rewDel && ...
                ...
                % Check if this reward was cued and if not, reward
                if D.UI.cueFeed ~= 'All'
                    if D.UI.cueFeed == 'Half' && D.B.cue_next
                        D.B.cue_next = false;
                    else
...
% ----------------------------LAP CHECK------------------------------------
    ...
    function [] = SF_Lap_Check()
        ...
        if all(D.B.track_lap);
            ...
            Hind = ~isnan(D.UI.ratPltH(:));
            if ~shouldExit
                set(D.UI.ratPltH(Hind), 'MarkerEdgeColor', [0.75,0.75,0.75]);
            end
            if ~shouldExit
                set(D.UI.ratPltHvel(Hind), 'MarkerEdgeColor', D.UI.ratHistCol);
            end
            if ~shouldExit
                set(D.UI.robPltHvel(Hind), 'MarkerEdgeColor', D.UI.robHistCol);
            end
...
% ------------------------REWARD RESET CHECK-------------------------------
    ...
    function [] = SF_Reward_Reset_Check()
        ...
        % Tell CS that this is a cued reward lap if it is
        if ...
                (D.UI.cueFeed == 'All' || ...
                (D.UI.cueFeed == 'Half' && D.B.cue_next)) && ...
                track_quad(3) && ...
            ...
            CS_Send('R', mean(D.UI.rewBnds(D.I.rot,:)));
            D.B.cue_sent_to_cs = true;
...
% --------------------------PLOT POSITION----------------------------------
    ...
    function [] = SF_Pos_Plot()
        ...
        if ...
            ...
            Hind = find(isnan(D.UI.ratPltH(:)), 1, 'first');
            ...
            if ~shouldExit
                D.UI.ratPltHvel(Hind) = ...
                    plot(D.P.Rat.velX, D.P.Rat.velY, '-', ...
                    'Color', D.UI.ratAllCol, ...
                    'LineWidth', 2, ...
                    'Parent', D.UI.axH(1));
            end
            ...
            if ~shouldExit
                D.UI.robPltHvel(Hind) = ...
                    plot(D.P.Rob.velX, D.P.Rob.velY, '-', ...
                    'Color', D.UI.robPosCol, ...
                    'LineWidth', 2, ...
                    'Parent', D.UI.axH(1));
            end
            ...
            if ~shouldExit
                D.UI.ratPltH(Hind) = ...
                    ...
                    'MarkerFaceColor', D.UI.ratAllCol, ...
                    'MarkerEdgeColor', D.UI.ratAllCol, ...
                    'MarkerSize', 6, ...
                    'Parent', D.UI.axH(1));
            end
            ...
            [xbnd, ybnd] =  Get_Trk_Bnds( [D.P.Rob.guardRad, D.P.Rob.buttRad]);
            if ~shouldExit
                D.UI.ptchRobPos = ...
                    patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                    [ybnd(1,:),fliplr(ybnd(2,:))], ...
                    D.UI.robPosCol, ...
                    'EdgeColor', D.UI.robPosCol, ...
                    'LineWidth', 2, ...
                    'FaceAlpha',0.75, ...
                    'Parent',D.UI.axH(1));
            end
            ...
            if ~shouldExit
                D.UI.vtRobPltNow = ...
                    ...
                    'MarkerEdgeColor', D.UI.robPosCol, ...
                    'MarkerSize', 10, ...
                    'Parent', D.UI.axH(2));
            end
            % plot guard pos
            ...
            [xbnd, ybnd] =  Get_Trk_Bnds(D.P.Rob.guardRad);
            if ~shouldExit
                D.UI.quardPltNow = ...
                    plot(xbnd, ybnd, ...
            ...
            [xbnd, ybnd] =  Get_Trk_Bnds([D.P.Rob.feedRad, D.P.Rob.buttRad]);
            if ~shouldExit
                D.UI.armPltNow = ...
                    plot(xbnd(1,:), ybnd(1,:), ...
                    'Color', D.UI.robPosCol, ...
            ...
            [xbnd, ybnd] =  Get_Trk_Bnds(D.P.Rob.setRad);
            if ~shouldExit
                D.UI.setPltNow = ...
                    plot(xbnd, ybnd, ...
                    'Color', D.UI.robPosCol, ...
            ...
            % Plot feeder pos feed cond color
            if isfield(D.UI, 'feedPltNow');
                delete(D.UI.feedPltNow);
            end
            [xbnd, ybnd] =  Get_Trk_Bnds(D.P.Rob.feedRad);
            if ~shouldExit
                D.UI.feedPltNow = ...
...
% RAT SELECTION
    function [] = PopRat(~, ~, ~)
        ...
        if get(D.UI.popRat,'Value') ~= 1
            ...
            set(D.UI.popRat, ...
                'BackgroundColor', D.UI.dfltBtnActiveCol, ...
                'ForegroundColor', D.UI.dfltTxtLightCol);
            ...
            if D.PAR.sesNum > 0
                ...
                set(D.UI.popCond, 'Value', ...
                    find(ismember(D.UI.condList, 'Manual Feed'))); %'Manual Feed' 'ICRb Session'
                % run callback
                PopCond();
...
% CUE CONDITION
    function [] = ToggCue(~, ~, ~)
        ...
        if get(D.UI.toggCue(1), 'Value') == 1
            % Set to None
            set(D.UI.toggCue(1), ...
                'BackgroundColor', D.UI.dfltBtnActiveCol);
            set(D.UI.toggCue([2,3]), ...
                'BackgroundColor', D.UI.enabledCol, ...
                'Value',0);
            D.UI.cueFeed(:) = 'All';
        elseif get(D.UI.toggCue(2), 'Value') == 1
            % Set to Half
            set(D.UI.toggCue(2), ...
                'BackgroundColor', D.UI.dfltBtnActiveCol);
            set(D.UI.toggCue([1,3]), ...
                'BackgroundColor', D.UI.enabledCol, ...
                'Value',0);
            D.UI.cueFeed(:) = 'Half';
        elseif get(D.UI.toggCue(3), 'Value') == 1
            % Set to All
            set(D.UI.toggCue(3), ...
                'BackgroundColor', D.UI.dfltBtnActiveCol);
            set(D.UI.toggCue([1,2]), ...
                'BackgroundColor', D.UI.enabledCol, ...
                'Value',0);
            D.UI.cueFeed(:) = 'None';
        end
...
% BULLDOZE RAT
    function [] = Bulldoze(~, ~, ~)
        if (get(D.UI.btnBulldoze, 'Value') == 1)
            ...
            D.UI.guardPosCol = D.UI.dfltBtnActiveCol;
            % Tell CS to bulldoze
            CS_Send('B', D.PAR.bullDel);
        elseif (strcmp(get(gcbo, 'Style'), 'togglebutton'))
            % Change backround color and text
            set(D.UI.btnBulldoze, ...
                'String', 'Ceasefire', ...
                'BackgroundColor', D.UI.enabledCol);
...
% CLEAR VT BUTTON
    function [] = BtnClrVT(~, ~, ~)
        ...
        delete(D.UI.ratPltH(~isnan(D.UI.ratPltH(1,:))))
        delete(D.UI.ratPltHvel(~isnan(D.UI.ratPltH(1,:))))
        delete(D.UI.robPltHvel(~isnan(D.UI.ratPltH(1,:))))
        % Reinitialize handle array back to NAN
        D.UI.ratPltH = NaN(1,60*120/D.PAR.polRate); % VT plot handle array
        D.UI.ratPltHvel = NaN(1,60*120/D.PAR.polRate); % VT plot handles of velocity
        D.UI.robPltHvel = NaN(1,60*120/D.PAR.polRate); % VT plot handles of velocity
        D.UI.ratPltHrew = false(1,60*120/D.PAR.polRate); % bool for tracking VT plot handles of reward delivery points
...
% --------------------------SEND CS COMMAND--------------------------------
    ...
    function[] = CS_Send(id, data1, data2)
