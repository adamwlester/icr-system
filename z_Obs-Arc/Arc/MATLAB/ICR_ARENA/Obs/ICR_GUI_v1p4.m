...
%   val 3 = speaker left chanel (aversive sound)
%   val 4 = pump
%   val 5 = ozone
%   val 6 = vacuum
%--------------------------------------------------------------------------
...
while D.B.poll_nlx
    ...
    if D.B.ratIn
        ...
        %fprintf('%s%s\r\n', blanks(10), D.NLX.port_out_update_cmd)
...
% -------------------------------UI SETUP-----------------------------------
    ...
    function[D] = SF_UI_Setup(D)
        ...
        D.UI.timFdRewDur = timer('TimerFcn',@TimStopFeed, ...
            'UserData', 0, ...
            'StartDelay', D.PAR.fdPlsDur/1000);
        
        % Reward tone stop timer
        D.UI.timToneRewDur = timer('TimerFcn',@TimStopTone, ...
            'UserData', 0, ...
            'StartDelay', D.PAR.toneDur/1000);
        
        % Ozone stop timer
        D.UI.timOzDur = timer('TimerFcn',@TimStopOz, ...
            'UserData', 0, ...
            'StartDelay', D.PAR.ozDur/1000);
        
        % Vac start timer
        D.UI.timVacDel = timer('TimerFcn',@TimStartVac, ...
            'UserData', 0, ...
            'StartDelay', D.PAR.vacDel/1000);
        
        % Vac stop timer
        D.UI.timVacDur = timer('TimerFcn',@TimStopVac, ...
            'UserData', 0, ...
    ...
    function [D] = SF_Reward_Feeder_Check(D)
        ...
        if ...
            ...
            if ...
                ...
                % start vacuum delay timer
                if D.UI.sndair(4)
                    % start vac delay timer
                    start(D.UI.timVacDel)
                end
                
                % Save current feeder to timer handle
                set(D.UI.timFdRewDur, 'UserData', D.I.feed_ind(D.I.rot));
...
% FEEDER PULSE
    function [] = BtnPlsFd(~, ~, ~)
        ...
        % Save current feeder to timer handle
        set(D.UI.timFdRewDur, 'UserData', ud_sltd)
...
% FEEDER REWARD TIMER STOP
    function [] = TimStopFeed(~, ~, ~)
        % Get handle data
        D = guidata(D.UI.figH);
        
        % Get current feeder
        fd_ind = get(D.UI.timFdRewDur, 'UserData');
        
        % Turn off feeder by changing/posting NLX ttl comand
        % feed solenoid bit
        D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind(fd_ind)) = '0';
        % post command
        NlxSendCommand(D.NLX.port_out_update_cmd);
        
        % Track timing
        D.T.fd_end_tim = clock;
        % print solenoid close time
        fprintf('F%d_CLOSE:%s%3.0f_ms\n', ...
            fd_ind, ...
            repmat('_',1,8), ...
            etime(D.T.fd_end_tim,D.T.fd_str_tim)*1000);
...
% REWARD TONE TIMER STOP
    function [] = TimStopTone(~, ~, ~)
        ...
        fprintf('Reward_Tone_OFF:%s%3.0f_ms\n', ...
            repmat('_',1,1), ...
            etime(clock,D.T.fd_str_tim)*1000);
...
% OZONE TIMER STOP
    function [] = TimStopOz(~, ~, ~)
        ...
        fprintf('Ozone_OFF:%s%3.0f_ms\n', ...
            repmat('_',1,7), ...
            etime(clock,D.T.oz_str_tim)*1000);
...
% VAC TIMER START
    function [] = TimStartVac(~, ~, ~)
        ...
        % Track timing
        D.T.vac_str_tim = clock;
        % print vac on time
        fprintf('Vacuum_ON:%s%3.0f(%3.0f)_ms\n', ...
            repmat('_',1,7), ...
            etime(D.T.vac_str_tim,D.T.fd_str_tim)*1000, ...
            etime(D.T.vac_str_tim,D.T.fd_end_tim)*1000);
...
% VAC TIMER STOP
    function [] = TimStopVac(~, ~, ~)
        ...
        NlxSendCommand(D.NLX.port_out_update_cmd);
        if D.B.setup
            % turn on pump if run after setup completed
            % pump bit on
            D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind(4)) = ...
                num2str(D.UI.sndair(4));
        end
        ...
        fprintf('Vacuum_OFF:%s%3.0f(%3.0f)_ms\n', ...
            repmat('_',1,6), ...
            etime(clock,D.T.fd_str_tim)*1000, ...
            etime(clock,D.T.vac_str_tim)*1000);
    ...
    function [] = Disconect_All(D)
        ...
        if isfield(D, 'NLX')
            if ~isempty(D.NLX)
                ...
                % feed solenoid bit
                D.NLX.port_out_update_cmd(D.NLX.port_solnd_bit_Ind) = '0';
                % air/sound bits
                D.NLX.port_out_update_cmd(D.NLX.port_snd_air_bit_Ind) = '0';
                % post command
                NlxSendCommand(D.NLX.port_out_update_cmd);
        ...
        if isfield(D, 'UI')
            ...
            delete(D.UI.timFdRewDur);
            delete(D.UI.timToneRewDur);
            delete(D.UI.timOzDur);
            delete(D.UI.timVacDel);
            delete(D.UI.timVacDur);
