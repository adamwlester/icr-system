...
OpenLog runs at my_setting_uart_speed by default. This is configurable to 1200, 2400, 4800, 9600, 19200, 38400, 57600, and 115200bps. You can alter
...
see a message 'Going to my_setting_uart_speed...' or some such message. You will need to power down OpenLog, change
...
in unison. Now power down OpenLog and remove the RX/GND jumper. OpenLog is now reset to my_setting_uart_speed.
...
#include <SPI.h>
//We do not use the built-in SD.h file because it calls Serial.print
#include <SdFat.h> 
//This is a new/beta library written by Bill Greiman. You rock Bill!
#include <SerialPort.h> 
//
#include <EEPROM.h>
//Allows us to print the available stack/RAM size
#include <FreeStack.h> 
...
//This is the baud rate that the system runs at, default is my_setting_uart_speed. Can be 1,200 to 1,000,000
long setting_uart_speed;
//This is the mode the system runs in, default is MODE_NEWLOG
byte setting_systemMode;
//This is the ASCII character we look for to break logging, default is ctrl+z
byte setting_escape_character;
//Number of escape chars before break logging, default is 3
byte setting_max_escape_character;
//This controls the whether we get extended or simple responses.
byte setting_verbose;
//This turns on/off echoing at the command prompt
byte setting_echo;
//This flag, when set to 1 will make OpenLog ignore the state of the RX pin when powering up
byte setting_ignore_RX;

//Default the system settings in case things go horribly wrong
long my_setting_uart_speed = 57600;
byte my_setting_systemMode = MODE_COMMAND;
byte my_setting_escape_character = 26;
byte my_setting_max_escape_character = 3;
byte my_setting_verbose = OFF;
byte my_setting_echo = OFF;
byte my_setting_ignore_RX = OFF;
...
void setup(void)
{
	...
	//AWL set EEPROM to my defaults
	setDefaultSettings();

	//AWL then load all system settings from EEPROM
	readSystemSettings(); 
...
//Disable SPI so that we control the LEDs
	...
	//Now sit in forever loop indicating system is now at my_setting_uart_speed
	while (1)
	{
		delay(500);
		toggleLED(stat1); //Blink the stat LEDs
...
{
	//Reset UART to my_setting_uart_speed
	writeBaud(my_setting_uart_speed);

	//Reset system to new log mode
	EEPROM.write(LOCATION_SYSTEM_SETTING, my_setting_systemMode);

	//Reset escape character to ctrl+z
	EEPROM.write(LOCATION_ESCAPE_CHAR, my_setting_escape_character);

	//Reset number of escape characters to 3
	EEPROM.write(LOCATION_MAX_ESCAPE_CHAR, my_setting_max_escape_character);

	//Reset verbose responses to on
	EEPROM.write(LOCATION_VERBOSE, my_setting_verbose);

	//Reset echo to on
	EEPROM.write(LOCATION_ECHO, my_setting_echo);

	//Reset the ignore RX to 'Pay attention to RX!'
	EEPROM.write(LOCATION_IGNORE_RX, my_setting_ignore_RX);
}
...
void readSystemSettings(void)
{
	...
	//Default is my_setting_uart_speed
...
void commandShell(void)
{
	...
	while (true)
	{
		...
		else if (strcmp_P(commandArg, PSTR("get")) == 0)
		{
			...
			sprintf_P(settingsString, PSTR("%ld %d %d %d %d %d %d\0"),
				setting_uart_speed, setting_escape_character, setting_max_escape_character, systemMode, setting_verbose, setting_echo, setting_ignore_RX);
			NewSerial.println(settingsString);
