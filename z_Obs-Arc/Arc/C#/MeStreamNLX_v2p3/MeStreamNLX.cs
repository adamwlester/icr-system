...
namespace MeStreamNLX
{
    class MeStreamNLX
    {
        ...
        private static char[] m2c_id = new char[8] { // prefix giving masage id
            'S', // start session
            'Q', // quit session
            'M', // move to position
            'R', // dispense reward
        ...
        // robot to cs
        private static char[] r2c_id = new char[8] {
            'S', // start session
            'Q', // quit session
            'M', // move to position
            'R', // dispense reward
        ...
        // general
        private static long t_c2rDel = 10;
            ...
            if (check_done)
            {
                while (!doExit)
                {
                    ...
                    for (int i = r2c_idHist.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (r2c_idHist[i] == r2c_id[6] && r2c_packHist[i] == pack)
                        {
                            // Send back done recieved confirmation
                            SendData(c2r_id[8], pack);
        ...
        private static void CheckDone(short pack)
        {
            ...
            while (stopWatch.ElapsedMilliseconds < t_resend_check && (!doExit || !doQuit))
            {
                if (r2c_idHist.Count > msg_cnt)
                {
                    ...
                    if (r2c_idHist[r2c_idHist.Count - 1] == r2c_id[6] && r2c_packHist[r2c_packHist.Count - 1] == pack)
                    {
            ...
            msg_str = String.Format("[CheckDone] DONE CheckDone Worker (pack:{0})", pack);
            PrintAction(msg_str);
        }
        ...
        private static void DoWork_MatCOM(object sender, DoWorkEventArgs e)
        {
            ...
            com_Matlab.PutWorkspaceData("m2c_id", "global", m2c_id[6]);
