...
%% ==========================MAIN FUNCTION==================================
    ...
    function[] = RunScript()
        ...
        function [] = SF_Finish_Setup()
            ...
            % rat vel
            infstr = sprintf('Rat_Velocity:_%0.2f(%0.0f)', 0, 0);
            set(D.UI.txtPerfInf(5), 'String', infstr)
            % rob vel
            infstr = sprintf('Rob_Velocity:_%0.2f(%0.0f)', 0, 0);
        ...
        function [] = SF_VT_Proc(fld)
            ...
            if vtNRecs > 0
                ...
                exc_2 = ...
                    rad_diff > (300/33)*((2 * pi)/(140 * pi)) &...
                    rad_diff < 0.9*(2 * pi);
                % Exclude values based on last kept sample
                rad_diff_last = abs(rad - D.P.(fld).radLast);
                exc_3 = ...
                    rad_diff_last > (300/33)*((2 * pi)/(140 * pi)) &...
                    rad_diff_last < 0.9*(2 * pi);
                
                % Track exc_3 use and skip if more than 1 sec of unused
                % data
                if etime(clock, D.T.(fld).last_pos_update) > 1
                ...
                if isempty(vel) || isempty(D.P.(fld).velRad)
                    ...
                    if D.P.(fld).vel < 150
                        D.P.(fld).vel_max = max(D.P.(fld).vel_max, D.P.(fld).vel);
                    end
...
%                 set(D.UI.robPltHvel(~isnan(D.UI.robPltHvel)), 'Color', D.UI.robHistCol, 'LineWidth', 1);
        ...
        function [] = SF_Inf_Print()
            ...
            % Rat vel
            infstr = sprintf('Rat_Velocity:_%0.2f(%0.0f)', ...
                D.P.Rat.vel, D.P.Rat.vel_max);
            set(D.UI.txtPerfInf(5), 'String', infstr)
            
            % Robot vel
            infstr = sprintf('Rob_Velocity:_%0.2f(%0.0f)', ...
                D.P.Rob.vel, D.P.Rat.vel_max);
            set(D.UI.txtPerfInf(6), 'String', infstr)
        ...
        function [] =  Save_Ses_Data()
            ...
            % Get file size
            fiGigs = dir(fullfile(D.DIR.cheetTempTop, D.DIR.recFi));
