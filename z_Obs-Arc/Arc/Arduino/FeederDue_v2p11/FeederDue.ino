...
const float defualtSetPoint = 40; // (cm)
const float guardDist = 4.5;
const float feedDist = 68;
float errorFeeder;
float errorDefault;
...
public:
	...
	void UpdatePID()
	{
		...
		// Compute new error 
		setPos = ekf_robPos + setPoint;
		error = ekf_ratPos - setPos;
		...
		if (mode == "Automatic")
		{
			...
			if (
				!isHolding4cross && // not waiting for setpoint pass
				ekfNew && // New EKF data 
	...
	void CheckDamp()
	{
		if (isDampened)
		{
			...
			if (damp_for == 0)
			{
				damp_for = deltaSetpoint * 300;
				cnt = 0;
			}

			// Update ellapsed time and setpoint
			t_dampDur += dtLoop * 1000;
	...
	void UpdateBull()
	{
		...
		if (mode == "On")
		{
				...
				millis() > t_loopNext)
			{
				...
				passedReset = errorDefault > 2 ? true : false;
...
void SendData()
{
	...
	if ((r2_queue[r2_lngR - 1][1] == 'r' ||
		r2_queue[r2_lngR - 1][1] == 'c'))
	{
		do_send = true;
	}
	// Avoid overlap between sent or rcvd events
	else if (millis() > t_sent + 20 &&
		millis() > t_rsvd + 30)
	{
		do_send = true;
	}
...
void HardStop(String called_from)
{
	...
	// Store string
	StoreStr("HARD STOP " + called_from);
}

// RUN AUTODRIVER
void RunMotor(char dir, float speed, String called_from)
{
	// Local vars
...
void SetMotorControl(String set_to)
{
	...
	else if (!fc_isHalted)
	{
		...
		// If not in "None" state set to whatever
		else
		{
			fc_motorControl = set_to;
		}
...
bool CompTarg(float move_targ, String move_type)
{
	...
	if (pid.ekfReady)
	{
		...
		if (move_type == "MoveTo")
		{
			...
			// Start movement
			RunMotor(moveDir, moveSpeed, "MoveTo");
		}
...
bool DoDecelerate(String move_type)
{
	...
	dec_pos = move_type == "MoveTo" ? 40 : 40;
...
void PrintStr()
{
	...
	// Copy to char array
	/*strncpy(tab2, tmp.toCharArray, sizeof(tab2));
...
void DebugRsvd(char id, uint16_t pack)
{
	...
	if (doPrint_rcvd && doDebug)
	{
		...
		{
			sprintf(s1, "--- rcvd: [id:%c d1:%d pack:%d", id, bullDel, pack);
		}
		else if (id == 'I')
		{
			sprintf(s1, "--- rcvd: [id:%c d1:%d pack:%d", id, fc_doStartTracking, pack);
		}
...
void Interupt_Halt() {
	// Disable if bulldozer running
	if (!(bull.state == "Active" && bull.mode == "On"))
	{
		HardStop("[Interupt_Halt]");
	}
}
