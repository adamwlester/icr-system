...
const int rewLEDduty = 20; // value between 0 and 255
...
public:
	...
	bool CompTarg(float now_pos, float targ_pos, float offset)
	{
		...
		if (!is_targ_set)
		{
			...
			if (pid.ekfReady)
			{
				...
				rel_pos = ((float)(round(((int)(pos_start * 100) % (int)(140 * PI * 100))))) / 100;

				// Diff and absolute targ_dist
				move_diff = offset_target - rel_pos;
				targ_dist =
	...
	float DecelToTarg(float dec_pos, float now_pos, float now_vel)
	{
	...
	{
		// Current relative pos on track
		rel_pos = ((float)(round(((int)(now_pos * 100) % (int)(140 * PI * 100))))) / 100;

		// Target error
		return offset_target - rel_pos;
	}
...
#pragma region //--- (M) DO MOVE ---
	...
	if (fc_doMove)
	{
		...
		{
			// If succesfull
			if (targ_moveTo.CompTarg(ekf_robPos, moveToTarg, -1 * feedDist))
			{
				// Start running
				if (
		...
		{
			// Do deceleration
			float new_speed = targ_moveTo.DecelToTarg(40, ekf_robPos, ekf_robVel);
...
#pragma region //--- (C) CUE REWARD ---
	...
	if (fc_doCueReward)
	{
		...
		if (!(targ_cueRat.is_targ_set && targ_cueRob.is_targ_set))
		{
			...
			if (
				targ_cueRat.CompTarg(ekf_ratPos, cueTarg, 0) &&
				targ_cueRob.CompTarg(ekf_robPos, cueTarg, -1 * defualtSetPoint)
				)
			{
				// Print message
				char str[50];
		...
		if (targ_cueRat.is_targ_set && targ_cueRob.is_targ_set)
		{
			...
			{
				if (
					targ_cueRat.CheckTargReached(pos_ratVT.posNow, 0) ||
					targ_cueRat.CheckTargReached(pos_ratPixy.posNow, 0) ||
...
void DebugRsvd(char id, uint16_t pack)
{
	...
	if (doPrint_rcvd && (doDebugConsole || doDebugLCD))
	{
		...
		{
			sprintf(str, "rcvd: [i:%c d1:%0.2f p:%d", id, moveToTarg, pack);
		}
		else if (id == 'R')
		{
