...
// Reward
uint32_t rewDur = 2000; // (ms) 
byte rewDur_byte = 2000; // (ms) 
uint32_t t_rewEnd;
float rewPos;
int rewCnt = 0;
...
#pragma region //--- (R) DO REWARD ---
	...
	{
		// Get reward duration in ms
		rewDur = (uint32_t)rewDur_byte * 10;
...
bool ParseSerial()
{
	...
	if (msg_id == 'R')
	{
		...
		// Get reward diration ind
		rewDur_byte = WaitBuffRead(0);
	}
...
void Interupt_Btn1() {
	...
	t_debounce[1] = millis() + rewDur + 100;
