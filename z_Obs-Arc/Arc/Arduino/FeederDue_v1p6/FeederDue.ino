...
public:
	...
	PosArr(int l, int dt)
	{
		nSamp = l;
		dtSamp = dt;
		for (int i = 0; i < l; i++) radArr[i] = 0;
	}

	float CompVel(float radNew)
	{
		// shift and add data
		...
		for (int i = 0; i < nSamp - 1; i++)
		{
			distRad = min((2 * PI) - abs(radArr[i + 1] - radArr[i]), abs(radArr[i + 1] - radArr[i]));
			if (abs(radArr[i + 1] - radArr[i]) == radArr[i + 1] - radArr[i])
			{
				distRad = distRad*-1;
			}
			distCM = distRad * ((140 * PI) / (2 * PI));
			distSum += distCM;
		}
		vel = distSum / (((nSamp - 1)*dtSamp) / (float)1000);
		int temp = 0;
		return vel;
	}
};
PosArr vtRatHist(4, 30);
PosArr pixyRatHist(6, 20);
PosArr vtRobHist(4, 30);
...
void loop() {
	...
	if (posUpdate[0] && posUpdate[1] && posUpdate[2])
	{
		...
		// Convert pixy cm to rad
		pixyPosRad = PixyCM_ToRad(pixyPosCM, vtRad[1]);
		double z[M] = {
			vtRad[0],
			vtRad[0],
			vtRad[1],
			vtRatHist.CompVel(vtRad[0]),
			pixyRatHist.CompVel(pixyPosRad),
			vtRobHist.CompVel(vtRad[1]),
		};
...
{
	// Compute position error
	float pxRad = (pxCM - pixyTarg) * ((2 * PI) / (140 * PI));
	pxRad = pxRad + vtTarg;
	pxRad = rbRad - pxRad;
	if (pxRad < 0)
	{
