...
void loop() {
	...
	// Check for start session mesage
	if (msg_id == msg_idArr[0] && msg_pass)
	{
		startRun = false;
		msg_pass = false;
		msg_id = '0';
	...
	// Check for new pos data
	else if (msg_id == msg_idArr[1] && msg_pass)
	{
	...
	// Check for end session mesage
	else if (msg_id == msg_idArr[5] && msg_pass)
	{
		QuitSession();
		msg_pass = false;
		msg_id = '0';
...
bool ParseSerial()
{
	...
	while (Serial1.available() > 0 && msg_id == '0')
	{
	...
	// Get VT data
	if (msg_id == msg_idArr[1])
	{

		// Contunue through main loop till complete packet
	...
	{
		pack_diff =
			min(254 - abs(msg_num - msg_numLast), abs(msg_num - msg_numLast));
		pack_tot += pack_diff;
...
void UpdateSpeed()
{
	...
	if (do_simulateRat)
	{
		...
		if (!isAligned)
		{
			// Aling robot to sim data
			static bool firstRun = true;
			setPos = vtCM[1] + setpoint;
			if (setPos > (140 * PI)) setPos = setPos - (140 * PI);
			posUpdate[0] = false;
			posUpdate[1] = false;
			if (!((setPos < 10) && (setPos > 5)))
			{
				if (firstRun)
				{
					//board.run(FWD, maxSpeed * cm2stp);
					firstRun = false;
				}
				return;
			}
