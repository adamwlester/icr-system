...
namespace MeStreamNLX
{
    class MeStreamNLX
    {
        ...
        private static long t_sendDel = 10;
        private static long t_nextSend = stopWatch.ElapsedMilliseconds + t_sendDel;
        ...
        static void Main()
        {
            ...
            // Initialize reapeat send thread object
            //ThreadStart repeatSendThread = new ThreadStart(RepeatSend());
        ...
        public static short SendData(char id, short pack_num = 0)
        {
            lock (lock_sendData)
            {
                ...
                if (stopWatch.ElapsedMilliseconds < t_nextSend)
                {
                    Thread.Sleep((int)(t_nextSend - stopWatch.ElapsedMilliseconds));
                }
                UnionHack u = new UnionHack(0, 0, 0, '0', 0);
                ...
                // Update next min run time
                t_nextSend = stopWatch.ElapsedMilliseconds + t_sendDel;
        ...
        private static void ProgressChanged_MatCOM(object sender, ProgressChangedEventArgs e)
        {
            ...
            string str = String.Format("   mCOM: [id:{0} dat1:{1:0.00} dat2:{2:0.00}]", matIn_id, matIn_dat1, matIn_dat2);
            PrintState(str);
