...
reward_lft = sin(2*pi*(1/nsfs:1/nsfs:nsmin*60)*2500)' * 0.2;
...
while true
    ...
    while true
        ...
        % Check if connection shoud be closed
        if data(1) == 0
            % Print status message
        ...
        if cue_fd
            if ... % flicker cue
                ...
                if cue_state(1) == 0
                    ...
                    frame_java = fullscreenAWL(fullfile(imgDir,subDir{2},fiIn{cue_ind,img_ind}));
                    
                    % Pause to allow trigger to register
                    pause(trgDel);
                    
                    % Show main cue image
                    frame_java = fullscreenAWL(fullfile(imgDir,subDir{1},fiIn{cue_ind,img_ind}));
                    fprintf('\rcue on: %s\r', ...
                        datestr(now, 'HH:MM:SS AM'))
