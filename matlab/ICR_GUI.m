...
% -----------------------------MAIN SETUP----------------------------------
    function[] = Setup()
        ...
        % IO top
        D.DIR.ioTop = fullfile(D.DIR.top,'data');
        ...
        % IO tables
        D.DIR.SS_IO_1 = fullfile(D.DIR.ioTop, 'session', 'SS_IO_1.mat');
        D.DIR.SS_IO_2 = fullfile(D.DIR.ioTop, 'session', 'SS_IO_2.mat');
        D.DIR.SS_IO_3 = fullfile(D.DIR.ioTop, 'session', 'SS_IO_3.mat');
        D.DIR.TT_IO_1 = fullfile(D.DIR.ioTop, 'session', 'TT_IO_1.mat');
        D.DIR.TT_IO_2 = fullfile(D.DIR.ioTop, 'session', 'TT_IO_2.mat');
        ...
        % Enable setup panel and load popup menu
...
% ------------------------------VAR SETUP--------------------------
    function[] = Var_Setup()
        ...
        D.PAR.listRat = ...
            ...
            cellstr(char(D.SS_IO_1.Tail_Mark(D.SS_IO_1.Include_Run)))];
...
% ------------------------------NLX SETUP--------------------------
    function[] = NLX_Start()
        ...
        % Sleep 2
        D.NLX.sleep_start_evt{2} = '-PostEvent Post_Sleep_2_Start 209 0';
        D.NLX.sleep_end_evt{2} = '-PostEvent Post_Sleep_2_End 210 0';
        
        % Reward Zoneet and Duration
        D.NLX.rew_evt = '-PostEvent Post_Reward_Zone:%d_Duration:%d 211 0';
        
        % Reward Missed
        D.NLX.miss_evt = '-PostEvent Post_Reward_Missed 213 0';
...
% -----------------------------TABLE TAB SETUP-------------------------
    function[] = Table_Tab_Setup()
        ...
        botm = sum(D.UI.food_pan_pos([2,4])) - D.UI.fontSzTxtMed(2)*3.5;
        lft = D.UI.food_pan_pos(1) + obj_gap;
        wd =  (D.UI.food_pan_pos(3) - obj_gap*2)/4 - obj_gap;
        ht = D.UI.fontSzTxtMed(2)*2;
        
        % Pellets text
        D.UI.txtFeedPellets(1) = copyobj(D.UI.txtTemplate, D.UI.tabTBL);
        D.UI.txtFeedPellets(1).Position = [lft, botm, wd, ht];
        str = sprintf('Pellets\n%0.2f', D.SS_IO_3.(D.PAR.ratLab).Fed_Pellets(end));
        ...
        D.UI.editFeedPellets.Position(1:3) = [lft, botm - D.UI.editTemplate.Position(4)*0.5, wd];
        ...
        D.UI.txtFeedMash(1).Position = [lft, botm, wd, ht];
        str = sprintf('Mash(TB)\n%0.2f', D.SS_IO_3.(D.PAR.ratLab).Fed_Mash(end));
        ...
        D.UI.editFeedMash.Position(1:3) = [lft, botm - D.UI.editTemplate.Position(4)*0.5, wd];
        ...
        D.UI.txtFeedEnsure(1).Position = [lft, botm, wd, ht];
        str = sprintf('Ensure(TB)\n%0.2f', D.SS_IO_3.(D.PAR.ratLab).Fed_Ensure(end));
        ...
        D.UI.editFeedEnsure.Position(1:3) = [lft, botm - D.UI.editTemplate.Position(4)*0.5, wd];
        ...
        D.UI.txtFeedSTAT(1).Position = [lft, botm, wd, ht];
        str = sprintf('STAT(TB)\n%0.2f', D.SS_IO_3.(D.PAR.ratLab).Fed_STAT(end));
        ...
        D.UI.editFeedSTAT.Position(1:3) = [lft, botm - D.UI.editTemplate.Position(4)*0.5, wd];
        ...
        % Save feed button
        botm = D.UI.food_pan_pos(2) + 0.005;
        ...
        inc_ind = cell2mat(cellfun(@(x) ~isempty(x), notes, 'uni', false));
        D.UI.tblGenNotes.Data = D.UI.tblGenNotes.Data(inc_ind);
        D.UI.tblGenNotes.RowName = D.UI.tblGenNotes.RowName(inc_ind);
...
% --------------------------TT TRACKING SETUP----------------------
    function[was_ran] = TT_Track_Setup()
        ...
        % Create number of rotations popup menu object
        ...
        % Create direction popup menu object
    ...
    function ICR_Session_Setup()
        ...
        if D.PAR.sesTask == 'Track'
            ...
            if D.PAR.sesCond == 'Manual_Training'
                ...
                % Set start to stored start quad based on total sessions
        ...
        % REWARD SEND/RESET BOUNDS
        D.UI.rewSendBnds(1,1:2) = D.PAR.rewZoneBnds(1,end,1) + [deg2rad(25),deg2rad(40)];
        D.UI.rewSendBnds(2,1:2) = D.PAR.rewZoneBnds(1,end,2) + [deg2rad(25),deg2rad(40)];
        D.UI.rewSendBnds = wrapTo2Pi(D.UI.rewSendBnds);
...
% --------------------------TRACK TASK SETUP-----------------------
    function[] = Track_Task_Setup()
        ...
        for z_rot = 1:2
            [xbnd, ybnd] =  ...
                Get_Cart_Bnds(D.UI.rewSendBnds(z_rot,:));
        ...
        % Set initial state of reward reset/send
        if D.PAR.sesCond ~= 'Manual_Training' %#ok<*STCMP>
            % Set reward reset/send patch to visible
...
% ------------------------TRACK REWARD CHECK-----------------------
    function Track_Reward_Check()
        ...
        function [rew_str] = SendReward(rew_str)
            ...
            check_inbound = Check_Pol_Bnds(D.P.Rat.rad, D.P.Rat.roh, D.UI.rewSendBnds(D.I.rot,:));
            if ~any(check_inbound)
                return
            end
        ...
        function CheckRewConf(rew_str)
            ...
            if c2m.('Z').dat2 == 0
                ...
                % Send/store NLX event
                Send_NLX_Cmd(D.NLX.miss_evt);
...
% ----------------------------BULLDOZE RAT---------------------------------
    function Togg_Bulldoze(~, ~, ~)
        ...
        % Store bulldoze delay
        D.PAR.bullDel = str2double(del_str{:});
        
        % Get bulldoze speed
        bull_spd = str2double(get(D.UI.editBulldoze, 'String'));
        % Make sure speed is < 100 cm/sec
        if bull_spd > 100 || isnan(bull_spd)
            bull_spd = D.PAR.bullSpeed;
            set(D.UI.editBulldoze, 'String', num2str(bull_spd))
        end
        % Store bulldoze speed
        D.PAR.bullSpeed = bull_spd;
        
        % Send bulldoze command
        if (get(D.UI.toggBulldoze, 'Value') == 1)
        ...
        Safe_Set(D.UI.toggBulldoze, 'String', sprintf('Bulldoze (%d sec)', D.PAR.bullDel));
