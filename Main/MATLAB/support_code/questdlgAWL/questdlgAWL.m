function ButtonName=questdlgAWL(Question,Title,Btn1,Btn2,Btn3,Default,ScreenCenter)
...
if nargin<1
    error(message('questdlgAWL:TooFewArguments'));
end
...
%%%%%%%%%%%%%%%%%%%%
if nargout > 1
    error(message('questdlgAWL:WrongNumberOutputs'));
end
if nargin > 7
    error(message('questdlgAWL:TooManyInputs'));
end
if isempty(Btn1)
    error(message('questdlgAWL:MustSpecifyAtLeastOneButton'));
elseif isempty(Btn2) && isempty(Btn3)
    NumButtons = 1;
elseif isempty(Btn3)
    NumButtons = 2;
else
...
if ~DefaultValid
    ...
    warning(message('questdlgAWL:StringMismatch'));
    warning(warnstate);
end
