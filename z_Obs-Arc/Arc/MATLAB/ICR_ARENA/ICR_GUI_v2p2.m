...
function[shouldExit] = ICR_GUI
global cmdMatOut; % message out to CS
global cmdMatIn; % message in from CS
global datMatOut_1; % data out to CS
global datMatOut_2; % data out to CS
...
% Tell C# Matlab is connected to Cheetah
D = CS_Send(D, D.CS.mat2cs_id(7));
...
while(~shouldExit)
    ...
    if D.B.setup
        ...
        if ~D.B.manual_ses
        D = CS_Send(D, D.CS.mat2cs_id(3), mean(D.UI.strQuadBnds));
        else
            D = CS_Send(D, D.CS.mat2cs_id(3), mean(D.UI.rewBnds(D.I.rot,:)));
        end
...
% ------------------------------VAR SETUP----------------------------------
    ...
    function [D] = SF_Var_Setup(D)
        ...
        % Matlab 2 C# vars
        % id chars
        D.CS.mat2cs_id = [ ... % prefix giving masage id
            'S', ... % start session
            'Q', ... % quit session
            'M', ... % move to position
            'R', ... % dispense reward
        ...
        D.CS.rot_cnt = 0;
        % lap by rotation
        D.CS.lap_cnt = num2cell(zeros(1,3));
        % rew by rotation
        D.CS.rew_cnt = num2cell(zeros(1,3));
        % track reversals
        D.CS.rev_cnt = 0;
...
% -------------------------------UI SETUP-----------------------------------
    ...
    function[D] = SF_UI_Setup(D)
        ...
        D.UI.fg_pos = [ ...
            ...
            fg_wh(1), ...
            fg_wh(2)];
        set(D.UI.figH,'Position',D.UI.fg_pos);
        ...
        D.UI.ratPosCol = [0, 0, 0];
        % color of marker for current rob pos
        D.UI.robPosCol = [1, 0.56, 0];
        % color of marker for current rob setpoint
        D.UI.setPosCol = [1, 1, 0];
...
% ------------------------------NLX SETUP----------------------------------
    ...
    function[D] = SF_NLX_Setup(D)
        ...
        if NlxAreWeConnected() ~= 1
            ...
            Update_Console(sprintf('\nAlready Connected To DigitalLynx SX\nIP: %s\nTime: %s\n', ...
                D.NLX.IP, datestr(now, 'HH:MM:SS')));
        end
...
% ----------------------------RAT IN CHECK---------------------------------
    ...
    function [D] = SF_Rat_In_Check(D)
        ...
        if ...
            ...
            % Send setup cond and tone to C#
            D = CS_Send(D, D.CS.mat2cs_id(1), ...
    ...
    function [D] = SF_Rotation_Trig_Check(D)
        ...
        if get(D.UI.btnICR, 'UserData')
            ...
            if any(D.B.track_rot)
                ...
                D.CS.rot_cnt = D.CS.rot_cnt + 1;
                
                % Add new lap counter
                D.CS.lap_cnt{D.I.rot} = [D.CS.lap_cnt{D.I.rot}, 0];
                D.CS.rew_cnt{D.I.rot} = [D.CS.rew_cnt{D.I.rot}, 0];
...
% -------------------------REWARD FEEDER CHECK-----------------------------
    ...
    function [D] = SF_Reward_Feeder_Check(D)
        ...
        if ...
            ...
            D.CS.inbnd_cnt = D.CS.inbnd_cnt + sum(D.B.track_fd);
            ...
            if ...
                    ...
                    D.CS.inbnd_cnt >= 30*D.PAR.rewDel && ...
                ...
                if D.B.rotated
                    D.CS.rew_cnt{D.I.rot}(end) = D.CS.rew_cnt{D.I.rot}(end) + 1;
                else
                    D.CS.rew_cnt{3}(end) = D.CS.rew_cnt{3}(end) + 1;
                end
...
% ----------------------------LAP CHECK------------------------------------
    ...
    function [D] = SF_Lap_Check(D)
        ...
        if all(D.B.track_lap);
            ...
            % Add lap to counter
            if D.B.rotated
                D.CS.lap_cnt{D.I.rot}(end) = D.CS.lap_cnt{D.I.rot}(end) + 1;
            else
                D.CS.lap_cnt{3}(end) = D.CS.lap_cnt{3}(end) + 1;
            end
            ...
            D.UI.lapTim = [D.UI.lapTim; ...
                {sprintf('%d:    %1.1f', sum([D.CS.lap_cnt{:}]), etime(clock,D.T.lap_tim))}];
    ...
    function [D] = SF_Pos_Plot(D)
        ...
        if ...
            ...
            D.UI.vtPltH(Hind) = ...
                ...
                'MarkerFaceColor', D.UI.ratPosCol, ...
                'MarkerEdgeColor', D.UI.ratPosCol, ...
                'MarkerSize', 6, ...
                'Parent', D.UI.axH(1));
            ...
            % Plot rob pos
            [xbnd, ybnd] =  Get_Trk_Bnds(D, [D.P.Rob.setRad, D.P.Rob.guardRad]);
            D.UI.ptchRobPos = ...
                patch([xbnd(1,:),fliplr(xbnd(2,:))], ...
                [ybnd(1,:),fliplr(ybnd(2,:))], ...
                D.UI.robPosCol, ...
            ...
            % Plot feeder
            [xbnd, ybnd] =  Get_Trk_Bnds(D, [D.P.Rob.feedRad,D.P.Rob.feedRad]);
            D.UI.feedPltNow = ...
                plot(xbnd(1), ybnd(1), 'o', ...
                'MarkerFaceColor', D.UI.setPosCol, ...
                'MarkerEdgeColor', [0, 0, 0], ...
                'MarkerSize', 8, ...
                'Parent', D.UI.axH(2));
...
% -------------------------PRINT SES INFO----------------------------------
    ...
    function [D] = SF_Inf_Print(D)
        ...
        infstr = sprintf([...
            ...
            'Reversals_Total:%s%d\n', ...
            'Rotations_Total:%s%d'], ...
            repmat('_',1,2), sum([D.CS.lap_cnt{:}]), ...
            repmat('_',1,2), sum([D.CS.rew_cnt{:}]), ...
            repmat('_',1,2), D.CS.rev_cnt, ...
            repmat('_',1,2), D.CS.rot_cnt);
        set(D.UI.txtPerfInf(4), 'String', infstr)
        ...
        infstr = sprintf([...
            'Laps______Stand:%s%d\n', ...
            'Rewards___Stand:%s%d'], ...
            repmat('_',1,2), D.CS.lap_cnt{3}(end), ...
            repmat('_',1,2), D.CS.rew_cnt{3}(end));
        set(D.UI.txtPerfInf(1), 'String', infstr)
        ...
        infstr = sprintf([...
            'Laps______40%c:%s%d|%d\n', ...
            'Rewards___40%c:%s%d|%d'], ...
            char(176), repmat('_',1,4), D.CS.lap_cnt{2}(end), sum(D.CS.lap_cnt{2}), ...
            char(176), repmat('_',1,4), D.CS.rew_cnt{2}(end), sum(D.CS.rew_cnt{2}));
        set(D.UI.txtPerfInf(2), 'String', infstr)
        % 0 deg
        infstr = sprintf([...
            'Laps______0%c:%s%d|%d\n', ...
            'Rewards___0%c:%s%d|%d'], ...
            char(176), repmat('_',1,5), D.CS.lap_cnt{1}(end), sum(D.CS.lap_cnt{1}), ...
            char(176), repmat('_',1,5), D.CS.rew_cnt{1}(end), sum(D.CS.rew_cnt{1}));
        set(D.UI.txtPerfInf(3), 'String', infstr)
    ...
    function [] = PopRat(~, ~, ~)
        ...
        if get(D.UI.popRat,'Value') ~= 1
            ...
            set(D.UI.popCond, 'Value', ...
                find(ismember(D.UI.condList, 'Manual Feed')));
            D.B.rot_ses = false;
...
% ICR CONDITION
    function [] = PopCond(~, ~, ~)
        ...
        if get(D.UI.popCond, 'Value') ~= 1
            ...
            % Set condition bools
            if get(D.UI.popCond, 'Value') == 1; % manual session
                D.B.manual_ses = true;
...
% SOUND CONDITION
    function [] = RadSnd(~, ~, ~)
        ...
        elseif get(D.UI.btnStupDn, 'UserData') == 1 && D.B.poll_nlx
            ...
            % Get button ID and state value
            D.UI.sndVal = get(gcbo, 'Value');
            D.UI.sndID = get(gcbo, 'UserData');
            
            % Check if value should really be changed
            qststr = sprintf('Are you sure you want to turn %s to %s?', ...
                D.UI.sndLabs{D.UI.sndID}, D.UI.sndState{D.UI.sndVal+1});
            ...
            switch choice
                case 'No'
                    ...
                    set(gcbo, 'Value', find([0, 1] ~=  D.UI.sndVal)-1)
                case 'Yes'
                    % Update vars
                    if D.UI.sndVal == 1
                        D.UI.snd(D.UI.sndID) = true;
                    else
                        D.UI.snd(D.UI.sndID) = false;
                    end
        ...
        if updateAC
            ...
            if D.AC.data(4) == 1 && ~D.UI.snd(1) % turn sound off
                D.AC.data(4) = 0; % turn off all sound
            elseif D.AC.data(4) == 0 && D.UI.snd(1) % turn sound on
                D.AC.data(4) = 1;
            end
            fwrite(tcpIP,D.AC.data,'int8');
        end
...
% ROTATION BUTTON
    function [] = BtnICR(~, ~, ~)
        ...
        D.UI.rotBndNext = D.UI.rotBnds(...
            find(D.PAR.catRotPos == D.PAR.rotPos(D.CS.rot_cnt+1)), ...
            :, rotNext);
...
% --------------------------SEND CS COMMAND--------------------------------
    ...
    function[D] = CS_Send(D, id, data1, data2)
...
% ---------------------------SAVE SESSION DATA-----------------------------
    function [] =  Save_Ses_Dat(D)
        ...
        D.SS_Out_ICR.(D.PAR.ratLab).Reversals(rowInd) = D.CS.rev_cnt;
        D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Standard{rowInd} = sum(D.CS.rew_cnt{3});
        D.SS_Out_ICR.(D.PAR.ratLab).Laps_Standard{rowInd} = sum(D.CS.lap_cnt{3});
        ...
        if D.B.rot_ses
            ...
            D.SS_Out_ICR.(D.PAR.ratLab).Rotations_Per_Session(rowInd) = D.CS.rot_cnt;
            D.SS_Out_ICR.(D.PAR.ratLab).Rotation_Positions{rowInd}(1:D.CS.rot_cnt) = ...
                D.PAR.rotPos(1:D.CS.rot_cnt);
            D.SS_Out_ICR.(D.PAR.ratLab).Rewards_Per_Rotation{rowInd} = ...
                [D.CS.rew_cnt{3}, reshape([[D.CS.rew_cnt{1}]; [D.CS.rew_cnt{2}]], 1, [])];
            D.SS_Out_ICR.(D.PAR.ratLab).Days_Till_Rotation(rowInd) = D.PAR.daysTilRot;
            D.SS_Out_ICR.(D.PAR.ratLab).Rewards_40_Deg{rowInd} = D.CS.rew_cnt{2};
            D.SS_Out_ICR.(D.PAR.ratLab).Laps_40_Deg{rowInd} = D.CS.lap_cnt{2};
            D.SS_Out_ICR.(D.PAR.ratLab).Rewards_0_Deg{rowInd} = D.CS.rew_cnt{1};
            D.SS_Out_ICR.(D.PAR.ratLab).Laps_0_Deg{rowInd} = D.CS.lap_cnt{1};
        else
