...
%% ==========================MAIN FUNCTION==================================
    ...
    function[] = RunScript()
        ...
        function [] = SF_Var_Setup()
            ...
            % REWARD VARS
            D.PAR.rewDurLim = [100, 2000];
            D.PAR.targLocs = 20:-5:-20;
            D.PAR.targRewDur = NaN(1,length(D.PAR.targLocs));
        ...
        function[] = SF_Targ_Dist_Setup()
            ...
            wdth = 0.1;
            ht = 0.1;
            lft = (D.UI.main_ax_bounds(1)+D.UI.main_ax_bounds(3)) - 0.01 - wdth;
            botm = 0.045;
            targ_ax_pos = [...
                lft, ...
                botm, ...
            ...
            db = db*0.5;
            db = (db/max(db)) * (max(db)/5)*4 + max(db)/10;
            db = [db; min(db)*ones(length(df)-length(db),1)];
            dist_long = [df;db]';
            ...
            set(D.UI.axTargH(2), ...
                'YLim' , [0, 1], ...
                'YTick',0:200/D.PAR.rewDurLim(2):1, ...
                'YTickLabels',0:200:D.PAR.rewDurLim(2), ...
        ...
        function [] = SF_Finish_Setup()
            ...
            uistack(reshape(D.UI.fdH(1:2,:),1,[]),'bottom');
        ...
        function [] = SF_VT_Proc(fld)
            ...
            if vtNRecs > 0
                ...
                if D.T.rew_start > 0
                    if etime(clock, D.T.rew_start) > 5
                        D.B.is_rewarding = false;
                    end
                end
                ...
                if strcmp(fld, 'Rob') && ~D.B.rat_in
                    ...
                    if abs(D.P.(fld).velRad - D.P.Rat.radLast) > (2*pi)/8
                ...
                if any(~isnan(D.P.Rat.rad))
                    ...
                    if any(check_inbound)
                        ...
                        if ~D.B.flag_reset_crossed
                            ...
                            % Check if reward was crossed but not triggered
                        ...
                        if ~D.B.flag_rew_reset
                            ...
                            if D.B.is_cued_rew
                                ...
                                %                                 % TEST
                                %                                 targ_rad = D.UI.rewBnds(5,2,D.I.rot);
                                %                                 Mat2CS('C', targ_rad, D.PAR.rewDur/10);
