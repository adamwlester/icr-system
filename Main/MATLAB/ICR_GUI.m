function[status] = ICR_GUI(sysTest, doDebug, isCheetahOpen, isMatSolo, doAutoLoad)
% INPUT:
%	sysTest = [0,1,2,3]
%    	0: No test
%     	1: PID calibration
...
% -----------------------------MAIN SETUP----------------------------------
    function[] = Setup()
        ...
        timer_save.TimerFcn = {@ShowBtnStatus, 'Save'};
        ...
        timer_quit.TimerFcn = {@ShowBtnStatus, 'Quit'};
...
% ------------------------------MAIN RUN-----------------------------------
    function[] = Run()
        ...
        function[] = UI_Setup()
            ...
            D.UI.toggTTplotType(1) = uicontrol('Style','togglebutton', ...
                'Parent',D.UI.figICR, ...
                'Enable', 'off', ...
                'Units','Normalized', ...
                'Position', pos, ...
        ...
        function [] = BtnSave(~, ~, ~)
            ...
            % Only run once
            if get(D.UI.btnSave, 'UserData') == 1
                return
            end
            
            % Disable save button so only pressed once
            set(D.UI.btnSave, 'UserData', 1);
        ...
        function [] = BtnQuit(~, ~, ~)
            ...
            % Only run once
            if get(D.UI.btnQuit, 'UserData') == 1
                return
            end
        ...
        % SET PATCH STATE
        function [] = Set_Patch_State(hand, setting, col)
            % Note:
            %   setting = ['Hide', 'ShowPartial', 'ShowAll', 'Select', 'Active']
...
% ------------------------BUTTON STATUS RUN TIMER--------------------------
    function [] = ShowBtnStatus(~, ~, btn_str)
        persistent cnt_dot
        if isempty(cnt_dot)
            cnt_dot = 1;
        end
