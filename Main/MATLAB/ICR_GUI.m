...
% --------------------------TT TRACKING SETUP----------------------
    function[was_ran] = TT_Track_Setup()
        ...
        % Get tt list
        D.TT.ttList = D.TT_IO_1.TT_Label{D.PAR.ratIndTT,:};
        ...
        % Get included TTs
        D.TT.ttInclude = D.TT_IO_1.TT_Include{D.PAR.ratIndTT,:};
        ...
        % Create 4 impedance txt boxes
        pos = [dflt_lf(1), dflt_btm(6), dflt_wd/2, dflt_ht/2];
        D.UI.editImpTT = gobjects(4,1);
        for z_t = 1:4
...
% --------------------------FINISH EPHYS SETUP------------------------
    function[was_ran] = NLX_Setup()
        ...
        if D.F.implant_session
            ...
            rec_fi_list = [D.TT_IO_2.(D.PAR.ratLab).Recording_File; rec_fi_list];
            ...
            for z_c = 1:length(clust_plot)
                ...
                if get(D.UI.toggPlotTypeTT(2),'Value') == 1
                    ...
                    if D.PAR.sesTask == 'Track'
                        ...
                        patch_h = findobj(D.UI.ttClustAxNow(tt_ind, clust_ind),'Type','patch');
        ...
        switch set_case
            ...
            case 'TT_Turn_Panel_Objects'
                ...
                if strcmp(setting_str, 'Enable')
                    ...
                    Safe_Set([D.UI.txtPanTT(7),D.UI.txtPanTT(8)], ...
                        'String', 'XX', ...
                        'ForegroundColor', D.UI.disabledCol);
...
% ---------------------BUTTON PRESS TO LOAD TT DATA------------------------
    function Togg_SelectTT(hObject, ~, ~)
        ...
        % Update ref
        D.UI.popRefTT.Value = ...
            find(ismember(D.TT.refList, char(D.TT_IO_1.TT_Reference{D.PAR.ratIndTT,:}(ismember(D.TT.ttList, D.TT.ttFldNow)))));
...
% --------------------BUTTON PRESS TO SAVE TT ENTRIES----------------------
    function Togg_UpdateLogTT(~, ~, ~)
        ...
        if val == 0
            ...
            D.TT.ttLogNew.([D.TT.ttFldNow,'_I']) = D.TT_IO_2.(D.PAR.ratLab).([D.TT.ttFldNow,'_I'])(end,:);
        ...
        if val == 1
            ...
            D.TT.ttLogNew.([D.TT.ttFldNow,'_I']) = imp_num_new;
            ...
            for z_tt = 1:length(tt_list)
                ...
                % Save for session
                tt_fld = tt_list{z_tt};
                D.TT_IO_1.TT_Reference{D.PAR.ratIndTT,:}(ismember(D.TT.ttList,tt_fld)) = ref_str;
            ...
            Safe_Set(D.UI.txtPanTT(7), 'String', or_new);
            txt = sprintf('%0.0f %sm', depth_new, char(181));
            Safe_Set(D.UI.txtPanTT(8), 'String', txt);
    ...
    function Togg_MainActionTT(hObject, ~, ~)
        ...
        function SetPlotTTs(setting_str)
            ...
            if strcmp(setting_str, 'Activate')
                ...
                % Activate and enable both plot type toggles
...
% ----------------------------ENABLE/DISABLE TT----------------------------
    function Togg_IncludeTT(hObject, ~, ~)
        ...
        D.TT_IO_1.TT_Include{D.PAR.ratIndTT,:} = D.TT.ttInclude;
