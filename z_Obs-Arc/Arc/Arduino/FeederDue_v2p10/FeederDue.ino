...
const float defualtSetPoint = 46; // (cm)
...
// Reward
const uint32_t solDir = 3000; // (ms) sol open duration
...
public:
	...
	void UpdatePID()
	{
		...
		if (mode == "Automatic")
		{
			...
			if (ekf_ratVel < 1 && error < -5 && !isHolding4cross)
			{
				// halt running
				HardStop("[UpdatePID]");
			}
		...
		if (mode == "On")
		{
				...
				millis() > t_loopNext)
			{
				...
				else
				{
					...
					// Stop bulldoze if rat ahead of set point
					if (passedReset &&
						state == "Active")
					{
...
bool DoDecelerate(String move_type)
{
	...
	dec_pos = move_type == "MoveTo" ? 40 : 20;
