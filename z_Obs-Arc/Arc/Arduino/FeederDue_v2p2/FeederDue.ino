...
const bool doPrintSentPack = true;
const bool doPrintMissedPack = false;
...
bool isPosUpdated[3] = { false, false, false };
volatile bool isRunHalted = true;
volatile bool doZeroIntegral = true;
bool doMove = false;
bool isMoving = false;
bool isDoneMoving = false;
volatile bool doRew = false;
volatile bool isRewarding = false;
...
uint16_t r2c_doneHist[10];
const int r2c_dhLng = sizeof(r2c_doneHist) / sizeof(r2c_doneHist[0]);
...
void loop() {
	...
	else
	{
		...
		if (doResend)
		{
			// Resend if last done pack matches
			if (r2c_doneHist[r2c_dhLng - 1] == c2r_packLast[7])
			{
				Send2_CS(r2c_id[5], r2c_doneHist[r2c_dhLng - 1]);
				doResend = false;
				PrintState("COMMAND 'DONE' RESENT");
			}
		}
		...
		if ((isPosUpdated[0] && isPosUpdated[2]) || isPosUpdated[1])
		{
			// Update pos values
			UpdatePos();
		}
...
bool ParseSerial()
{
	...
	else
	{
		...
		if (msg_id != c2r_id[5] && msg_id != c2r_id[7])
		{
			...
			c2r_packHist[c2r_phLng - 1] = pack_now;
		}
		...
		if (pack_diff > 0)
		{
			...
			if (pack_diff - 1 > 0)
			{
				PrintMissedPack((pack_diff - 1), missed_packs, pack_tot);
			}
			// Save new packet
			pack_last = pack_now;
			return pass = true;
...
void PrintMissedPack(uint16_t missed, int missed_total, uint32_t total)
{
	if (doPrintMissedPack)
	{
		char str[50];
...
void Send2_CS(char id, uint16_t pack)
{
	...
	if (id == r2c_id[5])
	{
		for (int i = 0; i < r2c_dhLng - 1; i++)
		{
			r2c_doneHist[i] = r2c_doneHist[i + 1];
		}
		r2c_doneHist[r2c_dhLng - 1] = pack;
...
void UpdatePos()
{
	...
	if (isPosUpdated[1])
	{
		...
		isPosUpdated[1] = false;
	}

	// UPDATE RAT
	if (isPosUpdated[0] && isPosUpdated[2])
	{
		if (do_simulateRat)
		{
		...
		// Check for new data next loop
		isPosUpdated[0] = false;
		isPosUpdated[2] = false;
		if (do_simulateRat)
		{
			isPosUpdated[2] = true;
		}
...
void UpdateSpeed()
{
	...
	if (estRatVel < 1 && error < 0 && !isRunHalted)
	{
		Interupt_Halt();
	}

	// Update AutoDriver
	if (!isRunHalted || (error > 0))
	{
		//board.run(FWD, runSpeed*cm2stp);
		isRunHalted = false;
		doZeroIntegral = false;
